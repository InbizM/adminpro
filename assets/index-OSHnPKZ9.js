(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e={},t=null,n=null;function r(t,n){e[t]=n}function i(e){t=e}function a(e){n=e}async function o(r){if(n){let e=n(r);if(e===!1)return;typeof e==`string`&&(r=e)}document.querySelectorAll(`[data-view]`).forEach(e=>{e.classList.add(`hidden`)});let i=document.querySelector(`[data-view="${r}"]`);if(!i)return console.warn(`View not found:`,r);i.classList.remove(`hidden`),t&&t(r),e[r]&&await e[r]()}var s=`https://adminpro-adminpro.aws-us-west-2.turso.io/v2/pipeline`,c=`eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3NzcyOTIwOTMsImlkIjoiMDE5ZGNlZGItOTIwMS03NGVkLWIwZGYtZjg4MTQ3NjlhODcxIiwicmlkIjoiNWIwMWViNTctYTgxYS00OTI0LWIzMDUtZjk1Y2EwMjUzNmRkIn0.oruUZmv_ZLWlKA2ctQnghAD5PIiJSIeR4nzbZia-q-f1r12IHhLv1hDw9CsReABIceaVRHPS52JMZ4j3lcZ1Bw`,l=`https://script.google.com/macros/s/AKfycbxxs-PUyNqqALN2-azEHaViv5PM1r5oteanvr2Sfydic-bBQJrGWj00R0FO7UAlP4Ug/exec`,u=localStorage.getItem(`adminpro_gas_token`)||``,d=e=>{u=e,localStorage.setItem(`adminpro_gas_token`,e)},f=()=>u,ee=()=>{localStorage.clear(),location.reload()};async function p(e){let t=Array.isArray(e)?e.map(e=>typeof e==`string`?{type:`execute`,stmt:{sql:e}}:e.type?e:{type:`execute`,stmt:e}):[{type:`execute`,stmt:typeof e==`string`?{sql:e}:e}],n=await(await fetch(s,{method:`POST`,headers:{Authorization:`Bearer ${c}`,"Content-Type":`application/json`},body:JSON.stringify({requests:t})})).json();if(n.error)throw Error(n.error.message);return(n.results||[]).map(e=>{if(!e.response||!e.response.result)return[];let{cols:t,rows:n}=e.response.result;return n.map(e=>{let n={};return t.forEach((t,r)=>{n[t.name]=e[r].value}),n})})}var m=e=>e.map(e=>({type:typeof e==`number`?`float`:`text`,value:String(e)}));async function te(e){return await(await fetch(l,{method:`POST`,mode:`cors`,body:JSON.stringify(e)})).json()}var ne=async(e,t,n)=>(await te({action:`uploadFoto`,token:u,base64Data:e,fileName:t,mimeType:n})).url||``,re=async(e,t)=>(await te({action:`uploadSignature`,token:u,base64Data:e,fileName:t})).url||``,ie=async(e,t,n)=>(await te({action:`uploadEvidencia`,token:u,base64Data:e,fileName:t,mimeType:n})).url||``,ae=async(e,t)=>{let n=await p({sql:`SELECT * FROM usuarios WHERE email = ? AND password = ? AND estado = 'Activo'`,args:[{type:`text`,value:e.toLowerCase()},{type:`text`,value:t}]});return!n[0]||n[0].length===0?{success:!1,mensaje:`Credenciales incorrectas`}:await(await fetch(`${l}?action=login&email=${encodeURIComponent(e)}&password=${t}`)).json()},oe=async(e,t)=>{let n=await(await fetch(`${l}?action=verifyPin&email=${encodeURIComponent(e)}&pin=${t}`)).json();if(n.success){d(n.token);let t={...(await p({sql:`SELECT nombre, rol, email FROM usuarios WHERE email = ?`,args:[{type:`text`,value:e.toLowerCase()}]}))[0]?.[0]||{nombre:`Usuario`},token:n.token};return localStorage.setItem(`adminpro_user`,JSON.stringify(t)),{success:!0,...t}}return n},se=async()=>(await p(`SELECT * FROM usuarios ORDER BY nombre ASC`))[0]||[],ce=e=>p({sql:`INSERT INTO usuarios (email, password, nombre, rol, estado) VALUES (?,?,?,?,?)`,args:m(e)}),le=(e,t,n)=>p({sql:`UPDATE usuarios SET email=?, password=?, nombre=?, rol=?, estado=? WHERE email=?`,args:[{type:`text`,value:t},...m(n),{type:`text`,value:e}]}),ue=e=>p({sql:`DELETE FROM usuarios WHERE email = ?`,args:[{type:`text`,value:e}]}),de=async()=>((await p(`SELECT * FROM clientes ORDER BY nombre ASC`))[0]||[]).map(e=>({...e,cedula:e.id})),fe=e=>p({sql:`INSERT INTO clientes VALUES (?,?,?,?,?,?,?)`,args:m([e.cedula,e.nombre,e.telefono,e.direccion,e.email,e.tipo,new Date().toISOString()])}),pe=(e,t)=>p({sql:`UPDATE clientes SET id=?, nombre=?, telefono=?, direccion=?, email=?, tipo=? WHERE id=?`,args:[...m([t.cedula,t.nombre,t.telefono,t.direccion,t.email,t.tipo]),{type:`text`,value:e}]}),me=e=>p({sql:`DELETE FROM clientes WHERE id = ?`,args:[{type:`text`,value:e}]}),he=async()=>(await p(`SELECT * FROM inventario ORDER BY id DESC`))[0].map(e=>({...e,stockActual:e.stock_actual,stockMinimo:e.stock_minimo,precioVenta:e.precio_venta,costo:e.costo})),ge=e=>p({sql:`INSERT INTO inventario VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),_e=(e,t)=>p({sql:`UPDATE inventario SET id=?, nombre=?, marca=?, categoria=?, tipo=?, costo=?, precio_venta=?, stock_minimo=?, stock_actual=?, ubicacion=?, sku=?, imagen=? WHERE id=?`,args:[...m(t),{type:`text`,value:e}]}),ve=e=>p({sql:`DELETE FROM inventario WHERE id = ?`,args:[{type:`text`,value:e}]}),ye=async()=>(await p(`SELECT * FROM equipos`))[0]||[],be=e=>p({sql:`INSERT INTO equipos VALUES (?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),xe=(e,t)=>p({sql:`UPDATE equipos SET imei1=?, imei2=?, id_producto=?, marca=?, nombre=?, proveedor=?, costo=?, venta=?, estado=?, fecha_ingreso=? WHERE imei1=?`,args:[...m(t),{type:`text`,value:e}]}),Se=e=>p({sql:`DELETE FROM equipos WHERE imei1 = ?`,args:[{type:`text`,value:e}]}),Ce=async()=>(await p(`SELECT * FROM ventas ORDER BY fecha DESC`))[0]||[],we=async e=>{let t=`FAC-${Date.now()}`,n=new Date().toISOString(),r=[{sql:`INSERT INTO ventas VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:[{type:`text`,value:t},{type:`text`,value:n},{type:`text`,value:e.cedula||``},{type:`text`,value:e.cliente||``},{type:`text`,value:e.direccion||``},{type:`text`,value:e.productoNombre||``},{type:`text`,value:String(e.total||0)},{type:`text`,value:String(e.items?.length||1)},{type:`text`,value:e.imeis||`N/A`},{type:`float`,value:e.subtotal||0},{type:`float`,value:e.descuento||0},{type:`float`,value:e.total||0},{type:`text`,value:e.metodo||``},{type:`text`,value:e.vendedor||``},{type:`text`,value:e.firmaVendedor||``},{type:`text`,value:e.firmaComprador||``},{type:`text`,value:e.evidencia||``},{type:`text`,value:e.ciudad||``},{type:`text`,value:e.telefono||``},{type:`text`,value:e.tipoFactura||`fisica`}]}];return e.items&&Array.isArray(e.items)?e.items.forEach(e=>{r.push({sql:`UPDATE inventario SET stock_actual = stock_actual - ? WHERE id = ?`,args:[{type:`float`,value:parseFloat(e.qty||1)},{type:`text`,value:e.id}]})}):r.push({sql:`UPDATE inventario SET stock_actual = stock_actual - 1 WHERE id = ?`,args:[{type:`text`,value:e.productoId||``}]}),await p(r),{success:!0,idFactura:t}},Te=async()=>(await p(`SELECT * FROM creditos`))[0].map(e=>({...e,id:e.id_credito,idFactura:e.id_factura_ref,abonado:e.total_abonado,saldo:e.saldo_pendiente,total:e.valor_total})),Ee=e=>p({sql:`INSERT INTO creditos (id_credito, cliente, telefono, id_factura_ref, fecha_deuda, tipo, valor_total, total_abonado, saldo_pendiente, estado, detalle) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,args:m([Date.now().toString(),e.cliente,e.telefono,e.idFactura,new Date().toISOString(),e.tipo||`Crédito`,e.total,0,e.total,`Activo`,e.detalle])}),De=(e,t)=>p({sql:`UPDATE creditos SET id_credito=?, cliente=?, telefono=?, id_factura_ref=?, fecha_deuda=?, tipo=?, valor_total=?, total_abonado=?, saldo_pendiente=?, estado=?, fecha_cancelacion=?, detalle=?, historial_abonos=? WHERE id_credito=?`,args:[...m(t),{type:`text`,value:e}]}),Oe=async()=>(await p(`SELECT * FROM reventas ORDER BY fecha DESC`))[0].map(e=>({...e,id:e.id_reventa,producto:e.producto,costo:e.costo_proveedor,precio:e.precio_venta,utilidad:e.utilidad})),ke=e=>p({sql:`INSERT INTO reventas VALUES (?,?,?,?,?,?,?,?)`,args:m([`REV-${Date.now()}`,new Date().toISOString(),e.producto,e.categoria,e.costo,e.precio,e.proveedor,e.precio-e.costo])}),Ae=e=>p({sql:`DELETE FROM reventas WHERE id_reventa = ?`,args:[{type:`text`,value:e}]}),je=async()=>(await p(`SELECT * FROM servicio_tecnico ORDER BY id_orden DESC`))[0]||[],Me=e=>p({sql:`INSERT INTO servicio_tecnico VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),Ne=(e,t)=>p({sql:`UPDATE servicio_tecnico SET id_orden=?, cliente=?, telefono=?, equipo=?, imei_serie=?, falla=?, clave_patron=?, repuestos=?, costo_taller=?, abono=?, precio_final=?, estado=?, evidencias=? WHERE id_orden=?`,args:[...m(t),{type:`text`,value:e}]}),Pe=e=>p({sql:`DELETE FROM servicio_tecnico WHERE id_orden = ?`,args:[{type:`text`,value:e}]}),Fe=async()=>{let e=await p([`SELECT COUNT(*) as c FROM inventario`,`SELECT COUNT(*) as c FROM clientes`,`SELECT SUM(total) as s FROM ventas WHERE date(fecha) = date('now')`,`SELECT SUM(monto) as s FROM egresos WHERE date(fecha) = date('now')`,`SELECT SUM(stock_actual) as s FROM inventario`,`SELECT COUNT(*) as c FROM inventario WHERE stock_actual <= 1`,`SELECT * FROM ventas ORDER BY fecha DESC LIMIT 8`,`SELECT date(fecha) as d, SUM(total) as m FROM ventas WHERE date(fecha) >= date('now','-7 days') GROUP BY d`,`SELECT COUNT(*) as c FROM equipos`,`SELECT productos, COUNT(*) as qty FROM ventas GROUP BY productos ORDER BY qty DESC LIMIT 5`,`SELECT id, nombre, stock_actual, stock_minimo FROM inventario WHERE stock_actual <= 1 LIMIT 5`,`SELECT id_orden, cliente, equipo, estado FROM servicio_tecnico ORDER BY id_orden DESC LIMIT 5`,`SELECT COUNT(*) as c FROM creditos WHERE estado != 'Pagado' AND estado != 'Cancelado'`,`SELECT COUNT(*) as c FROM reventas`]),t=[],n=[];for(let r=6;r>=0;r--){let i=new Date;i.setDate(i.getDate()-r);let a=i.toISOString().slice(0,10);t.push(i.toLocaleDateString(`es-CO`,{weekday:`short`,day:`numeric`}));let o=(e[7]||[]).find(e=>e.d===a);n.push(o?o.m:0)}return{ingresosHoy:e[2]?.[0]?.s||0,egresosHoy:e[3]?.[0]?.s||0,utilidad:(e[2]?.[0]?.s||0)-(e[3]?.[0]?.s||0),totalProductos:e[0]?.[0]?.c||0,totalClientes:e[1]?.[0]?.c||0,totalStock:e[4]?.[0]?.s||0,stockCritico:e[5]?.[0]?.c||0,totalEquipos:e[8]?.[0]?.c||0,ventasRecientes:e[6]||[],topProductos:(e[9]||[]).map(e=>({nombre:e.productos,cantidad:e.qty})),productosBajoStock:(e[10]||[]).map(e=>({...e,stockActual:e.stock_actual})),tecRecientes:e[11]||[],creditosActivos:e[12]?.[0]?.c||0,totalReventas:e[13]?.[0]?.c||0,labels7d:t,ventas7d:n}},Ie=async()=>((await p(`SELECT * FROM egresos ORDER BY fecha DESC`))[0]||[]).map(e=>({...e,id:e.id_gasto})),Le=e=>p({sql:`INSERT INTO egresos VALUES (?,?,?,?,?,?,?)`,args:m([`EGR-${Date.now()}`,new Date().toISOString(),e.categoria,e.concepto,e.responsable,e.monto,``])}),Re=async()=>((await p(`SELECT * FROM nominas ORDER BY fecha DESC`))[0]||[]).map(e=>({...e,id:e.id_nomina})),ze=e=>p({sql:`INSERT INTO nominas VALUES (?,?,?,?,?,?,?,?,?,?)`,args:m([`NOM-${Date.now()}`,e.fecha||new Date().toISOString(),e.empleado,e.periodo,e.salario_base,e.deducciones,e.bonificaciones,e.total_pagar,e.estado||`Pendiente`,e.notas||``])}),Be=e=>p({sql:`DELETE FROM nominas WHERE id_nomina = ?`,args:[{type:`text`,value:e}]}),Ve=async()=>(await p(`SELECT * FROM tareas ORDER BY date(fecha_vencimiento) ASC`))[0]||[],He=e=>p({sql:`INSERT INTO tareas VALUES (?,?,?,?,?,?,?,?,?)`,args:m([`T-${Date.now()}`,e.tarea,e.fecha_inicio,e.fecha_vencimiento,e.prioridad,e.estado||`Pendiente`,e.responsable,e.notas||``,e.color||`#4f46e5`])}),Ue=(e,t)=>p({sql:`UPDATE tareas SET estado = ? WHERE id = ?`,args:[{type:`text`,value:t},{type:`text`,value:e}]}),We=e=>p({sql:`DELETE FROM tareas WHERE id = ?`,args:[{type:`text`,value:e}]});function h(e,t=`success`){let n={success:`bg-green-600`,error:`bg-error`,info:`bg-tertiary`,warning:`bg-amber-500`},r={success:`check_circle`,error:`error`,info:`info`,warning:`warning`},i=document.createElement(`div`);i.className=`fixed bottom-4 right-4 z-[9999] toast-animate`,i.innerHTML=`
  ...
  `,document.body.appendChild(i),setTimeout(()=>i.remove(),4500)}var Ge=`modulepreload`,Ke=function(e){return`/adminpro/`+e},qe={},Je=function(e,t,n){let r=Promise.resolve();if(t&&t.length>0){let e=document.getElementsByTagName(`link`),i=document.querySelector(`meta[property=csp-nonce]`),a=i?.nonce||i?.getAttribute(`nonce`);function o(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}r=o(t.map(t=>{if(t=Ke(t,n),t in qe)return;qe[t]=!0;let r=t.endsWith(`.css`),i=r?`[rel="stylesheet"]`:``;if(n)for(let n=e.length-1;n>=0;n--){let i=e[n];if(i.href===t&&(!r||i.rel===`stylesheet`))return}else if(document.querySelector(`link[href="${t}"]${i}`))return;let o=document.createElement(`link`);if(o.rel=r?`stylesheet`:Ge,r||(o.as=`script`),o.crossOrigin=``,o.href=t,a&&o.setAttribute(`nonce`,a),document.head.appendChild(o),r)return new Promise((e,n)=>{o.addEventListener(`load`,e),o.addEventListener(`error`,()=>n(Error(`Unable to preload CSS for ${t}`)))})}))}function i(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return r.then(t=>{for(let e of t||[])e.status===`rejected`&&i(e.reason);return e().catch(i)})},Ye=!1,g=null,Xe=`environment`,_=!1,v=null,Ze=null,y=null,Qe=null,b,$e,et,tt,x,S,nt,rt,it,C,at,w;async function ot(){if(typeof BarcodeDetector<`u`)try{let e=await BarcodeDetector.getSupportedFormats(),t=[`qr_code`,`code_128`,`code_39`,`ean_13`,`ean_8`,`upc_a`,`upc_e`,`pdf417`].filter(t=>e.includes(t));y=new BarcodeDetector({formats:t.length?t:[`qr_code`,`code_128`,`ean_13`]}),Qe=`native`,console.log(`[Scanner] Motor: BarcodeDetector nativo`);return}catch{}let{readBarcodesFromImageData:e}=await Je(async()=>{let{readBarcodesFromImageData:e}=await import(`./reader-C_2V0ICK.js`);return{readBarcodesFromImageData:e}},[]);y=e,Qe=`zxing-wasm`,console.log(`[Scanner] Motor: zxing-wasm (C++ WASM)`)}async function st(e){if(e!==`environment`)return null;try{(await navigator.mediaDevices.getUserMedia({video:!0})).getTracks().forEach(e=>e.stop());let e=(await navigator.mediaDevices.enumerateDevices()).filter(e=>e.kind===`videoinput`),t=e.filter(e=>/back|rear|environment|posterior|trasera/i.test(e.label)),n=t.length?t:e,r=n.filter(e=>!/ultra|wide|macro|depth|tof|ir\b/i.test(e.label)),i=r.length?r:n,a=i[0]?.deviceId,o=-1;for(let e of i)try{let t=await navigator.mediaDevices.getUserMedia({video:{deviceId:{exact:e.deviceId},width:{ideal:1920}}}),n=t.getVideoTracks()[0],r=n.getSettings(),i=n.getCapabilities?.()??{},s=0,c=(r.width??0)*(r.height??0);c>=1280*720&&(s+=10),c>=1920*1080&&(s+=5),c>=3840*2160&&(s-=5),i.focusMode?.includes?.(`continuous`)&&(s+=20);let l=i.focusDistance?.min??999;l<.1&&(s+=15),l<.3&&(s+=8);let u=i.zoom?.min??1;u>1.5&&(s-=20),console.log(`[Scanner] ${e.label} → score:${s} px:${Math.round(c/1e3)}K focus:${l} zoom:${u}`),t.getTracks().forEach(e=>e.stop()),s>o&&(o=s,a=e.deviceId)}catch{}return console.log(`[Scanner] Elegida: ${a} (score: ${o})`),a}catch{return null}}function ct(){try{let e=new(window.AudioContext||window.webkitAudioContext),t=e.createOscillator(),n=e.createGain();t.connect(n),n.connect(e.destination),t.type=`sine`,t.frequency.setValueAtTime(800,e.currentTime),n.gain.setValueAtTime(.5,e.currentTime),n.gain.exponentialRampToValueAtTime(1e-5,e.currentTime+.15),t.start(e.currentTime),t.stop(e.currentTime+.15)}catch{}}function lt(){let e=document.getElementById(`scanner-modal`);if(e){if(document.getElementById(`scanner-video`))return;e.remove()}document.body.insertAdjacentHTML(`beforeend`,`
    ...
    `,tt.appendChild(a),setTimeout(()=>a.remove(),800);try{await t.applyConstraints({advanced:[{focusMode:`manual`}]}),await t.applyConstraints({advanced:[{pointOfInterest:{x:r,y:i}}]}),await new Promise(e=>setTimeout(e,400)),await t.applyConstraints({advanced:[{focusMode:`continuous`}]})}catch{try{await t.applyConstraints({advanced:[{focusMode:`manual`}]}),await new Promise(e=>setTimeout(e,200)),await t.applyConstraints({advanced:[{focusMode:`continuous`}]})}catch{}}})}function dt(){let e=rt.value.trim();e&&(g&&g(e),yt())}async function ft(){if(!v)return;let e=v.getVideoTracks()[0];if(e){_=!_;try{await e.applyConstraints({advanced:[{torch:_}]}),C.innerHTML=`<span class="material-symbols-outlined text-[20px]">${_?`flashlight_off`:`flashlight_on`}</span>`,_?C.classList.replace(`bg-black/50`,`bg-primary`):C.classList.replace(`bg-primary`,`bg-black/50`)}catch(e){console.error(`Torch not supported`,e),_=!1}}}async function pt(){Ye&&(Xe=Xe===`environment`?`user`:`environment`,S.textContent=`Cambiando cámara...`,mt(),_=!1,C.innerHTML=`<span class="material-symbols-outlined text-[20px]">flashlight_on</span>`,C.classList.remove(`bg-primary`),C.classList.add(`bg-black/50`),await ht())}function mt(){v&&=(v.getTracks().forEach(e=>e.stop()),null),x&&(x.srcObject=null),Ze&&=(cancelAnimationFrame(Ze),null)}async function ht(){try{let e={video:{facingMode:Xe}},t=await st(Xe);t&&(e.video={deviceId:{exact:t}}),v=await navigator.mediaDevices.getUserMedia(e),x.srcObject=v;let n=v.getVideoTracks()[0];if(n?.applyConstraints){try{await n.applyConstraints({advanced:[{focusMode:`continuous`}]})}catch{}try{await n.applyConstraints({zoom:1})}catch{}}await new Promise(e=>{x.onloadedmetadata=()=>{x.play(),e()}}),Ye=!0,S.textContent=`📷 Apunta al código de barras o QR...`,S.className=`px-5 py-3 text-center text-sm text-blue-800 bg-blue-50 font-medium`,gt()}catch(e){console.error(`Scanner error:`,e),S.textContent=`⚠️ No se pudo acceder a la cámara. Usa la entrada manual.`,S.className=`px-5 py-3 text-center text-sm text-amber-800 bg-amber-50`}}function gt(){Ye&&(x.readyState>=2&&!x.paused?_t().then(e=>{e?(ct(),S.textContent=`✅ Detectado: ${e}`,S.className=`px-5 py-3 text-center text-sm text-green-800 bg-green-50 font-bold`,g&&g(e),setTimeout(()=>yt(),600)):Ze=requestAnimationFrame(gt)}).catch(e=>{Ze=requestAnimationFrame(gt)}):Ze=requestAnimationFrame(gt))}async function _t(){let e=x.getBoundingClientRect(),t=w.getBoundingClientRect();if(e.width===0||e.height===0)return null;let n=e.width/x.videoWidth,r=e.height/x.videoHeight,i=Math.max(n,r),a=x.videoWidth*i,o=x.videoHeight*i,s=(a-e.width)/2,c=(o-e.height)/2,l=(t.left-e.left+s)/i,u=(t.top-e.top+c)/i,d=t.width/i,f=t.height/i;if(d<=0||f<=0)return null;let ee=d<400?400/d:1,p=document.createElement(`canvas`);p.width=Math.round(d*ee),p.height=Math.round(f*ee);let m=p.getContext(`2d`,{willReadFrequently:!0});if(m.imageSmoothingEnabled=!1,m.drawImage(x,l,u,d,f,0,0,p.width,p.height),Qe===`native`&&y)try{let e=await y.detect(p);if(e.length>0)return e[0].rawValue}catch{}else if(Qe===`zxing-wasm`&&y)try{let e=m.getImageData(0,0,p.width,p.height),t=await y(e,{tryHarder:!0,formats:[`QRCode`,`Code128`,`Code39`,`EAN13`,`EAN8`,`UPCA`,`UPCE`,`PDF417`,`DataMatrix`],maxNumberOfSymbols:1});if(t.length>0)return t[0].text}catch{}return null}async function vt({title:e=`Escanear Código`,onScan:t}={}){await ot(),lt(),g=t,Xe=`environment`,_=!1,Ye=!1,nt.textContent=e,rt.value=``,S.textContent=`Iniciando cámara...`,S.className=`px-5 py-3 text-center text-sm text-on-surface-variant bg-surface-container-low`,C&&(C.innerHTML=`<span class="material-symbols-outlined text-[20px]">flashlight_on</span>`,C.classList.remove(`bg-primary`),C.classList.add(`bg-black/50`)),w&&(w.style.width=`280px`,w.style.height=`120px`),b.classList.remove(`hidden`),b.classList.add(`flex`),await ht()}async function yt(){Ye=!1,mt(),b&&(b.classList.add(`hidden`),b.classList.remove(`flex`)),g=null}var T=[],E=[],bt=null,xt=`grid`;function St(e,t){let n=Number(e),r=Number(t);return n===0?{label:`Sin Stock`,cls:`bg-red-100 text-red-800 border-red-200`,icon:`block`}:n<=r?{label:`Bajo: ${n}`,cls:`bg-amber-100 text-amber-800 border-amber-200`,icon:`warning`}:{label:`OK: ${n}`,cls:`bg-emerald-100 text-emerald-800 border-emerald-200`,icon:`check_circle`}}function Ct(){let e=document.getElementById(`inv-grid`),t=document.getElementById(`inv-table-wrapper`),n=document.getElementById(`inv-table-body`);if(!e)return;let r=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`),i=r.rol===`Administrador`,a=r.rol===`Técnico de reparación`;xt===`grid`?(t.classList.add(`hidden`),e.classList.remove(`hidden`),e.innerHTML=E.length?E.map(e=>{let t=St(e.stockActual,e.stockMinimo),n=Number(e.stockActual)===0,r=Number(String(e.precioVenta).replace(/\D/g,``)).toLocaleString(`es-CO`);return`
         ...
         </div>`):(e.classList.add(`hidden`),t.classList.remove(`hidden`),n.innerHTML=E.length?E.map(e=>{let t=St(e.stockActual,e.stockMinimo),n=Number(e.stockActual)===0,r=Number(String(e.precioVenta).replace(/\D/g,``)).toLocaleString(`es-CO`),o=Number(String(e.costo||0).replace(/\D/g,``)).toLocaleString(`es-CO`);return`
          ...
          `}).join(``):`<tr><td colspan="6" class="text-center py-10 text-on-surface-variant">No hay productos</td></tr>`);let o=document.getElementById(`inv-stat-total`),s=document.getElementById(`inv-stat-alert`);o&&(o.textContent=T.length.toLocaleString()),s&&(s.textContent=T.filter(e=>Number(e.stockActual)<=Number(e.stockMinimo)).length)}function wt(){let e=(document.getElementById(`inv-search`)?.value||``).toLowerCase(),t=document.getElementById(`inv-filter-cat`)?.value||``,n=document.getElementById(`inv-filter-tipo`)?.value||``;E=T.filter(r=>(!e||[r.nombre,r.marca,r.sku,r.id].some(t=>String(t).toLowerCase().includes(e)))&&(!t||r.categoria===t)&&(!n||(r.tipo||``)===n)),Ct()}function Tt(e){document.getElementById(`inv-modal-title`).textContent=e,document.getElementById(`inv-modal`).classList.remove(`hidden`),document.getElementById(`inv-modal`).classList.add(`flex`)}function Et(){document.getElementById(`inv-modal`).classList.add(`hidden`),document.getElementById(`inv-modal`).classList.remove(`flex`),bt=null,window.__posReventaMode=!1,document.getElementById(`inv-form`).reset(),document.getElementById(`inv-img-preview`).innerHTML=`<span class="material-symbols-outlined text-3xl text-on-surface-variant/40">add_photo_alternate</span>`}function Dt(e){let t=e.target.files[0];if(!t)return;let n=new FileReader;n.onload=e=>{document.getElementById(`inv-img-preview`).innerHTML=`<img src="${e.target.result}" class="w-full h-full object-cover">`},n.readAsDataURL(t)}async function Ot(){let e=document.getElementById(`inv-save-btn`);e.disabled=!0,e.innerHTML=`<span class="material-symbols-outlined animate-spin text-[18px]">progress_activity</span> Guardando...`;try{let t=document.getElementById(`inv-existing-img`).value,n=document.getElementById(`inv-img-file`);if(n.files[0]){let r=n.files[0];e.innerHTML=`<span class="material-symbols-outlined animate-spin text-[18px]">progress_activity</span> Subiendo foto...`;let i=new FileReader;t=await new Promise((e,t)=>{i.onload=async n=>{try{let t=await ne(n.target.result,r.name,r.type);e(t.url||t)}catch(e){t(e)}},i.readAsDataURL(r)})}let r=[document.getElementById(`inv-id`).value,document.getElementById(`inv-nombre`).value,document.getElementById(`inv-marca`).value,document.getElementById(`inv-categoria`).value,document.getElementById(`inv-tipo`).value,document.getElementById(`inv-costo`).value.replace(/\D/g,``),document.getElementById(`inv-venta`).value.replace(/\D/g,``),document.getElementById(`inv-stock-min`).value,document.getElementById(`inv-stock-act`).value,document.getElementById(`inv-ubicacion`).value,document.getElementById(`inv-sku`).value,t],i;if(i=bt?await _e(bt,r):await ge(r),h(i.mensaje||`Guardado correctamente`,i.success?`success`:`error`),i.success){if(window.__posReventaMode&&!bt){let e={id:r[0],nombre:r[1],marca:r[2],categoria:r[3],costo:r[5],precioVenta:r[6]};typeof window.__posAddReventaToCart==`function`&&window.__posAddReventaToCart(e),window.__posReventaMode=!1}Et(),await At()}}catch(e){h(`Error: `+e.message,`error`)}finally{e.disabled=!1,e.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}}var kt=null;window.inventoryView={openDetail(e){let t=T.find(t=>t.id===e);if(!t)return;kt=e;let n=e=>Number(String(e||0).replace(/\D/g,``)||0).toLocaleString(`es-CO`),r=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`),i=r.rol===`Técnico de reparación`;document.getElementById(`inv-d-nombre`).textContent=t.nombre||`—`,document.getElementById(`inv-d-marca`).textContent=t.marca||`—`,document.getElementById(`inv-d-cat`).textContent=t.categoria||`—`,document.getElementById(`inv-d-costo`).textContent=i?`N/A`:`$${n(t.costo)}`,document.getElementById(`inv-d-venta`).textContent=i?`N/A`:`$${n(t.precioVenta)}`,document.getElementById(`inv-d-stock`).textContent=t.stockActual??`—`,document.getElementById(`inv-d-stockmin`).textContent=t.stockMinimo??`—`,document.getElementById(`inv-d-tipo`).textContent=t.tipo||`—`,document.getElementById(`inv-d-ubicacion`).textContent=t.ubicacion||`—`,document.getElementById(`inv-d-sku`).textContent=t.sku||t.id||`—`;let a=document.getElementById(`inv-detail-img-wrap`),o=document.getElementById(`inv-detail-img`);t.imagen?(o.src=t.imagen,a.classList.remove(`hidden`)):a.classList.add(`hidden`);let s=r.rol===`Administrador`,c=document.getElementById(`inv-detail-edit-btn`);c&&(s?c.classList.remove(`hidden`):c.classList.add(`hidden`));let l=document.getElementById(`inv-detail-modal`);l.classList.remove(`hidden`),l.classList.add(`flex`)},openEdit(e){let t=T.find(t=>t.id===e);if(!t)return;bt=e;let n=document.getElementById(`inv-detail-modal`);n.classList.add(`hidden`),n.classList.remove(`flex`),document.getElementById(`inv-id`).value=t.id,document.getElementById(`inv-nombre`).value=t.nombre,document.getElementById(`inv-marca`).value=t.marca,document.getElementById(`inv-categoria`).value=t.categoria,document.getElementById(`inv-tipo`).value=t.tipo||`Físico`,document.getElementById(`inv-costo`).value=t.costo?new Intl.NumberFormat(`es-CO`).format(t.costo):``,document.getElementById(`inv-venta`).value=t.precioVenta?new Intl.NumberFormat(`es-CO`).format(t.precioVenta):``,document.getElementById(`inv-stock-min`).value=t.stockMinimo,document.getElementById(`inv-stock-act`).value=t.stockActual,document.getElementById(`inv-ubicacion`).value=t.ubicacion,document.getElementById(`inv-sku`).value=t.sku,document.getElementById(`inv-existing-img`).value=t.imagen||``,t.imagen&&(document.getElementById(`inv-img-preview`).innerHTML=`<img src="${t.imagen}" class="w-full h-full object-cover">`),Tt(`Editar Producto`)},async deleteProduct(e){if(confirm(`¿Eliminar este producto?`))try{let t=await ve(e);h(t.mensaje||`Eliminado`,t.success?`success`:`error`),t.success&&await At()}catch(e){h(`Error: `+e.message,`error`)}},openNuevo(e=!1){bt=null,document.getElementById(`inv-form`)?.reset(),document.getElementById(`inv-tipo`).value=e?`Reventa`:`Físico`,document.getElementById(`inv-existing-img`).value=``,document.getElementById(`inv-img-preview`).innerHTML=`<span class="material-symbols-outlined text-3xl text-on-surface-variant/40">add_photo_alternate</span>`,e&&(document.getElementById(`inv-id`).value=`REV-`+Date.now().toString().slice(-6)),Tt(e?`Nueva Reventa`:`Nuevo Producto`)}};async function At(){let e=document.getElementById(`inv-grid`);e&&(e.innerHTML=`<div class="col-span-full flex justify-center py-20">
    <span class="material-symbols-outlined animate-spin text-4xl text-primary">progress_activity</span></div>`);try{let e=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Técnico de reparación`,t=await he();T=e?t.filter(e=>e.categoria&&e.categoria.toLowerCase().includes(`repuesto`)):t,E=[...T],Ct(),jt()}catch(t){e&&(e.innerHTML=`<div class="col-span-full text-center py-20 text-error font-semibold">
      <span class="material-symbols-outlined text-4xl">wifi_off</span>
      <p class="mt-2">Error al cargar: ${t.message}</p></div>`)}}function jt(){let e=document.getElementById(`inv-filter-cat`),t=[...new Set(T.map(e=>e.categoria).filter(Boolean))];e&&(e.innerHTML=`<option value="">Todas las categorías</option>`+t.map(e=>`<option value="${e}">${e}</option>`).join(``));let n=document.getElementById(`datalist-categorias`);n&&(n.innerHTML=t.map(e=>`<option value="${e}">`).join(``));let r=document.getElementById(`datalist-marcas`);r&&(r.innerHTML=[...new Set(T.map(e=>e.marca).filter(Boolean))].map(e=>`<option value="${e}">`).join(``))}function Mt(e){let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))}function Nt(){document.addEventListener(`barcodeScanned`,e=>{let t=document.querySelector(`[data-view="inventory"]`);if(!t||t.classList.contains(`hidden`))return;let n=e.detail,r=document.getElementById(`inv-modal`);if(r&&!r.classList.contains(`hidden`)){let e=document.getElementById(`inv-sku`);e&&(e.value=n,h(`SKU ingresado: ${n}`,`success`));return}let i=document.getElementById(`inv-search`);i&&(i.value=n,wt(),E.length===1?(inventoryView.openDetail(E[0].id),h(`Producto encontrado`,`success`)):E.length===0&&h(`No se encontró el producto`,`warning`))}),document.getElementById(`inv-search`)?.addEventListener(`input`,wt),document.getElementById(`inv-filter-cat`)?.addEventListener(`change`,wt),document.getElementById(`inv-filter-tipo`)?.addEventListener(`change`,wt),document.getElementById(`inv-view-toggle`)?.addEventListener(`click`,()=>{xt=xt===`grid`?`table`:`grid`;let e=document.getElementById(`inv-view-toggle`).querySelector(`span`);e.textContent=xt===`grid`?`view_list`:`grid_view`,Ct()}),document.getElementById(`inv-auto-id-btn`)?.addEventListener(`click`,()=>{document.getElementById(`inv-id`).value=`PROD-`+Date.now().toString().slice(-6)}),document.getElementById(`inv-new-btn`)?.addEventListener(`click`,()=>{window.inventoryView&&window.inventoryView.openNuevo&&window.inventoryView.openNuevo(!1)}),document.getElementById(`inv-costo`)?.addEventListener(`input`,Mt),document.getElementById(`inv-venta`)?.addEventListener(`input`,Mt),document.getElementById(`inv-modal-close`)?.addEventListener(`click`,Et),document.getElementById(`inv-modal-backdrop`)?.addEventListener(`click`,Et),document.getElementById(`inv-img-file`)?.addEventListener(`change`,Dt),document.getElementById(`inv-save-btn`)?.addEventListener(`click`,Ot),document.getElementById(`inv-scan-sku`)?.addEventListener(`click`,()=>{vt({title:`Escanear SKU / Barcode`,onScan:e=>{document.getElementById(`inv-sku`).value=e,h(`SKU Detectado: ${e}`,`success`)}})});let e=null,t=document.getElementById(`inv-quick-input-modal`),n=document.getElementById(`inv-qi-title`),r=document.getElementById(`inv-qi-input`),i=document.getElementById(`inv-qi-save`),a=document.getElementById(`inv-qi-cancel`),o=document.getElementById(`inv-qi-backdrop`);function s(i){e=i,n.textContent=i===`marca`?`Nueva Marca`:`Nueva Categoría`,r.value=``,t.classList.remove(`hidden`),t.classList.add(`flex`),setTimeout(()=>r.focus(),50)}function c(){t.classList.add(`hidden`),t.classList.remove(`flex`)}a.addEventListener(`click`,c),o.addEventListener(`click`,c),r.addEventListener(`keydown`,e=>{e.key===`Enter`&&i.click()}),i.addEventListener(`click`,()=>{let t=r.value.trim();if(t){if(e===`marca`){let e=document.getElementById(`datalist-marcas`);if(![...e.options].some(e=>e.value.toLowerCase()===t.toLowerCase())){let n=document.createElement(`option`);n.value=t,e.appendChild(n)}document.getElementById(`inv-marca`).value=t,h(`Marca "${t}" agregada`,`success`)}else{let e=document.getElementById(`datalist-categorias`),n=document.getElementById(`inv-filter-cat`);if(![...e.options].some(e=>e.value.toLowerCase()===t.toLowerCase())){let r=document.createElement(`option`);if(r.value=t,e.appendChild(r),n){let e=document.createElement(`option`);e.value=t,e.textContent=t,n.appendChild(e)}}document.getElementById(`inv-categoria`).value=t,h(`Categoría "${t}" agregada`,`success`)}c()}}),document.getElementById(`inv-add-marca-btn`)?.addEventListener(`click`,()=>s(`marca`)),document.getElementById(`inv-add-cat-btn`)?.addEventListener(`click`,()=>s(`cat`));let l=()=>{let e=document.getElementById(`inv-detail-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)};return document.getElementById(`inv-detail-close`)?.addEventListener(`click`,l),document.getElementById(`inv-detail-close2`)?.addEventListener(`click`,l),document.getElementById(`inv-detail-backdrop`)?.addEventListener(`click`,l),document.getElementById(`inv-detail-edit-btn`)?.addEventListener(`click`,()=>{kt&&inventoryView.openEdit(kt)}),At}var Pt=!1;function Ft(){let e=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol||`Vendedor`,t=document.getElementById(`dash-card-egresos`),n=document.getElementById(`dash-card-utilidad`),r=document.getElementById(`dash-kpi-row`),i=document.getElementById(`dash-qa-tecnico`),a=document.getElementById(`dash-qa-egresos`),o=document.getElementById(`dash-qa-vender`),s=document.getElementById(`dash-qa-creditos`),c=document.getElementById(`dash-ventas-section`),l=document.getElementById(`dash-chart-card`),u=document.getElementById(`dash-tec-queue-card`),d=document.getElementById(`dash-bottom-row`);[t,n,r,i,a,o,s,c,l,u,d].forEach(e=>{e&&(e.style.display=``)}),d&&(d.className=`grid grid-cols-1 lg:grid-cols-2 gap-5`),e===`Vendedor`?(t&&(t.style.display=`none`),n&&(n.style.display=`none`),i&&(i.style.display=`none`),a&&(a.style.display=`none`),u&&(u.style.display=`none`),d&&(d.className=`grid grid-cols-1 gap-5`)):e===`Técnico de reparación`&&(r&&(r.style.display=`none`),o&&(o.style.display=`none`),a&&(a.style.display=`none`),s&&(s.style.display=`none`),c&&(c.style.display=`none`),l&&(l.style.display=`none`),d&&(d.className=`grid grid-cols-1 gap-5`))}function It(){return async()=>{Lt(),Ft(),Pt||=(document.getElementById(`dash-refresh-btn`)?.addEventListener(`click`,Rt),document.querySelectorAll(`[data-goto]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.goto;t&&o(t)})}),!0),await Rt()}}function Lt(){let e=new Date().getHours(),t=e<12?`¡Buenos días! 👋`:e<18?`¡Buenas tardes! ☕`:`¡Buenas noches! 🌙`,n=document.getElementById(`dash-greeting`);n&&(n.textContent=t)}async function Rt(){try{let[e,t]=await Promise.all([Fe(),Ve()]);zt(`dash-ventas-hoy`,e.ingresosHoy,!0),zt(`dash-egresos-hoy`,e.egresosHoy,!0),zt(`dash-utilidad`,e.utilidad,!0),zt(`dash-stock-critico`,e.stockCritico),Bt(e.ventasRecientes),Vt(e.topProductos),Ht(e.productosBajoStock),Ut(e.tecRecientes),Gt(e.labels7d,e.ventas7d),Wt(t)}catch(e){console.error(`Dashboard error:`,e)}}function zt(e,t,n=!1){let r=document.getElementById(e);r&&(r.textContent=n?`$`+new Intl.NumberFormat(`es-CO`).format(t||0):(t||0).toLocaleString())}function Bt(e){let t=document.getElementById(`dash-ventas-list`);if(t){if(!e||e.length===0){t.innerHTML=`<p class="p-5 text-sm text-on-surface-variant text-center">No hay ventas hoy</p>`;return}t.innerHTML=e.map(e=>`
  ...
  `).join(``)}}function Vt(e){let t=document.getElementById(`dash-top-productos`);if(t){if(!e||e.length===0){t.innerHTML=`<p class="p-4 text-center text-xs text-on-surface-variant italic">Sin ventas aún</p>`;return}t.innerHTML=e.map((e,t)=>`
  ...
  `).join(``)}}function Ht(e){let t=document.getElementById(`dash-stock-alertas`);if(t){if(!e||e.length===0){t.innerHTML=`<p class="p-4 text-center text-xs text-on-surface-variant italic">Stock ok</p>`;return}t.innerHTML=e.map(e=>`
  ...
  `).join(``)}}function Ut(e){let t=document.getElementById(`dash-tec-list`);if(t){if(!e||e.length===0){t.innerHTML=`<p class="p-5 text-sm text-on-surface-variant text-center">Sin servicios</p>`;return}t.innerHTML=e.map(e=>`
  ...
  `).join(``)}}function Wt(e){let t=document.getElementById(`dash-tasks-list`);if(!t)return;let n=(e||[]).filter(e=>e.estado!==`Completada`).slice(0,5);if(n.length===0){t.innerHTML=`<p class="p-8 text-center text-xs text-on-surface-variant italic">No hay pendientes</p>`;return}t.innerHTML=n.map(e=>`
  ...
  `).join(``)}function Gt(e,t){let n=document.getElementById(`dash-chart`);if(!n)return;if(!t||t.length===0||t.every(e=>e===0)){n.innerHTML=`<div class="flex-1 flex items-center justify-center text-on-surface-variant text-xs italic opacity-50">Sin ventas</div>`;return}let r=Math.max(...t,1);n.innerHTML=t.map((t,n)=>{let i=Math.max(t/r*100,5);return`
    ...
    `}).join(``)}var D=[],Kt=!1,qt=null,Jt,Yt,Xt,Zt,Qt,$t,en;async function tn(){if(nn(),!Kt)try{D=await de(),Kt=!0}catch(e){console.error(`Error loading clients for selector`,e)}}function nn(){document.getElementById(`customer-selector-modal`)||(document.body.insertAdjacentHTML(`beforeend`,`
  ...
  `),Jt=document.getElementById(`customer-selector-modal`),Yt=document.getElementById(`cs-backdrop`),Xt=document.getElementById(`cs-close`),Zt=document.getElementById(`cs-search`),Qt=document.getElementById(`cs-new-btn`),$t=document.getElementById(`cs-results`),en=document.getElementById(`cs-new-form-area`),Xt.addEventListener(`click`,cn),Yt.addEventListener(`click`,cn),Zt.addEventListener(`input`,rn),Qt.addEventListener(`click`,()=>{en.classList.toggle(`hidden`),document.getElementById(`cs-new-doc`).focus()}),document.getElementById(`cs-cancel-new`).addEventListener(`click`,()=>{en.classList.add(`hidden`)}),document.getElementById(`cs-save-new`).addEventListener(`click`,async()=>{let e=document.getElementById(`cs-new-doc`).value.trim(),t=document.getElementById(`cs-new-nom`).value.trim(),n=document.getElementById(`cs-new-tel`).value.trim(),r=document.getElementById(`cs-new-email`).value.trim(),i=document.getElementById(`cs-new-dir`).value.trim(),a=document.getElementById(`cs-new-tipo`).value;if(!e||!t){h(`Documento y Nombre son obligatorios`,`warning`);return}let o=document.getElementById(`cs-save-new`);o.disabled=!0,o.textContent=`...`;try{let o=await fe({documento:e,cedula:e,nombre:t,telefono:n,direccion:i,email:r,tipo:a});if(o&&o.success){h(`Cliente creado`,`success`);let o={cedula:e,documento:e,nombre:t,telefono:n,direccion:i,email:r,tipo:a,id:e};D.push(o),en.classList.add(`hidden`),document.getElementById(`cs-new-doc`).value=``,document.getElementById(`cs-new-nom`).value=``,document.getElementById(`cs-new-tel`).value=``,document.getElementById(`cs-new-email`).value=``,document.getElementById(`cs-new-dir`).value=``,document.getElementById(`cs-new-tipo`).value=`General`,on(o)}else h(o.mensaje||`Error al crear`,`error`)}catch{h(`Error de conexión`,`error`)}finally{o.disabled=!1,o.textContent=`Guardar`}}))}function rn(){let e=Zt.value.toLowerCase().trim();if(!e){$t.innerHTML=`<li class="p-4 text-center text-sm text-on-surface-variant">Escribe para buscar...</li>`;return}an(D.filter(t=>(t.cedula||t.documento||``).toLowerCase().includes(e)||t.nombre&&t.nombre.toLowerCase().includes(e)||t.telefono&&t.telefono.toLowerCase().includes(e)).slice(0,20))}function an(e){if(e.length===0){$t.innerHTML=`<li class="p-4 text-center text-sm text-on-surface-variant">No se encontraron clientes.</li>`;return}$t.innerHTML=e.map(e=>{let t=e.cedula||e.documento||``;return`
    <li>
  ...
  `}).join(``),document.querySelectorAll(`.cs-item-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.doc,n=D.find(e=>(e.cedula||e.documento||``)===t);n&&(n.documento=t,on(n))})})}function on(e){qt&&qt(e),cn()}async function sn(e){await tn(),qt=e,Zt.value=``,D.length>0?an([...D].reverse().slice(0,20)):$t.innerHTML=`<li class="p-4 text-center text-sm text-on-surface-variant">No hay clientes. Crea uno nuevo.</li>`,en.classList.add(`hidden`),Jt.classList.remove(`hidden`),Jt.classList.add(`flex`),setTimeout(()=>Zt.focus(),100)}function cn(){Jt.classList.add(`hidden`),Jt.classList.remove(`flex`),qt=null}var O=[],k=[],ln=!1,un=!1,dn=null,A,fn,pn,mn,hn,gn,_n,vn,yn,bn,xn,j,Sn,M,N,Cn,wn,Tn,En,Dn,On,kn,An,jn,Mn,Nn,P,F,I;function Pn(){return async()=>{Fn(),ln||=(await In(),Rn(),Un(M,e=>e),Un(N,e=>e),Un(P,e=>F=e),!0),Ln(O),L()}}function Fn(){A=document.getElementById(`pos-search`),fn=document.getElementById(`pos-products-grid`),pn=document.getElementById(`pos-cart-items`),mn=document.getElementById(`pos-subtotal`),hn=document.getElementById(`pos-descuento`),gn=document.getElementById(`pos-total`),document.getElementById(`pos-pay-btn`),_n=document.getElementById(`pos-cliente-doc`),vn=document.getElementById(`pos-cliente-nombre`),yn=document.getElementById(`pos-checkout-modal`),bn=document.getElementById(`pos-checkout-close`),xn=document.getElementById(`pos-checkout-cancel`),j=document.getElementById(`pos-checkout-confirm`),Sn=document.getElementById(`pos-evidencia-file`),M=document.getElementById(`pos-canvas-cliente`),N=document.getElementById(`pos-canvas-vendedor`),Cn=document.getElementById(`pos-cliente-direccion`),wn=document.getElementById(`pos-cliente-ciudad`),Tn=document.getElementById(`pos-cliente-tel`),En=document.getElementById(`pos-digital-fields`),Dn=document.getElementById(`pos-signatures-container`),On=document.getElementById(`pos-imei-container`),kn=document.getElementById(`pos-imei-list`),An=document.getElementById(`pos-signature-modal`),jn=document.getElementById(`pos-sig-modal-close`),Mn=document.getElementById(`pos-sig-modal-clear`),Nn=document.getElementById(`pos-sig-modal-save`),P=document.getElementById(`pos-canvas-fullscreen`)}async function In(){try{O=(await he()).filter(e=>e.stockActual>0)}catch{O=[]}}function Ln(e){if(e.length===0){fn.innerHTML=`<p class="p-4 col-span-full text-center opacity-50 italic text-sm">Sin stock disponible</p>`;return}fn.innerHTML=e.map(e=>`
  ...
  `).join(``)}function Rn(){A.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();Ln(t?O.filter(e=>e.nombre.toLowerCase().includes(t)||e.sku.toLowerCase().includes(t)):O)}),document.addEventListener(`barcodeScanned`,e=>{let t=document.querySelector(`[data-view="pos"]`);if(!t||t.classList.contains(`hidden`))return;let n=e.detail,r=O.find(e=>e.sku===n||e.id===n);r?(window.posAddToCart(r.id),h(`✅ ${r.nombre} agregado`,`success`),document.activeElement===A&&(A.value=``,Ln(O),A.blur())):h(`Código ${n} no encontrado`,`warning`)}),document.getElementById(`pos-scan-btn`)?.addEventListener(`click`,()=>{vt({title:`Escanear`,onScan:e=>{A.value=e,A.dispatchEvent(new Event(`input`));let t=O.find(t=>t.sku===e||t.id===e);t?(window.posAddToCart(t.id),h(`✅ ${t.nombre} agregado`,`success`),setTimeout(()=>{A.value===e&&(A.value=``,A.dispatchEvent(new Event(`input`)))},1500)):h(`Código ${e} no encontrado en inventario`,`warning`)}})}),document.getElementById(`pos-reventa-btn`)?.addEventListener(`click`,()=>{window.__posReventaMode=!0,o(`inventory`),setTimeout(()=>{window.inventoryView&&window.inventoryView.openNuevo&&window.inventoryView.openNuevo(!0)},150)}),document.getElementById(`pos-select-client-btn`)?.addEventListener(`click`,()=>{sn(e=>{vn.value=e.nombre,_n.value=e.documento})}),document.querySelectorAll(`input[name="pos-billing-type"]`).forEach(e=>{e.addEventListener(`change`,e=>{let t=e.target.value===`digital`,n=document.getElementById(`pos-evidencia-container`);t?(n.classList.add(`hidden`),En.classList.remove(`hidden`),Dn.classList.remove(`hidden`),Vn(),setTimeout(Hn,50)):(n.classList.remove(`hidden`),En.classList.add(`hidden`),Dn.classList.add(`hidden`),On.classList.add(`hidden`))})}),jn.addEventListener(`click`,()=>An.classList.add(`hidden`)),Mn.addEventListener(`click`,()=>F.clearRect(0,0,P.width,P.height)),Nn.addEventListener(`click`,()=>{if(I){let e=I.getContext(`2d`);e.clearRect(0,0,I.width,I.height),e.drawImage(P,0,0,I.width,I.height)}An.classList.add(`hidden`)});let e=(e,t)=>{I=e,document.getElementById(`pos-sig-modal-title`).textContent=t,An.classList.remove(`hidden`),P.width=P.offsetWidth,P.height=P.offsetHeight,F.lineWidth=3,F.lineCap=`round`,F.strokeStyle=`#000`,F.clearRect(0,0,P.width,P.height)};M.parentElement.addEventListener(`click`,()=>e(M,`Firma del Comprador`)),N.parentElement.addEventListener(`click`,()=>e(N,`Firma del Vendedor`)),window.posAddToCart=e=>{let t=O.find(t=>t.id===e);if(!t)return;let n=k.find(t=>t.id===e);if(n){if(n.qty>=t.stockActual)return h(`Sin stock`,`warning`);n.qty++}else k.push({...t,qty:1,precioManual:0});L(),setTimeout(()=>{let e=pn.querySelectorAll(`input[oninput*="posUpdatePrice"]`);e.length>0&&e[e.length-1].focus()},100)},window.posRemoveItem=e=>{k=k.filter(t=>t.id!==e),L()},window.posUpdateQty=(e,t)=>{let n=k.find(t=>t.id===e),r=O.find(t=>t.id===e);n&&(n.qty+=t,n.qty<=0?window.posRemoveItem(e):n.qty>r.stockActual&&(n.qty=r.stockActual),L())},window.posUpdatePrice=(e,t)=>{let n=k.find(t=>t.id===e),r=Number(t.value.replace(/\D/g,``));n&&(n.precioManual=r,t.value=new Intl.NumberFormat(`es-CO`).format(r),zn())},hn.addEventListener(`input`,L);let t=e=>{if(k.length!==0){if(!vn.value)return h(`Nombre cliente ok?`,`warning`);dn=e,Wn()}};document.getElementById(`pos-pay-btn-venta`)?.addEventListener(`click`,()=>t(`venta`)),document.getElementById(`pos-pay-btn-credito`)?.addEventListener(`click`,()=>t(`credito`)),document.getElementById(`pos-pay-btn-separe`)?.addEventListener(`click`,()=>t(`separe`)),bn.addEventListener(`click`,Gn),xn.addEventListener(`click`,Gn),j.addEventListener(`click`,Kn)}function L(){if(k.length===0){pn.innerHTML=`<div class="flex flex-col items-center justify-center h-full text-on-surface-variant/50"><span class="material-symbols-outlined text-5xl mb-2" style="font-variation-settings:'FILL' 1">shopping_cart</span><p class="text-sm font-medium">El carrito está vacío</p></div>`,Bn(0);return}let e=0;pn.innerHTML=k.map(t=>{let n=t.precioManual||0;return e+=n*t.qty,`
    <div class="bg-white border-2 ${n===0?`border-orange-400 animate-pulse`:`border-slate-100`} p-3 rounded-2xl flex gap-3 shadow-sm transition-all">
      ...
      <div class="flex flex-col justify-between items-end">
        <button onclick="window.posRemoveItem('${t.id}')" class="text-slate-300 hover:text-red-500 transition-colors"><span class="material-symbols-outlined text-[18px]">delete</span></button>
    ...
    </div>`}).join(``),Bn(e)}function zn(){let e=0;k.forEach(t=>e+=(t.precioManual||t.precioVenta||0)*t.qty),Bn(e)}function Bn(e){let t=parseFloat(hn.value)||0;mn.textContent=`$${new Intl.NumberFormat(`es-CO`).format(e)}`,gn.textContent=`$${new Intl.NumberFormat(`es-CO`).format(Math.max(0,e-t))}`}function Vn(){let e=k.filter(e=>{let t=O.find(t=>t.id===e.id);return t&&(t.categoria===`Celulares`||t.nombre.toLowerCase().includes(`celular`)||t.nombre.toLowerCase().includes(`teléfono`))});e.length>0?(On.classList.remove(`hidden`),kn.innerHTML=e.map(e=>`
      <div class="bg-white p-2 rounded-lg border border-amber-200">
        <p class="text-[9px] font-black text-slate-500 uppercase mb-1">${e.nombre}</p>
        <div class="flex gap-1">
          <input type="text" placeholder="IMEI principal" data-id="${e.id}" class="pos-imei-input w-full bg-slate-50 border border-slate-200 rounded px-2 py-1 text-xs outline-none focus:border-primary">
        </div>
      </div>
    `).join(``)):(On.classList.add(`hidden`),kn.innerHTML=``)}function Hn(){[M,N].forEach(e=>{if(e&&e.offsetWidth>0){e.width=e.offsetWidth,e.height=e.offsetHeight;let t=e.getContext(`2d`);t.lineWidth=2,t.lineCap=`round`}})}function Un(e,t){let n=e.getContext(`2d`);t(n);let r=t=>{let n=e.getBoundingClientRect(),r=(t.touches?t.touches[0].clientX:t.clientX)-n.left,i=(t.touches?t.touches[0].clientY:t.clientY)-n.top;return{x:r*(e.width/n.width),y:i*(e.height/n.height)}},i=!1,a=e=>{i=!0,n.beginPath();let{x:t,y:a}=r(e);n.moveTo(t,a),e.preventDefault()},o=e=>{if(!i)return;let{x:t,y:a}=r(e);n.lineTo(t,a),n.stroke(),e.preventDefault()};e.addEventListener(`mousedown`,a),e.addEventListener(`mousemove`,o),e.addEventListener(`mouseup`,()=>i=!1),e.addEventListener(`touchstart`,a,{passive:!1}),e.addEventListener(`touchmove`,o,{passive:!1}),e.addEventListener(`touchend`,()=>i=!1),e.nextElementSibling.addEventListener(`click`,t=>{t.stopPropagation(),n.clearRect(0,0,e.width,e.height)})}function Wn(){yn.classList.remove(`hidden`),yn.classList.add(`flex`),[M,N].forEach(e=>{e.width=e.offsetWidth,e.height=e.offsetHeight;let t=e.getContext(`2d`);t.lineWidth=2,t.lineCap=`round`,t.clearRect(0,0,e.width,e.height)}),Cn.value=``,wn.value=``,Tn.value=``,document.querySelector(`input[name="pos-billing-type"][value="fisica"]`).checked=!0,document.getElementById(`pos-evidencia-container`).classList.remove(`hidden`),En.classList.add(`hidden`),Dn.classList.add(`hidden`),On.classList.add(`hidden`)}function Gn(){yn.classList.add(`hidden`),yn.classList.remove(`flex`)}async function Kn(){if(un)return;un=!0;let e=document.querySelector(`input[name="pos-billing-type"]:checked`).value;j.textContent=`Procesando...`,j.disabled=!0;try{let t=``,n=``,r=``;if(e===`digital`){let e=document.createElement(`canvas`);if(e.width=M.width,e.height=M.height,M.toDataURL()!==e.toDataURL()){let e=await re(M.toDataURL(`image/png`),`FirmaCliente_${Date.now()}.png`);t=typeof e==`string`?e:e?.url||``}if(N.toDataURL()!==e.toDataURL()){let e=await re(N.toDataURL(`image/png`),`FirmaVendedor_${Date.now()}.png`);n=typeof e==`string`?e:e?.url||``}}if(e===`fisica`)if(Sn.files[0]){j.textContent=`Subiendo evidencia...`;let e=Sn.files[0],t=await ie(await new Promise(t=>{let n=new FileReader;n.onload=e=>t(e.target.result),n.readAsDataURL(e)}),e.name,e.type);r=typeof t==`string`?t:t?.url||``}else{h(`Por favor sube la foto de la factura física`,`warning`),un=!1,j.textContent=`Confirmar y Facturar`,j.disabled=!1;return}let i=document.querySelectorAll(`.pos-imei-input`),a={};i.forEach(e=>{let t=e.dataset.id;e.value.trim()&&(a[t]||(a[t]=[]),a[t].push(e.value.trim()))}),j.textContent=`Registrando venta...`;let o=Number(mn.textContent.replace(/\D/g,``)),s=Number(gn.textContent.replace(/\D/g,``)),c=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`),l={cedula:_n.value.trim(),cliente:vn.value.trim(),direccion:Cn.value.trim(),ciudad:wn.value.trim(),telefono:Tn.value.trim(),productoNombre:k.map(e=>`${e.nombre} (x${e.qty})`).join(`, `),productoId:k[0]?.id,items:k.map(e=>({id:e.id,qty:e.qty})),subtotal:o,descuento:Number(hn.value)||0,total:s,metodo:document.getElementById(`pos-metodo-pago`).value,vendedor:c.nombre||`Vendedor`,firmaComprador:t,firmaVendedor:n,evidencia:r,tipoFactura:e,imeis:JSON.stringify(a),emisor:{nombre:`CLAROCELL.COM`,propietario:`Yeison Rangel Rangel`,nit:`1193400777-2`,direccion:`Calle 12 No. 10 - 108, Maicao - La Guajira`,contacto:`3016807310`,correo:`yeison0021@hotmail.com`}},u=await we(l);u.success?(dn!==`venta`&&await Ee({cliente:l.cliente,telefono:l.cedula,idFactura:u.idFactura,total:s,detalle:l.productoNombre,tipo:dn===`separe`?`Plan Separe`:`Crédito`}),h(`Venta Exitosa`,`success`),e===`digital`&&qn({...l,idFactura:u.idFactura},M.toDataURL(),N.toDataURL()),k=[],L(),_n.value=``,vn.value=``,Gn(),In().then(()=>Ln(O))):h(`Error al guardar`,`error`)}catch(e){h(e.message,`error`)}finally{un=!1,j.textContent=`Confirmar y Facturar`,j.disabled=!1}}function qn(e,t,n){let r=window.open(``,`_blank`,`width=300,height=600`),i=new Date,a=`${i.getDate()}/${i.getMonth()+1}/${i.getFullYear()} ${i.getHours()}:${i.getMinutes()}`,o=JSON.parse(e.imeis||`{}`),s=Object.values(o).flat().join(`, `),c=k.map(e=>`
    <tr>
  ...
  `),r.document.close(),setTimeout(()=>{r.print(),r.close()},500)}var R=[],Jn=!1,Yn=!1,Xn,Zn,Qn,$n,er,tr,nr,rr,z,B,ir,V,ar,or,H,U,sr,cr;function lr(){return async()=>{ur(),Jn||=(await dr(),pr(),!0),fr(R)}}function ur(){Xn=document.getElementById(`imei-table-body`),Zn=document.getElementById(`imei-search`),Qn=document.getElementById(`imei-filter-status`),$n=document.getElementById(`imei-new-btn`),er=document.getElementById(`imei-modal`),tr=document.getElementById(`imei-modal-close`),nr=document.getElementById(`imei-modal-backdrop`),rr=document.getElementById(`imei-form`),z=document.getElementById(`imei-save-btn`),B=document.getElementById(`imei-1`),ir=document.getElementById(`imei-2`),V=document.getElementById(`imei-nombre`),ar=document.getElementById(`imei-marca`),or=document.getElementById(`imei-proveedor`),H=document.getElementById(`imei-costo`),U=document.getElementById(`imei-venta`),sr=document.getElementById(`imei-estado`),cr=document.getElementById(`imei-original`)}async function dr(){try{Xn.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando equipos...</td></tr>`,R=await ye();let e=await he();V.innerHTML=`<option value="">Seleccione un equipo...</option>`+e.map(e=>`<option value="${e.nombre}" data-marca="${e.marca}" data-costo="${e.costo}" data-precio="${e.precioVenta}" data-prov="${e.proveedor||``}" data-id="${e.id}">${e.nombre}${e.marca?` (`+e.marca+`)`:``}</option>`).join(``)}catch(e){h(`Error cargando equipos: `+e.message,`error`),R=[]}}function fr(e){if(e.length===0){Xn.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron equipos</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;Xn.innerHTML=e.map(e=>{e.estado;let n=e.estado===`Disponible`?`bg-green-100 text-green-800`:e.estado===`Vendido`?`bg-red-100 text-red-800`:`bg-yellow-100 text-yellow-800`;return`
    ...
    `}).join(``)}function pr(){let e=()=>{let e=Zn.value.toLowerCase().trim(),t=Qn.value;fr(R.filter(n=>{let r=(n.imei1||``).toLowerCase().includes(e)||(n.imei2||``).toLowerCase().includes(e)||(n.nombre||``).toLowerCase().includes(e),i=t?n.estado===t:!0;return r&&i}))},t=e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))};H.addEventListener(`input`,t),U.addEventListener(`input`,t),V.addEventListener(`change`,e=>{let t=e.target.selectedOptions[0];if(t&&t.value){ar.value=t.dataset.marca||``,or.value=t.dataset.prov||``;let e=parseInt(String(t.dataset.costo||`0`).replace(/\D/g,``))||0,n=parseInt(String(t.dataset.precio||`0`).replace(/\D/g,``))||0;H.value=new Intl.NumberFormat(`es-CO`).format(e),U.value=new Intl.NumberFormat(`es-CO`).format(n)}else ar.value=``,or.value=``,H.value=``,U.value=``}),Zn.addEventListener(`input`,e),Qn.addEventListener(`change`,e),document.getElementById(`imei-scan-btn`)?.addEventListener(`click`,()=>{vt({title:`Escanear IMEI`,onScan:t=>{Zn.value=t,e(),h(`IMEI: ${t}`,`info`)}})}),document.getElementById(`imei-scan-1`)?.addEventListener(`click`,()=>{vt({title:`Escanear IMEI 1`,onScan:e=>{B.value=e,h(`IMEI 1: ${e}`,`success`)}})}),document.getElementById(`imei-scan-2`)?.addEventListener(`click`,()=>{vt({title:`Escanear IMEI 2`,onScan:e=>{ir.value=e,h(`IMEI 2: ${e}`,`success`)}})}),$n.addEventListener(`click`,()=>mr(null)),tr.addEventListener(`click`,hr),nr.addEventListener(`click`,hr),z.addEventListener(`click`,gr),window.imeiEdit=e=>{let t=R.find(t=>t.imei1==e);t&&mr(t)},window.imeiDelete=async t=>{if(confirm(`¿Eliminar el equipo con IMEI ${t}?`))try{h(`Eliminando...`,`info`);let n=await Se(t);n&&n.success?(h(`Equipo eliminado`,`success`),await dr(),e()):h(n.mensaje||`Error al eliminar`,`error`)}catch(e){h(`Error: `+e.message,`error`)}}}function mr(e){rr.reset(),e?(cr.value=e?e.imei1:``,B.value=e?e.imei1:``,ir.value=e?e.imei2:``,V.value=e?e.nombre:``,ar.value=e?e.marca:``,or.value=e?e.proveedor:``,H.value=e&&e.costo?new Intl.NumberFormat(`es-CO`).format(parseInt(String(e.costo).replace(/\D/g,``))||0):``,U.value=e&&e.venta?new Intl.NumberFormat(`es-CO`).format(parseInt(String(e.venta).replace(/\D/g,``))||0):``,sr.value=e?e.estado:`Disponible`,document.getElementById(`imei-modal-title`).textContent=`Editar Equipo`):(cr.value=``,sr.value=`Disponible`,document.getElementById(`imei-modal-title`).textContent=`Registrar Equipo`),er.classList.remove(`hidden`),er.classList.add(`flex`)}function hr(){er.classList.add(`hidden`),er.classList.remove(`flex`)}async function gr(){if(!B.value.trim()){h(`El IMEI Principal es obligatorio`,`warning`),B.focus();return}if(!V.value){h(`Debe seleccionar un equipo del inventario`,`warning`),V.focus();return}if(!Yn){Yn=!0,z.textContent=`Guardando...`,z.disabled=!0;try{let e=cr.value,t={imei1:B.value.trim(),imei2:ir.value.trim(),nombre:V.value.trim(),marca:ar.value.trim(),proveedor:or.value.trim(),costo:parseInt(H.value.replace(/\D/g,``))||0,venta:parseInt(U.value.replace(/\D/g,``))||0,estado:sr.value},n;n=e?await xe(e,t):await be(t),n&&n.success?(h(`Equipo guardado`,`success`),hr(),await dr(),fr(R)):h(n?.mensaje||`Error al guardar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{Yn=!1,z.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,z.disabled=!1}}}var W=[],_r=!1,vr=!1,yr,br,xr,Sr,Cr,wr,Tr,G,Er,Dr,Or,kr,Ar,jr,Mr;function Nr(){return async()=>{Pr(),_r||=(await Fr(),Lr(),!0),Ir(W)}}function Pr(){yr=document.getElementById(`cli-table-body`),br=document.getElementById(`cli-search`),xr=document.getElementById(`cli-new-btn`),Sr=document.getElementById(`cli-modal`),Cr=document.getElementById(`cli-modal-close`),wr=document.getElementById(`cli-modal-backdrop`),Tr=document.getElementById(`cli-form`),G=document.getElementById(`cli-save-btn`),Er=document.getElementById(`cli-doc`),Dr=document.getElementById(`cli-nombre`),Or=document.getElementById(`cli-tel`),kr=document.getElementById(`cli-email`),Ar=document.getElementById(`cli-dir`),jr=document.getElementById(`cli-tipo`),Mr=document.getElementById(`cli-original`)}async function Fr(){try{yr.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando clientes...</td></tr>`,W=await de()}catch(e){h(`Error cargando clientes: `+e.message,`error`),W=[]}}function Ir(e){if(e.length===0){yr.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron clientes</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;yr.innerHTML=e.map(e=>{let n=e.tipo===`VIP`?`bg-amber-100 text-amber-800`:e.tipo===`Empresa`?`bg-blue-100 text-blue-800`:e.tipo===`Mayorista`?`bg-purple-100 text-purple-800`:`bg-surface-container text-on-surface-variant`;return`
      <tr class="hover:bg-surface-container-low transition-colors">
    ...
    `}).join(``)}function Lr(){let e=()=>{let e=br.value.toLowerCase().trim();Ir(W.filter(t=>(t.cedula||``).toLowerCase().includes(e)||(t.nombre||``).toLowerCase().includes(e)||(t.telefono||``).toLowerCase().includes(e)))};br.addEventListener(`input`,e),xr.addEventListener(`click`,()=>Rr(null)),Cr.addEventListener(`click`,zr),wr.addEventListener(`click`,zr),G.addEventListener(`click`,Br),window.cliEdit=e=>{let t=W.find(t=>t.cedula==e);t&&Rr(t)},window.cliDelete=async t=>{if(confirm(`¿Eliminar al cliente con documento ${t}?`))try{h(`Eliminando...`,`info`);let n=await me(t);n&&n.success?(h(`Cliente eliminado`,`success`),await Fr(),e()):h(n.mensaje||`Error al eliminar`,`error`)}catch(e){h(`Error: `+e.message,`error`)}}}function Rr(e){Tr.reset(),e?(Mr.value=e.cedula,Er.value=e.cedula,Dr.value=e.nombre||``,Or.value=e.telefono||``,kr.value=e.email||``,Ar.value=e.direccion||``,jr.value=e.tipo||`Normal`,document.getElementById(`cli-modal-title`).textContent=`Editar Cliente`):(Mr.value=``,jr.value=`Normal`,document.getElementById(`cli-modal-title`).textContent=`Nuevo Cliente`),Sr.classList.remove(`hidden`),Sr.classList.add(`flex`)}function zr(){Sr.classList.add(`hidden`),Sr.classList.remove(`flex`)}async function Br(){if(!Tr.checkValidity()){Tr.reportValidity();return}if(!vr){vr=!0,G.textContent=`Guardando...`,G.disabled=!0;try{let e=Mr.value,t={cedula:Er.value.trim(),nombre:Dr.value.trim(),telefono:Or.value.trim(),email:kr.value.trim(),direccion:Ar.value.trim(),tipo:jr.value},n;n=e?await pe(e,t):await fe(t),n&&n.success?(h(`Cliente guardado`,`success`),zr(),await Fr(),Ir(W)):h(n?.mensaje||`Error al guardar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{vr=!1,G.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,G.disabled=!1}}}var K=[],Vr=!1,Hr=!1;function Ur(){return async()=>{Vr||=(await Wr(),Xr(),!0),Yr(K)}}async function Wr(){let e=document.getElementById(`cred-table-body`);e&&(e.innerHTML=`<tr><td colspan="9" class="p-6 text-center text-on-surface-variant">Cargando créditos...</td></tr>`);try{K=await Te()}catch(e){h(`Error cargando créditos: `+e.message,`error`),K=[]}}var q=e=>`$`+new Intl.NumberFormat(`es-CO`).format(Math.round(e||0));function Gr(e,t){if(!e)return 0;let n=e=>{if(!e)return null;let t=String(e).split(`/`);return t.length===3?new Date(t[2],t[1]-1,t[0]):new Date(e)},r=n(e),i=t?n(t):new Date;return!r||isNaN(r)?0:Math.max(0,Math.floor((i-r)/864e5))}function Kr(e){return e?e.split(`;`).filter(Boolean).map(e=>{let t=e.split(`|`);return{fecha:t[0]||``,monto:parseFloat(t[1])||0,nota:t[2]||``}}):[]}function qr(e){return e.map(e=>`${e.fecha}|${e.monto}|${e.nota}`).join(`;`)}function Jr(e){let t=0,n=0;e.forEach(e=>{e.estado!==`Cancelado`&&(t+=e.saldo||0),n+=e.abonado||0});let r=document.getElementById(`cred-stat-total`),i=document.getElementById(`cred-stat-recaudo`);r&&(r.textContent=q(t)),i&&(i.textContent=q(n))}function Yr(e){Jr(K);let t=document.getElementById(`cred-table-body`);if(t){if(e.length===0){t.innerHTML=`<tr><td colspan="9" class="p-6 text-center text-on-surface-variant">No se encontraron créditos</td></tr>`;return}t.innerHTML=e.map(e=>{let t=e.estado===`Cancelado`||e.estado===`Entregado`,n=Gr(e.fecha,t?e.fechaCancelacion:null),r=t?`bg-green-100 text-green-800`:e.estado===`En Mora`?`bg-red-100 text-red-800`:`bg-orange-100 text-orange-800`,i=e.tipo===`Plan Separe`?`bg-emerald-100 text-emerald-800 border-emerald-200`:`bg-blue-100 text-blue-800 border-blue-200`,a=t?`<span class="text-[10px] text-on-surface-variant">Pagó en ${n}d</span>`:`<span class="text-[10px] font-bold ${n>30?`text-red-500`:`text-orange-500`}">${n} días</span>`,o=String(e.telefono||``).replace(/\D/g,``),s=`https://wa.me/57${o}?text=${encodeURIComponent(`Hola ${e.cliente}, le recordamos que tiene un saldo pendiente de ${q(e.saldo)} con nosotros. Gracias.`)}`;return`
      <tr class="hover:bg-surface-container-low transition-colors ${t?`opacity-70`:``}">
        ...
        <td class="px-4 py-3 text-sm font-medium">${q(e.total)}</td>
        <td class="px-4 py-3 text-sm font-medium text-green-600">${q(e.abonado)}</td>
        <td class="px-4 py-3 text-sm font-black text-error">${q(e.saldo)}</td>
    ...
    `}).join(``)}}function Xr(){let e=document.getElementById(`cred-search`),t=document.getElementById(`cred-filter-status`),n=document.getElementById(`cred-filter-tipo`),r=()=>{let r=(e?.value||``).toLowerCase().trim(),i=t?.value||``,a=n?.value||``;Yr(K.filter(e=>{let t=(e.cliente||``).toLowerCase().includes(r)||(e.idFactura||``).toLowerCase().includes(r),n=i?e.estado===i:!0,o=a?(e.tipo||`Crédito`)===a:!0;return t&&n&&o}))};e?.addEventListener(`input`,r),t?.addEventListener(`change`,r),n?.addEventListener(`change`,r);let i=e=>{let t=e.target.value.replace(/\D/g,``);e.target.value=t?new Intl.NumberFormat(`es-CO`).format(parseInt(t)):``};document.getElementById(`cred-new-total`)?.addEventListener(`input`,i),document.getElementById(`cred-new-abono`)?.addEventListener(`input`,i),document.getElementById(`cred-monto-abono`)?.addEventListener(`input`,i);let a=document.getElementById(`cred-modal`),o=document.getElementById(`cred-modal-close`),s=document.getElementById(`cred-modal-backdrop`),c=document.getElementById(`cred-save-btn`),l=()=>{a?.classList.add(`hidden`),a?.classList.remove(`flex`)};o?.addEventListener(`click`,l),s?.addEventListener(`click`,l),window.credAddAbono=e=>{let t=K.find(t=>t.id==e);if(!t)return;document.getElementById(`cred-id`).value=t.id,document.getElementById(`cred-cliente-name`).textContent=t.cliente,document.getElementById(`cred-saldo-actual`).textContent=q(t.saldo);let n=Kr(t.historialAbonos),r=document.getElementById(`cred-historial`);r&&(r.innerHTML=n.length===0?`<p class="text-xs text-on-surface-variant">Sin abonos anteriores</p>`:n.map(e=>`
            <div class="flex justify-between items-center py-1 border-b border-surface-variant text-xs">
              ...
              <span class="font-bold text-green-600">${q(e.monto)}</span>
              <span class="text-on-surface-variant">${e.nota||``}</span>
            </div>`).join(``)),document.getElementById(`cred-monto-abono`).value=``,document.getElementById(`cred-nota-abono`).value=``,a?.classList.remove(`hidden`),a?.classList.add(`flex`),document.getElementById(`cred-monto-abono`)?.focus()},c?.addEventListener(`click`,async()=>{let e=document.getElementById(`cred-id`).value,t=parseInt((document.getElementById(`cred-monto-abono`).value||``).replace(/\D/g,``))||0,n=(document.getElementById(`cred-nota-abono`)?.value||``).trim();if(!t||t<=0){h(`Ingresa un monto válido`,`warning`);return}if(!Hr){Hr=!0,c.textContent=`Aplicando...`,c.disabled=!0;try{let r=K.find(t=>t.id==e),i=(r.abonado||0)+t,a=Math.max(0,(r.total||0)-i),o=a<=0,s=Kr(r.historialAbonos);s.push({fecha:new Date().toLocaleDateString(`es-CO`),monto:t,nota:n});let c=r.tipo===`Plan Separe`,u=c?`Separado`:`Activo`,d=c?`Entregado`:`Cancelado`,f=await De(e,{...r,abonado:i,saldo:a,estado:o||r.estado===`Cancelado`||r.estado===`Entregado`?d:u,fechaCancelacion:o?new Date().toLocaleDateString(`es-CO`):r.fechaCancelacion||``,historialAbonos:qr(s)});f?.success?(h(o?`✅ ¡Crédito cancelado!`:`Abono registrado`,`success`),l(),Vr=!1,await Wr(),Yr(K)):h(f?.mensaje||`Error al guardar`,`error`)}catch(e){h(`Error: `+e.message,`error`)}finally{Hr=!1,c.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Aplicar Abono`,c.disabled=!1}}});let u=document.getElementById(`cred-new-modal`),d=()=>{u?.classList.add(`hidden`),u?.classList.remove(`flex`)};document.getElementById(`cred-new-btn`)?.addEventListener(`click`,()=>{document.getElementById(`cred-new-form`)?.reset(),document.getElementById(`cred-new-cliente`).value=``,document.getElementById(`cred-new-cliente-doc`).value=``,u?.classList.remove(`hidden`),u?.classList.add(`flex`)}),document.getElementById(`cred-new-close`)?.addEventListener(`click`,d),document.getElementById(`cred-new-backdrop`)?.addEventListener(`click`,d),document.getElementById(`cred-select-client-btn`)?.addEventListener(`click`,()=>{sn(e=>{document.getElementById(`cred-new-cliente`).value=e.nombre,document.getElementById(`cred-new-cliente-doc`).value=e.cedula||e.documento||e.telefono||``})}),document.getElementById(`cred-save-new-btn`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`cred-new-cliente`).value.trim(),t=document.getElementById(`cred-new-cliente-doc`).value.trim(),n=parseInt((document.getElementById(`cred-new-total`).value||``).replace(/\D/g,``))||0,r=parseInt((document.getElementById(`cred-new-abono`)?.value||``).replace(/\D/g,``))||0,i=document.getElementById(`cred-new-detalle`).value.trim();if(!e||!n){h(`Cliente y monto son requeridos`,`warning`);return}let a=document.getElementById(`cred-save-new-btn`);a.disabled=!0,a.textContent=`Guardando...`;try{let a=await Ee({cliente:e,telefono:t,total:n,detalle:i,historialAbonos:r>0?qr([{fecha:new Date().toLocaleDateString(`es-CO`),monto:r,nota:`Abono inicial`}]):``});a?.success?(h(`Crédito creado`,`success`),d(),Vr=!1,await Wr(),Yr(K)):h(a?.mensaje||`Error al crear crédito`,`error`)}catch(e){h(`Error: `+e.message,`error`)}finally{a.disabled=!1,a.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}})}var Zr=[],Qr=[];function $r(){return async()=>{document.getElementById(`sales-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();Qr=Zr.filter(e=>(e.id_factura||``).toLowerCase().includes(t)||(e.cliente||``).toLowerCase().includes(t)||(e.cedula||``).toLowerCase().includes(t)),ti()});let e=()=>{let e=document.getElementById(`sale-detail-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)};document.getElementById(`sale-detail-close`)?.addEventListener(`click`,e),document.getElementById(`sale-detail-backdrop`)?.addEventListener(`click`,e),await ei()}}async function ei(){let e=document.getElementById(`sales-history-list`);if(e)try{e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-on-surface-variant italic text-sm">Cargando todas las ventas...</td></tr>`,Zr=await Ce(),Qr=[...Zr],ti()}catch(t){e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-error italic text-sm">Error: ${t.message}</td></tr>`}}function ti(){let e=document.getElementById(`sales-history-list`);if(e){if(Qr.length===0){e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-on-surface-variant italic text-sm">No se encontraron ventas</td></tr>`;return}e.innerHTML=Qr.map((e,t)=>{let n=new Date(e.fecha).toLocaleDateString(`es-CO`,{day:`2-digit`,month:`short`}),r=new Intl.NumberFormat(`es-CO`).format(e.total||0);return`
    ...
    `}).join(``)}}window.viewSaleDetail=e=>{let t=Zr.find(t=>t.id_factura===e);if(!t)return;let n=document.getElementById(`sale-detail-modal`),r=document.getElementById(`sale-detail-content`),i=e=>new Intl.NumberFormat(`es-CO`).format(e||0);r.innerHTML=`
    <div class="space-y-6">
  ...
  `,n.classList.remove(`hidden`),n.classList.add(`flex`)};var ni=[],ri=!1;function ii(){return async()=>{ai(),ri||=(await oi(),!0)}}function ai(){let e=document.getElementById(`task-new-btn`),t=document.getElementById(`task-modal-close`),n=document.getElementById(`task-modal-backdrop`),r=document.getElementById(`task-form`);e?.replaceWith(e.cloneNode(!0)),t?.replaceWith(t.cloneNode(!0)),n?.replaceWith(n.cloneNode(!0)),r?.replaceWith(r.cloneNode(!0)),document.getElementById(`task-new-btn`)?.addEventListener(`click`,ui),document.getElementById(`task-modal-close`)?.addEventListener(`click`,di),document.getElementById(`task-modal-backdrop`)?.addEventListener(`click`,di),document.getElementById(`task-form`)?.addEventListener(`submit`,fi)}async function oi(){let e=document.getElementById(`task-list`);try{let e=await Ve(),t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`);ni=t.rol===`Técnico de reparación`||t.rol===`Vendedor`?e.filter(e=>e.responsable===t.nombre):e,si()}catch(t){e.innerHTML=`<li class="p-8 text-center text-error">Error: ${t.message}</li>`}}function si(){let e=document.getElementById(`task-list`);if(!ni||ni.length===0){e.innerHTML=`<li class="p-12 text-center text-on-surface-variant italic text-sm">No hay tareas. ¡Buen trabajo!</li>`;return}e.innerHTML=ni.map(e=>{let t=e.estado===`Completada`;return`
      <li class="flex items-center gap-4 p-4 hover:bg-surface-container-low transition-colors group">
        ...
        <div class="text-right">
          <span class="text-[9px] font-black px-2 py-0.5 rounded-full ${ci(e.prioridad)}">${e.prioridad}</span>
          <p class="text-[10px] text-on-surface-variant mt-1">${li(e.fecha_vencimiento)}</p>
        </div>
    ...
    `}).join(``)}function ci(e){return e===`Alta`?`bg-red-100 text-red-700`:e===`Media`?`bg-blue-100 text-blue-700`:`bg-slate-100 text-slate-700`}function li(e){return e?new Date(e+`T12:00:00`).toLocaleDateString(`es-CO`,{day:`numeric`,month:`short`}):`—`}function ui(){let e=document.getElementById(`task-modal`);document.getElementById(`task-form`).reset(),document.getElementById(`task-input-date`).value=new Date().toISOString().slice(0,10),e.classList.remove(`hidden`),e.classList.add(`flex`)}function di(){document.getElementById(`task-modal`).classList.add(`hidden`),document.getElementById(`task-modal`).classList.remove(`flex`)}async function fi(e){e.preventDefault();let t=document.getElementById(`task-save-btn`);t.disabled=!0,t.innerHTML=`Guardando...`;let n={tarea:document.getElementById(`task-input-title`).value.trim(),fecha_inicio:new Date().toISOString().slice(0,10),fecha_vencimiento:document.getElementById(`task-input-date`).value,prioridad:document.getElementById(`task-input-priority`).value,responsable:JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).nombre||`Admin`,notas:document.getElementById(`task-input-notes`).value.trim(),estado:`Pendiente`,color:pi(document.getElementById(`task-input-priority`).value)};try{await He(n)&&(h(`Tarea creada`,`success`),di(),await oi())}catch{h(`Error al guardar`,`error`)}finally{t.disabled=!1,t.innerHTML=`<span class="material-symbols-outlined text-[20px]">save</span> Guardar Tarea`}}function pi(e){return e===`Alta`?`#ef4444`:e===`Media`?`#3b82f6`:`#64748b`}window.toggleTaskStatus=async(e,t)=>{let n=t===`Completada`?`Pendiente`:`Completada`;try{await Ue(e,n),await oi()}catch(e){console.error(e)}},window.deleteTask=async e=>{if(confirm(`¿Eliminar tarea?`))try{await We(e),await oi()}catch(e){console.error(e)}};var J=[],mi=!1;function hi(){return async()=>{mi||=(await gi(),!0),_i()}}async function gi(){let e=document.getElementById(`calendar-container`);e&&(e.innerHTML=`<div class="p-10 text-center text-on-surface-variant italic animate-pulse">Recopilando actividad global...</div>`);try{let e=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`),t=e.rol===`Administrador`,n=[Ce().catch(()=>[]),Ve().catch(()=>[]),Oe().catch(()=>[])];t&&(n.push(Ie().catch(()=>[])),n.push(Re().catch(()=>[])));let[r,i,a,o=[],s=[]]=await Promise.all(n);J=[],r.forEach(e=>{e.fecha&&J.push({fecha:new Date(e.fecha),tipo:`Venta`,icono:`point_of_sale`,color:`text-green-600`,bg:`bg-green-100`,border:`border-green-200`,titulo:`Factura: ${e.id_factura||`N/A`}`,subtitulo:`Cliente: ${e.cliente||`Consumidor Final`}`,monto:e.total,signo:`+`,usuario:e.vendedor||`Admin`})}),i.forEach(e=>{e.fecha_vencimiento&&J.push({fecha:new Date(e.fecha_vencimiento),tipo:`Tarea`,icono:`task_alt`,color:`text-indigo-600`,bg:`bg-indigo-100`,border:`border-indigo-200`,titulo:e.tarea,subtitulo:`Resp: ${e.responsable||`Sin asignar`}`,monto:null,usuario:e.responsable||`Admin`})}),a.forEach(e=>{e.fecha&&J.push({fecha:new Date(e.fecha),tipo:`Reventa`,icono:`storefront`,color:`text-purple-600`,bg:`bg-purple-100`,border:`border-purple-200`,titulo:`Reventa: ${e.producto}`,subtitulo:`Proveedor: ${e.proveedor||`N/A`}`,monto:e.utilidad,signo:`+`,usuario:e.vendedor||`Admin`})}),o.forEach(e=>{e.fecha&&J.push({fecha:new Date(e.fecha),tipo:`Egreso`,icono:`payments`,color:`text-red-600`,bg:`bg-red-100`,border:`border-red-200`,titulo:`Gasto: ${e.concepto}`,subtitulo:`Cat: ${e.categoria}`,monto:e.monto,signo:`-`})}),s.forEach(e=>{e.fecha&&J.push({fecha:new Date(e.fecha),tipo:`Nómina`,icono:`request_quote`,color:`text-orange-600`,bg:`bg-orange-100`,border:`border-orange-200`,titulo:`Pago: ${e.empleado}`,subtitulo:`Período: ${e.periodo}`,monto:e.total_pagar,signo:`-`})}),J.sort((e,t)=>t.fecha-e.fecha),t||(J=J.filter(t=>t.usuario===e.nombre))}catch(t){console.error(t),e&&(e.innerHTML=`<div class="p-10 text-center text-error font-bold">Error cargando actividad: ${t.message}</div>`)}}function _i(){let e=document.getElementById(`calendar-container`);if(!e)return;if(J.length===0){e.innerHTML=`
    ...
    `}),e.innerHTML=r}var Y=[],vi=!1,yi=!1,bi=null;function xi(){return async()=>{vi||=(await Si(),wi(),!0),Ci(Y)}}async function Si(){let e=document.getElementById(`user-table-body`);try{e&&(e.innerHTML=`<tr><td colspan="5" class="p-8 text-center opacity-50">Cargando equipo...</td></tr>`),Y=await se()}catch{h(`Error al cargar usuarios`,`error`),Y=[]}}function Ci(e){let t=document.getElementById(`user-table-body`);if(t){if(e.length===0){t.innerHTML=`<tr><td colspan="5" class="p-10 text-center opacity-40 italic">No hay usuarios registrados</td></tr>`;return}t.innerHTML=e.map(e=>`
  ...
  `).join(``)}}function wi(){document.getElementById(`user-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();Ci(Y.filter(e=>e.nombre.toLowerCase().includes(t)||e.email.toLowerCase().includes(t)))}),document.getElementById(`user-new-btn`)?.addEventListener(`click`,()=>Ti()),document.getElementById(`user-modal-close`)?.addEventListener(`click`,Ei),document.getElementById(`user-modal-backdrop`)?.addEventListener(`click`,Ei),document.getElementById(`user-form`)?.addEventListener(`submit`,Di),window.userEdit=e=>{let t=Y.find(t=>t.email===e);t&&Ti(t)},window.userDelete=async e=>{if(e===JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).email)return h(`No puedes eliminarte a ti mismo`,`warning`);if(confirm(`¿Eliminar al usuario ${e}?`))try{await ue(e),h(`Usuario eliminado`,`success`),await Si(),Ci(Y)}catch(e){h(e.message,`error`)}}}function Ti(e=null){bi=e?e.email:null,document.getElementById(`user-form`).reset(),document.getElementById(`user-modal-title`).textContent=e?`Editar Usuario`:`Nuevo Usuario`,e&&(document.getElementById(`user-input-name`).value=e.nombre,document.getElementById(`user-input-email`).value=e.email,document.getElementById(`user-input-password`).value=e.password,document.getElementById(`user-input-rol`).value=e.rol,document.getElementById(`user-input-estado`).value=e.estado);let t=document.getElementById(`user-modal`);t.classList.remove(`hidden`),t.classList.add(`flex`)}function Ei(){let e=document.getElementById(`user-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)}async function Di(e){if(e.preventDefault(),yi)return;yi=!0;let t=document.getElementById(`user-save-btn`);t.disabled=!0,t.innerHTML=`Guardando...`;let n=e=>document.getElementById(e).value.trim(),r=n(`user-input-email`).toLowerCase(),i=[r,n(`user-input-password`),n(`user-input-name`),n(`user-input-rol`),n(`user-input-estado`)];try{bi?await le(bi,r,[i[1],i[2],i[3],i[4]]):await ce(i),h(bi?`Actualizado`:`Creado`,`success`),Ei(),await Si(),Ci(Y)}catch(e){h(e.message,`error`)}finally{yi=!1,t.disabled=!1,t.innerHTML=`Guardar Usuario`}}var X=[],Oi=!1,ki=!1,Z,Ai,ji,Mi,Ni,Pi,Fi,Ii,Li,Ri,zi,Bi,Vi,Hi,Ui;function Wi(){return async()=>{Gi(),Oi||=(await Ki(),Ji(),!0),qi(X)}}function Gi(){Z=document.getElementById(`ped-table-body`),Ai=document.getElementById(`ped-search`),ji=document.getElementById(`ped-filter-status`),Mi=document.getElementById(`ped-new-btn`),Ni=document.getElementById(`ped-modal`),Pi=document.getElementById(`ped-modal-close`),Fi=document.getElementById(`ped-modal-backdrop`),Ii=document.getElementById(`ped-form`),Li=document.getElementById(`ped-save-btn`),Ri=document.getElementById(`ped-id`),zi=document.getElementById(`ped-producto`),Bi=document.getElementById(`ped-categoria`),Vi=document.getElementById(`ped-proveedor`),Hi=document.getElementById(`ped-costo`),Ui=document.getElementById(`ped-precio`)}async function Ki(){try{Z&&(Z.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando reventas...</td></tr>`),X=await Oe()}catch(e){h(`Error cargando reventas: `+e.message,`error`),X=[]}}function qi(e){if(!Z)return;if(e.length===0){Z.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron reventas</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;Z.innerHTML=e.map(e=>{let n=Number(e.costo||0),r=Number(e.precio||0),i=Number(e.utilidad||r-n),a=e=>new Intl.NumberFormat(`es-CO`).format(e);return`
    ...
    `}).join(``)}function Ji(){let e=()=>{if(!Ai)return;let e=Ai.value.toLowerCase().trim(),t=ji?ji.value:``;qi(X.filter(n=>{let r=(n.producto||``).toLowerCase().includes(e)||(n.id||``).toLowerCase().includes(e),i=t?n.categoria===t:!0;return r&&i}))};Ai?.addEventListener(`input`,e),ji?.addEventListener(`change`,e);let t=e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))};Hi?.addEventListener(`input`,t),Ui?.addEventListener(`input`,t),Mi?.addEventListener(`click`,()=>Yi(null)),Pi?.addEventListener(`click`,Xi),Fi?.addEventListener(`click`,Xi),Li?.addEventListener(`click`,Zi),window.pedDelete=async e=>{if(confirm(`¿Eliminar la reventa ${e}?`))try{let t=await Ae(e);t&&t.success&&(h(`Eliminada`,`success`),await Ki(),qi(X))}catch(e){h(e.message,`error`)}}}function Yi(e){Ni&&(Ii?.reset(),e&&(Ri&&(Ri.value=e.id),zi&&(zi.value=e.producto||``),Hi&&(Hi.value=e.costo||0),Ui&&(Ui.value=e.precio||0)),Ni.classList.remove(`hidden`),Ni.classList.add(`flex`))}function Xi(){Ni?.classList.add(`hidden`),Ni?.classList.remove(`flex`)}async function Zi(){if(!ki){ki=!0;try{(await ke({producto:zi?.value.trim(),categoria:Bi?.value,proveedor:Vi?.value.trim(),costo:parseInt(Hi?.value.replace(/\D/g,``))||0,precio:parseInt(Ui?.value.replace(/\D/g,``))||0})).success&&(h(`Guardado`,`success`),Xi(),await Ki(),qi(X))}catch(e){h(e.message,`error`)}finally{ki=!1}}}var Q=[],Qi=!1,$i=!1,ea=null,$={};function ta(){return async()=>{Qi||=(await na(),aa(),!0),ra(Q)}}async function na(){let e=document.getElementById(`tech-grid`);try{e&&(e.innerHTML=`<p class="col-span-full text-center p-10 opacity-50 italic">Cargando servicios...</p>`),Q=await je()}catch{h(`Error al cargar datos`,`error`),Q=[]}}function ra(e){let t=document.getElementById(`tech-grid`);if(!t)return;if(e.length===0){t.innerHTML=`<p class="col-span-full text-center p-20 opacity-30 italic text-sm">No hay órdenes de servicio activas</p>`;return}let n=e=>new Intl.NumberFormat(`es-CO`).format(e||0);t.innerHTML=e.map(e=>{let t=(e.precio_final||0)-(e.abono||0);return`
      <div class="bg-surface-container-lowest border border-surface-variant rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group relative">
        <div class="flex justify-between items-start mb-3">
          ...
          <span class="px-2 py-0.5 rounded-full text-[10px] font-bold ${ia(e.estado)}">${e.estado}</span>
        </div>
    ...
    `}).join(``)}function ia(e){return{Ingresado:`bg-slate-100 text-slate-700`,"En Revisión":`bg-blue-100 text-blue-700`,"En Taller":`bg-blue-100 text-blue-700`,Reparado:`bg-green-100 text-green-700`,Entregado:`bg-emerald-600 text-white`,"Sin Arreglo":`bg-red-100 text-red-700`}[e]||`bg-slate-100 text-slate-600`}function aa(){document.getElementById(`tech-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();ra(Q.filter(e=>e.cliente.toLowerCase().includes(t)||e.id_orden.toLowerCase().includes(t)||e.equipo.toLowerCase().includes(t)))}),document.getElementById(`tech-new-btn`)?.addEventListener(`click`,()=>oa()),document.getElementById(`tech-modal-close`)?.addEventListener(`click`,sa),document.getElementById(`tech-modal-backdrop`)?.addEventListener(`click`,sa),document.getElementById(`tech-form`)?.addEventListener(`submit`,ca),[`recepcion`,`resultado`].forEach(e=>{let t=document.getElementById(`tech-preview-${e}-btn`),n=document.getElementById(`tech-img-${e}`),r=document.getElementById(`tech-preview-${e}-wrap`),i=document.getElementById(`tech-preview-${e}`),a=document.getElementById(`tech-remove-${e}`);t?.addEventListener(`click`,()=>n?.click()),n?.addEventListener(`change`,n=>{let a=n.target.files[0];if(!a)return;let o=new FileReader;o.onload=n=>{$[e]={file:a,base64:n.target.result.split(`,`)[1],mime:a.type},i.src=n.target.result,t.classList.add(`hidden`),r.classList.remove(`hidden`)},o.readAsDataURL(a)}),a?.addEventListener(`click`,()=>{n.value=``,delete $[e],i.src=``,r.classList.add(`hidden`),t.classList.remove(`hidden`)})}),window.techEdit=e=>{let t=Q.find(t=>t.id_orden===e);t&&oa(t)},window.techPrint=e=>{let t=Q.find(t=>t.id_orden===e);if(!t)return;let n=e=>new Intl.NumberFormat(`es-CO`).format(e||0),r=(t.precio_final||0)-(t.abono||0),i=new Date().toLocaleDateString(`es-CO`,{year:`numeric`,month:`2-digit`,day:`2-digit`,hour:`2-digit`,minute:`2-digit`}),a=`
      <html>
      <head>
    ...
    `,o=window.open(``,``,`width=400,height=600`);o?(o.document.open(),o.document.write(a),o.document.close(),o.focus(),setTimeout(()=>{o.print(),o.close()},250)):h(`Por favor permite las ventanas emergentes para imprimir`,`warning`)},window.techDelete=async e=>{if(confirm(`¿Eliminar orden ${e}?`))try{(await Pe(e)).success&&(h(`Orden eliminada`,`success`),await na(),ra(Q))}catch(e){h(e.message,`error`)}}}function oa(e=null){if(ea=e?e.id_orden:null,document.getElementById(`tech-form`).reset(),document.getElementById(`tech-modal-title`).textContent=e?`Editar Orden`:`Ingreso a Servicio Técnico`,$={},[`recepcion`,`resultado`].forEach(e=>{document.getElementById(`tech-preview-${e}-btn`)?.classList.remove(`hidden`),document.getElementById(`tech-preview-${e}-wrap`)?.classList.add(`hidden`),document.getElementById(`tech-preview-${e}`).src=``}),e&&(document.getElementById(`tech-cliente`).value=e.cliente,document.getElementById(`tech-equipo`).value=e.equipo,document.getElementById(`tech-falla`).value=e.falla,document.getElementById(`tech-costo`).value=new Intl.NumberFormat(`es-CO`).format(e.precio_final||0),document.getElementById(`tech-estado`).value=e.estado,e.evidencias))try{let t=JSON.parse(e.evidencias);[`recepcion`,`resultado`].forEach(e=>{t[e]&&(document.getElementById(`tech-preview-${e}`).src=t[e],document.getElementById(`tech-preview-${e}-btn`).classList.add(`hidden`),document.getElementById(`tech-preview-${e}-wrap`).classList.remove(`hidden`),$[e]={url:t[e]})})}catch{}let t=document.getElementById(`tech-modal`);t.classList.remove(`hidden`),t.classList.add(`flex`)}function sa(){let e=document.getElementById(`tech-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)}async function ca(e){if(e.preventDefault(),$i)return;$i=!0;let t=document.getElementById(`tech-save-btn`);t.disabled=!0;let n=ea||`ST-${Date.now().toString().slice(-6)}`;t.innerHTML=`<span class="material-symbols-outlined animate-spin">progress_activity</span> Subiendo...`;let r={};for(let e of[`recepcion`,`resultado`])if($[e])if($[e].base64)try{let t=await ie($[e].base64,`${n}_${e}`,$[e].mime);t&&(r[e]=t)}catch(e){console.error(`Error upload evidencia`,e)}else $[e].url&&(r[e]=$[e].url);let i=[n,document.getElementById(`tech-cliente`).value.trim(),`310`,document.getElementById(`tech-equipo`).value.trim(),`S/N`,document.getElementById(`tech-falla`).value.trim(),`0000`,``,0,0,parseInt(document.getElementById(`tech-costo`).value.replace(/\D/g,``))||0,document.getElementById(`tech-estado`).value,JSON.stringify(r)];try{(ea?await Ne(ea,i):await Me(i)).success&&(h(ea?`Actualizado`:`Ingresado`,`success`),sa(),await na(),ra(Q))}catch(e){h(e.message,`error`)}finally{$i=!1,t.disabled=!1,t.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}}var la=[],ua=!1,da=!1,fa,pa,ma,ha,ga,_a,va,ya,ba,xa,Sa,Ca,wa;function Ta(){return async()=>{Ea(),ua||=(await Da(),ka(),!0),Oa(la)}}function Ea(){fa=document.getElementById(`exp-table-body`),pa=document.getElementById(`exp-search`),ma=document.getElementById(`exp-filter-cat`),ha=document.getElementById(`exp-new-btn`),ga=document.getElementById(`exp-modal`),_a=document.getElementById(`exp-modal-close`),va=document.getElementById(`exp-modal-backdrop`),ya=document.getElementById(`exp-form`),ba=document.getElementById(`exp-save-btn`),xa=document.getElementById(`exp-monto`),Sa=document.getElementById(`exp-categoria`),Ca=document.getElementById(`exp-concepto`),wa=document.getElementById(`exp-responsable`)}async function Da(){try{fa.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando egresos...</td></tr>`,la=await Ie()}catch(e){h(`Error cargando egresos: `+e.message,`error`),la=[]}}function Oa(e){if(e.length===0){fa.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron egresos</td></tr>`;return}fa.innerHTML=e.map(e=>`
      <tr class="hover:bg-surface-container-low transition-colors">
        <td class="px-4 py-3">
    ...
    `).join(``)}function ka(){let e=()=>{let e=pa.value.toLowerCase().trim(),t=ma.value;Oa(la.filter(n=>{let r=(n.concepto||``).toLowerCase().includes(e)||(n.responsable||``).toLowerCase().includes(e)||(n.id||``).toLowerCase().includes(e),i=t?n.categoria===t:!0;return r&&i}))};pa.addEventListener(`input`,e),ma.addEventListener(`change`,e),xa.addEventListener(`input`,e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))}),ha.addEventListener(`click`,()=>Aa()),_a.addEventListener(`click`,ja),va.addEventListener(`click`,ja),ba.addEventListener(`click`,Ma)}function Aa(){ya.reset();try{let e=localStorage.getItem(`adminpro_user`);if(e){let t=JSON.parse(e);wa.value=t.nombre||t.email}}catch{wa.value=`Sistema`}ga.classList.remove(`hidden`),ga.classList.add(`flex`),xa.focus()}function ja(){ga.classList.add(`hidden`),ga.classList.remove(`flex`)}async function Ma(){if(!ya.checkValidity()){ya.reportValidity();return}if(!da){da=!0,ba.textContent=`Registrando...`,ba.disabled=!0;try{let e=await Le({monto:parseInt(xa.value.replace(/\D/g,``))||0,categoria:Sa.value,concepto:Ca.value.trim(),responsable:wa.value});e&&e.success?(h(`Egreso registrado`,`success`),ja(),await Da(),Oa(la)):h(e?.mensaje||`Error al registrar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{da=!1,ba.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,ba.disabled=!1}}}var Na=[],Pa=[];function Fa(){return async()=>{await Ia(),Va()}}async function Ia(){try{let e=document.getElementById(`nom-list`);e&&(e.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-sm text-on-surface-variant">Cargando nóminas...</td></tr>`),[Na,Pa]=await Promise.all([Re(),se()]),La(Na),Ra(Na)}catch{h(`Error al cargar nóminas`,`error`)}}function La(e){let t=document.getElementById(`nom-list`);if(t){if(!e||e.length===0){t.innerHTML=`
      <tr>
    ...
    `}).join(``),document.querySelectorAll(`.nom-del-btn`).forEach(e=>{e.addEventListener(`click`,async e=>{let t=e.currentTarget.dataset.id;if(confirm(`¿Estás seguro de eliminar este registro de nómina?`))try{await Be(t),h(`Nómina eliminada`,`success`),Ia()}catch{h(`Error al eliminar`,`error`)}})})}}function Ra(e){let t=new Date,n=t.getMonth(),r=t.getFullYear(),i=0,a=0;e.forEach(e=>{let t=new Date(e.fecha);t.getMonth()===n&&t.getFullYear()===r&&e.estado!==`Anulado`&&(i+=parseFloat(e.total_pagar)||0),e.estado===`Pendiente`&&(a+=parseFloat(e.total_pagar)||0)});let o=document.getElementById(`nom-stat-mes`),s=document.getElementById(`nom-stat-pendiente`);o&&(o.textContent=`$`+i.toLocaleString(`es-CO`)),s&&(s.textContent=`$`+a.toLocaleString(`es-CO`))}function za(){let e=parseFloat(document.getElementById(`nom-base`).value)||0,t=parseFloat(document.getElementById(`nom-bonos`).value)||0,n=parseFloat(document.getElementById(`nom-deduc`).value)||0,r=e+t-n;document.getElementById(`nom-total-calc`).textContent=`$`+r.toLocaleString(`es-CO`)}var Ba=!1;function Va(){if(Ba)return;Ba=!0;let e=document.getElementById(`nom-modal`),t=document.getElementById(`nom-form`);document.getElementById(`nom-new-btn`)?.addEventListener(`click`,()=>{t.reset();let n=document.getElementById(`nom-empleado`);n&&(n.innerHTML=`<option value="">Seleccione empleado...</option>`+Pa.map(e=>`<option value="${e.nombre}">${e.nombre} (${e.rol})</option>`).join(``)),za(),e.classList.remove(`hidden`),e.classList.add(`flex`)}),document.getElementById(`nom-modal-close`)?.addEventListener(`click`,()=>{e.classList.add(`hidden`),e.classList.remove(`flex`)}),document.getElementById(`nom-modal-cancel`)?.addEventListener(`click`,()=>{e.classList.add(`hidden`),e.classList.remove(`flex`)}),document.getElementById(`nom-base`)?.addEventListener(`input`,za),document.getElementById(`nom-bonos`)?.addEventListener(`input`,za),document.getElementById(`nom-deduc`)?.addEventListener(`input`,za),document.getElementById(`nom-save-btn`)?.addEventListener(`click`,async()=>{if(!t.checkValidity()){t.reportValidity();return}let n=parseFloat(document.getElementById(`nom-base`).value)||0,r=parseFloat(document.getElementById(`nom-bonos`).value)||0,i=parseFloat(document.getElementById(`nom-deduc`).value)||0,a=n+r-i,o={empleado:document.getElementById(`nom-empleado`).value,periodo:document.getElementById(`nom-periodo`).value,salario_base:n,bonificaciones:r,deducciones:i,total_pagar:a,estado:document.getElementById(`nom-estado`).value,notas:document.getElementById(`nom-notas`).value},s=document.getElementById(`nom-save-btn`);s.disabled=!0,s.innerHTML=`<span class="material-symbols-outlined text-[18px] animate-spin">refresh</span> Guardando...`;try{await ze(o),h(`Nómina registrada exitosamente`,`success`),e.classList.add(`hidden`),e.classList.remove(`flex`),Ia()}catch(e){h(`Error al guardar: `+e.message,`error`)}finally{s.disabled=!1,s.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}})}function Ha(){return()=>{Ua(),Wa()}}function Ua(){let e=document.getElementById(`set-avatar`),t=document.getElementById(`set-name`),n=document.getElementById(`set-role`),r=document.getElementById(`set-email`);try{let i=localStorage.getItem(`adminproSession`),a=localStorage.getItem(`adminpro_user`),o=i?JSON.parse(i):a?JSON.parse(a):null;o?(t&&(t.textContent=o.nombre||`Usuario`),n&&(n.textContent=o.rol||`Administrador`),r&&(r.textContent=o.email||`No disponible`),e&&(e.textContent=(o.nombre?o.nombre.charAt(0):`U`).toUpperCase())):console.warn(`No se encontró sesión activa para cargar el perfil.`)}catch(e){console.error(`Error loading profile`,e)}let i=document.getElementById(`set-theme-toggle`);i&&(i.checked=document.documentElement.classList.contains(`dark`))}function Wa(){let e=document.getElementById(`set-logout-btn`);e?.replaceWith(e.cloneNode(!0)),document.getElementById(`set-logout-btn`)?.addEventListener(`click`,()=>{confirm(`¿Estás seguro de que deseas cerrar sesión?`)&&ee()});let t=document.getElementById(`set-theme-toggle`);t?.replaceWith(t.cloneNode(!0)),document.getElementById(`set-theme-toggle`)?.addEventListener(`change`,e=>{e.target.checked?(document.documentElement.classList.add(`dark`),localStorage.setItem(`adminpro_theme`,`dark`)):(document.documentElement.classList.remove(`dark`),localStorage.setItem(`adminpro_theme`,`light`))})}`serviceWorker`in navigator&&window.addEventListener(`load`,()=>{navigator.serviceWorker.register(`/adminpro/sw.js`,{scope:`/adminpro/`}).catch(e=>{console.log(`ServiceWorker registration failed: `,e)})});var Ga=``,Ka=``,qa=null;document.addEventListener(`keydown`,e=>{if(!(e.ctrlKey||e.altKey||e.metaKey))if(e.key===`Enter`){if(Ka.length>=3){let t=Ka;Ka=``,clearTimeout(qa),document.dispatchEvent(new CustomEvent(`barcodeScanned`,{detail:t})),(document.activeElement?document.activeElement.tagName:``)!==`TEXTAREA`&&e.preventDefault();return}Ka=``}else e.key.length===1&&(Ka+=e.key,clearTimeout(qa),qa=setTimeout(()=>{Ka=``},50))});function Ja(){try{let e=JSON.parse(localStorage.getItem(`adminproSession`)||`null`);return e&&Date.now()<e.expiresAt?e:null}catch{return null}}function Ya(e,t){d(t),localStorage.setItem(`adminproSession`,JSON.stringify({...e,token:t,expiresAt:Date.now()+480*60*1e3}))}function Xa(){d(null),localStorage.removeItem(`adminproSession`),localStorage.removeItem(`adminpro_user`)}var Za=[{label:`Inicio`,items:[{id:`dashboard`,label:`Dashboard`,icon:`dashboard`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]}]},{label:`Operaciones`,items:[{id:`pos`,label:`Ventas (POS)`,icon:`point_of_sale`,roles:[`Administrador`,`Vendedor`]},{id:`sales-history`,label:`Historial de Ventas`,icon:`history`,roles:[`Administrador`,`Vendedor`]},{id:`credits`,label:`Créditos`,icon:`credit_score`,roles:[`Administrador`,`Vendedor`]},{id:`expenses`,label:`Egresos`,icon:`payments`,roles:[`Administrador`]},{id:`nominas`,label:`Nóminas`,icon:`request_quote`,roles:[`Administrador`]}]},{label:`Inventario`,items:[{id:`inventory`,label:`Catálogo General`,icon:`inventory_2`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]},{id:`imei`,label:`Equipos IMEI`,icon:`phone_android`,roles:[`Administrador`,`Vendedor`]},{id:`reventas`,label:`Reventas`,icon:`storefront`,roles:[`Administrador`,`Vendedor`]}]},{label:`Servicios`,items:[{id:`technical`,label:`Servicio Técnico`,icon:`build`,roles:[`Administrador`,`Técnico de reparación`]}]},{label:`Organización`,items:[{id:`tasks`,label:`Lista de Tareas`,icon:`check_circle`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]},{id:`calendar`,label:`Actividad`,icon:`history_toggle_off`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]}]},{label:`Personas`,items:[{id:`clients`,label:`Clientes`,icon:`people`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]},{id:`users`,label:`Equipo / Usuarios`,icon:`manage_accounts`,roles:[`Administrador`]}]},{label:`Otros`,items:[{id:`settings`,label:`Ajustes`,icon:`settings`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]}]}];function Qa(e){let t=document.getElementById(`mobile-drawer`),n=document.getElementById(`mobile-drawer-backdrop`),r=document.getElementById(`mobile-drawer-content`);e?(t.classList.remove(`hidden`),setTimeout(()=>{n.classList.replace(`opacity-0`,`opacity-100`),r.classList.replace(`translate-y-full`,`translate-y-0`)},10)):(n.classList.replace(`opacity-100`,`opacity-0`),r.classList.replace(`translate-y-0`,`translate-y-full`),setTimeout(()=>t.classList.add(`hidden`),300))}function $a(e,t,n=!1){let r=document.getElementById(e);if(!r)return;let i=t||`Vendedor`,a=``;if(n){let e=[];e=i===`Técnico de reparación`?[{id:`dashboard`,label:`Home`,icon:`dashboard`},{id:`technical`,label:`Reparar`,icon:`build`},{id:`inventory`,label:`Stock`,icon:`inventory_2`},{id:`tasks`,label:`Tareas`,icon:`check_circle`}]:[{id:`dashboard`,label:`Home`,icon:`dashboard`},{id:`pos`,label:`Venta`,icon:`point_of_sale`},{id:`inventory`,label:`Stock`,icon:`inventory_2`},{id:`tasks`,label:`Tareas`,icon:`check_circle`}],a=e.map(e=>`
    ...
    `;let t=document.getElementById(`mobile-drawer-grid`);t&&(t.innerHTML=Za.flatMap(e=>e.items).filter(e=>!e.roles||e.roles.includes(i)).map(e=>`
      ...
      `).join(``))}else Za.forEach(e=>{let t=e.items.filter(e=>!e.roles||e.roles.includes(i));t.length!==0&&(a+=`<p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500 mt-6 mb-2 px-4 italic">${e.label}</p>`,t.forEach(e=>{a+=`
        ...
        `}))});r.innerHTML=a,n&&(document.getElementById(`mobile-more-btn`)?.addEventListener(`click`,()=>Qa(!0)),document.getElementById(`mobile-drawer-close`)?.addEventListener(`click`,()=>Qa(!1)),document.getElementById(`mobile-drawer-backdrop`)?.addEventListener(`click`,()=>Qa(!1)),document.querySelectorAll(`#mobile-drawer-grid [data-nav]`).forEach(e=>{e.addEventListener(`click`,()=>{Qa(!1),o(e.dataset.nav)})})),r.querySelectorAll(`[data-nav]`).forEach(e=>{e.addEventListener(`click`,()=>o(e.dataset.nav))})}function eo(e){document.querySelectorAll(`#desktop-nav [data-nav]`).forEach(t=>{let n=t.dataset.nav===e;t.classList.toggle(`bg-primary`,n),t.classList.toggle(`text-white`,n),t.classList.toggle(`shadow-lg`,n),t.classList.toggle(`text-slate-400`,!n)}),document.querySelectorAll(`#mobile-nav [data-nav]`).forEach(t=>{let n=t.dataset.nav===e;t.classList.toggle(`text-primary`,n),t.classList.toggle(`text-on-surface-variant`,!n)});let t=`AdminPro`;Za.forEach(n=>{let r=n.items.find(t=>t.id===e);r&&(t=r.label)});let n=document.getElementById(`header-title`);n&&(n.textContent=t)}function to(e,t){document.getElementById(`login-screen`).classList.add(`hidden`),document.getElementById(`app-shell`).classList.remove(`hidden`);let n=document.getElementById(`user-name`);n&&(n.textContent=e||`Usuario`),$a(`desktop-nav`,t,!1),$a(`mobile-nav`,t,!0);let s=t||`Vendedor`;a(e=>{let t=!1,n=!1;if(Za.forEach(r=>{let i=r.items.find(t=>t.id===e);i&&(n=!0,(!i.roles||i.roles.includes(s))&&(t=!0))}),n&&!t)return h(`No tienes permiso para acceder a este módulo`,`warning`),`dashboard`}),document.querySelectorAll(`[data-goto]`).forEach(e=>{let t=e.dataset.goto,n=!1;Za.forEach(e=>{let r=e.items.find(e=>e.id===t);r&&(!r.roles||r.roles.includes(s))&&(n=!0)}),n?e.classList.remove(`hidden`):e.classList.add(`hidden`)}),i(eo),r(`inventory`,Nt()),r(`dashboard`,It()),r(`pos`,Pn()),r(`imei`,lr()),r(`clients`,Nr()),r(`credits`,Ur()),r(`sales-history`,$r()),r(`tasks`,ii()),r(`calendar`,hi()),r(`users`,xi()),r(`reventas`,Wi()),r(`technical`,ta()),r(`expenses`,Ta()),r(`nominas`,Fa()),r(`settings`,Ha()),o(`dashboard`)}function no(e){let t=document.getElementById(`step-credentials`),n=document.getElementById(`step-pin`);t&&t.classList.toggle(`hidden`,e!==`credentials`),n&&n.classList.toggle(`hidden`,e!==`pin`)}function ro(){document.getElementById(`app-shell`).classList.add(`hidden`),document.getElementById(`login-screen`).classList.remove(`hidden`),no(`credentials`)}async function io(){await ee(),Xa(),ro()}window.addEventListener(`session-expired`,()=>{Xa(),ro()}),document.getElementById(`login-form`)?.addEventListener(`submit`,ao),document.getElementById(`pin-form`)?.addEventListener(`submit`,oo),document.getElementById(`back-to-login`)?.addEventListener(`click`,()=>no(`credentials`)),document.getElementById(`logout-btn`)?.addEventListener(`click`,io);async function ao(e){e.preventDefault();let t=document.getElementById(`login-btn`),n=document.getElementById(`login-email`).value.trim(),r=document.getElementById(`login-pwd`).value.trim();t.disabled=!0,t.textContent=`Verificando...`;try{let e=await ae(n,r);e.success&&e.step===`pin`?(Ga=n,document.getElementById(`pin-hint`).textContent=`Enviamos un PIN a ${n}`,no(`pin`),document.getElementById(`login-pin`).focus()):h(e.mensaje||`Credenciales incorrectas`,`error`)}catch{h(`Error de conexión`,`error`)}finally{t.disabled=!1,t.textContent=`Ingresar`}}async function oo(e){e.preventDefault();let t=document.getElementById(`pin-btn`),n=document.getElementById(`login-pin`).value.trim();t.disabled=!0,t.textContent=`Verificando...`;try{let e=await oe(Ga,n);e.success&&e.token?(Ya({email:e.email,nombre:e.nombre,rol:e.rol},e.token),to(e.nombre,e.rol)):h(e.mensaje||`PIN incorrecto`,`error`)}catch{h(`Error`,`error`)}finally{t.disabled=!1,t.textContent=`Verificar`}}var so=Ja();so&&so.token&&f()?(d(so.token),to(so.nombre,so.rol)):(Xa(),ro());