(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e={},t=null,n=null;function r(t,n){e[t]=n}function i(e){t=e}function a(e){n=e}async function o(r){if(n){let e=n(r);if(e===!1)return;typeof e==`string`&&(r=e)}document.querySelectorAll(`[data-view]`).forEach(e=>{e.classList.add(`hidden`)});let i=document.querySelector(`[data-view="${r}"]`);if(!i)return console.warn(`View not found:`,r);i.classList.remove(`hidden`),t&&t(r),e[r]&&await e[r]()}var s=`https://adminpro-adminpro.aws-us-west-2.turso.io/v2/pipeline`,c=`eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3NzcyOTIwOTMsImlkIjoiMDE5ZGNlZGItOTIwMS03NGVkLWIwZGYtZjg4MTQ3NjlhODcxIiwicmlkIjoiNWIwMWViNTctYTgxYS00OTI0LWIzMDUtZjk1Y2EwMjUzNmRkIn0.oruUZmv_ZLWlKA2ctQnghAD5PIiJSIeR4nzbZia-q-f1r12IHhLv1hDw9CsReABIceaVRHPS52JMZ4j3lcZ1Bw`,l=`https://script.google.com/macros/s/AKfycbxxs-PUyNqqALN2-azEHaViv5PM1r5oteanvr2Sfydic-bBQJrGWj00R0FO7UAlP4Ug/exec`,u=localStorage.getItem(`adminpro_gas_token`)||``,d=e=>{u=e,localStorage.setItem(`adminpro_gas_token`,e)},f=()=>u,ee=()=>{localStorage.clear(),location.reload()};async function p(e){let t=Array.isArray(e)?e.map(e=>typeof e==`string`?{type:`execute`,stmt:{sql:e}}:e.type?e:{type:`execute`,stmt:e}):[{type:`execute`,stmt:typeof e==`string`?{sql:e}:e}],n=await(await fetch(s,{method:`POST`,headers:{Authorization:`Bearer ${c}`,"Content-Type":`application/json`},body:JSON.stringify({requests:t})})).json();if(n.error)throw Error(n.error.message);return(n.results||[]).map(e=>{if(!e.response||!e.response.result)return[];let{cols:t,rows:n}=e.response.result;return n.map(e=>{let n={};return t.forEach((t,r)=>{n[t.name]=e[r].value}),n})})}var m=e=>e.map(e=>({type:typeof e==`number`?`float`:`text`,value:String(e)}));async function te(e){return await(await fetch(l,{method:`POST`,mode:`cors`,body:JSON.stringify(e)})).json()}var ne=async(e,t,n)=>(await te({action:`uploadFoto`,token:u,base64Data:e,fileName:t,mimeType:n})).url||``,re=async(e,t)=>(await te({action:`uploadSignature`,token:u,base64Data:e,fileName:t})).url||``,ie=async(e,t,n)=>(await te({action:`uploadEvidencia`,token:u,base64Data:e,fileName:t,mimeType:n})).url||``,ae=async(e,t)=>{let n=await p({sql:`SELECT * FROM usuarios WHERE email = ? AND password = ? AND estado = 'Activo'`,args:[{type:`text`,value:e.toLowerCase()},{type:`text`,value:t}]});return!n[0]||n[0].length===0?{success:!1,mensaje:`Credenciales incorrectas`}:await(await fetch(`${l}?action=login&email=${encodeURIComponent(e)}&password=${t}`)).json()},oe=async(e,t)=>{let n=await(await fetch(`${l}?action=verifyPin&email=${encodeURIComponent(e)}&pin=${t}`)).json();if(n.success){d(n.token);let t={...(await p({sql:`SELECT nombre, rol, email FROM usuarios WHERE email = ?`,args:[{type:`text`,value:e.toLowerCase()}]}))[0]?.[0]||{nombre:`Usuario`},token:n.token};return localStorage.setItem(`adminpro_user`,JSON.stringify(t)),{success:!0,...t}}return n},se=async()=>(await p(`SELECT * FROM usuarios ORDER BY nombre ASC`))[0]||[],ce=e=>p({sql:`INSERT INTO usuarios (email, password, nombre, rol, estado) VALUES (?,?,?,?,?)`,args:m(e)}),le=(e,t,n)=>p({sql:`UPDATE usuarios SET email=?, password=?, nombre=?, rol=?, estado=? WHERE email=?`,args:[{type:`text`,value:t},...m(n),{type:`text`,value:e}]}),ue=e=>p({sql:`DELETE FROM usuarios WHERE email = ?`,args:[{type:`text`,value:e}]}),de=async()=>((await p(`SELECT * FROM clientes ORDER BY nombre ASC`))[0]||[]).map(e=>({...e,cedula:e.id})),fe=e=>p({sql:`INSERT INTO clientes VALUES (?,?,?,?,?,?,?)`,args:m([e.cedula,e.nombre,e.telefono,e.direccion,e.email,e.tipo,new Date().toISOString()])}),pe=(e,t)=>p({sql:`UPDATE clientes SET id=?, nombre=?, telefono=?, direccion=?, email=?, tipo=? WHERE id=?`,args:[...m([t.cedula,t.nombre,t.telefono,t.direccion,t.email,t.tipo]),{type:`text`,value:e}]}),me=e=>p({sql:`DELETE FROM clientes WHERE id = ?`,args:[{type:`text`,value:e}]}),he=async()=>(await p(`SELECT * FROM inventario ORDER BY id DESC`))[0].map(e=>({...e,stockActual:e.stock_actual,stockMinimo:e.stock_minimo,precioVenta:e.precio_venta,costo:e.costo})),ge=e=>p({sql:`INSERT INTO inventario VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),_e=(e,t)=>p({sql:`UPDATE inventario SET id=?, nombre=?, marca=?, categoria=?, tipo=?, costo=?, precio_venta=?, stock_minimo=?, stock_actual=?, ubicacion=?, sku=?, imagen=? WHERE id=?`,args:[...m(t),{type:`text`,value:e}]}),ve=e=>p({sql:`DELETE FROM inventario WHERE id = ?`,args:[{type:`text`,value:e}]}),ye=async()=>(await p(`SELECT * FROM equipos`))[0]||[],be=e=>p({sql:`INSERT INTO equipos VALUES (?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),xe=(e,t)=>p({sql:`UPDATE equipos SET imei1=?, imei2=?, id_producto=?, marca=?, nombre=?, proveedor=?, costo=?, venta=?, estado=?, fecha_ingreso=? WHERE imei1=?`,args:[...m(t),{type:`text`,value:e}]}),Se=e=>p({sql:`DELETE FROM equipos WHERE imei1 = ?`,args:[{type:`text`,value:e}]}),Ce=async()=>(await p(`SELECT * FROM ventas ORDER BY fecha DESC`))[0]||[],we=async e=>{let t=`FAC-${Date.now()}`,n=new Date().toISOString(),r=[{sql:`INSERT INTO ventas VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:[{type:`text`,value:t},{type:`text`,value:n},{type:`text`,value:e.cedula||``},{type:`text`,value:e.cliente||``},{type:`text`,value:e.direccion||``},{type:`text`,value:e.productoNombre||``},{type:`text`,value:String(e.total||0)},{type:`text`,value:String(e.items?.length||1)},{type:`text`,value:e.imeis||`N/A`},{type:`float`,value:e.subtotal||0},{type:`float`,value:e.descuento||0},{type:`float`,value:e.total||0},{type:`text`,value:e.metodo||``},{type:`text`,value:e.vendedor||``},{type:`text`,value:e.firmaVendedor||``},{type:`text`,value:e.firmaComprador||``},{type:`text`,value:e.evidencia||``},{type:`text`,value:e.ciudad||``},{type:`text`,value:e.telefono||``},{type:`text`,value:e.tipoFactura||`fisica`}]}];return e.items&&Array.isArray(e.items)?e.items.forEach(e=>{r.push({sql:`UPDATE inventario SET stock_actual = stock_actual - ? WHERE id = ?`,args:[{type:`float`,value:parseFloat(e.qty||1)},{type:`text`,value:e.id}]})}):r.push({sql:`UPDATE inventario SET stock_actual = stock_actual - 1 WHERE id = ?`,args:[{type:`text`,value:e.productoId||``}]}),await p(r),{success:!0,idFactura:t}},Te=async()=>(await p(`SELECT * FROM creditos`))[0].map(e=>({...e,id:e.id_credito,idFactura:e.id_factura_ref,abonado:e.total_abonado,saldo:e.saldo_pendiente,total:e.valor_total})),Ee=e=>p({sql:`INSERT INTO creditos (id_credito, cliente, telefono, id_factura_ref, fecha_deuda, tipo, valor_total, total_abonado, saldo_pendiente, estado, detalle) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,args:m([Date.now().toString(),e.cliente,e.telefono,e.idFactura,new Date().toISOString(),e.tipo||`Crédito`,e.total,0,e.total,`Activo`,e.detalle])}),De=(e,t)=>p({sql:`UPDATE creditos SET id_credito=?, cliente=?, telefono=?, id_factura_ref=?, fecha_deuda=?, tipo=?, valor_total=?, total_abonado=?, saldo_pendiente=?, estado=?, fecha_cancelacion=?, detalle=?, historial_abonos=? WHERE id_credito=?`,args:[...m(t),{type:`text`,value:e}]}),Oe=async()=>(await p(`SELECT * FROM reventas ORDER BY fecha DESC`))[0].map(e=>({...e,id:e.id_reventa,producto:e.producto,costo:e.costo_proveedor,precio:e.precio_venta,utilidad:e.utilidad})),ke=e=>p({sql:`INSERT INTO reventas VALUES (?,?,?,?,?,?,?,?)`,args:m([`REV-${Date.now()}`,new Date().toISOString(),e.producto,e.categoria,e.costo,e.precio,e.proveedor,e.precio-e.costo])}),Ae=e=>p({sql:`DELETE FROM reventas WHERE id_reventa = ?`,args:[{type:`text`,value:e}]}),je=async()=>(await p(`SELECT * FROM servicio_tecnico ORDER BY id_orden DESC`))[0]||[],Me=e=>p({sql:`INSERT INTO servicio_tecnico VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),Ne=(e,t)=>p({sql:`UPDATE servicio_tecnico SET id_orden=?, cliente=?, telefono=?, equipo=?, imei_serie=?, falla=?, clave_patron=?, repuestos=?, costo_taller=?, abono=?, precio_final=?, estado=?, evidencias=? WHERE id_orden=?`,args:[...m(t),{type:`text`,value:e}]}),Pe=e=>p({sql:`DELETE FROM servicio_tecnico WHERE id_orden = ?`,args:[{type:`text`,value:e}]}),Fe=async()=>{let e=await p([`SELECT COUNT(*) as c FROM inventario`,`SELECT COUNT(*) as c FROM clientes`,`SELECT SUM(total) as s FROM ventas WHERE date(fecha) = date('now')`,`SELECT SUM(monto) as s FROM egresos WHERE date(fecha) = date('now')`,`SELECT SUM(stock_actual) as s FROM inventario`,`SELECT COUNT(*) as c FROM inventario WHERE stock_actual <= 1`,`SELECT * FROM ventas ORDER BY fecha DESC LIMIT 8`,`SELECT date(fecha) as d, SUM(total) as m FROM ventas WHERE date(fecha) >= date('now','-7 days') GROUP BY d`,`SELECT COUNT(*) as c FROM equipos`,`SELECT productos, COUNT(*) as qty FROM ventas GROUP BY productos ORDER BY qty DESC LIMIT 5`,`SELECT id, nombre, stock_actual, stock_minimo FROM inventario WHERE stock_actual <= 1 LIMIT 5`,`SELECT id_orden, cliente, equipo, estado FROM servicio_tecnico ORDER BY id_orden DESC LIMIT 5`,`SELECT COUNT(*) as c FROM creditos WHERE estado != 'Pagado' AND estado != 'Cancelado'`,`SELECT COUNT(*) as c FROM reventas`]),t=[],n=[];for(let r=6;r>=0;r--){let i=new Date;i.setDate(i.getDate()-r);let a=i.toISOString().slice(0,10);t.push(i.toLocaleDateString(`es-CO`,{weekday:`short`,day:`numeric`}));let o=(e[7]||[]).find(e=>e.d===a);n.push(o?o.m:0)}return{ingresosHoy:e[2]?.[0]?.s||0,egresosHoy:e[3]?.[0]?.s||0,utilidad:(e[2]?.[0]?.s||0)-(e[3]?.[0]?.s||0),totalProductos:e[0]?.[0]?.c||0,totalClientes:e[1]?.[0]?.c||0,totalStock:e[4]?.[0]?.s||0,stockCritico:e[5]?.[0]?.c||0,totalEquipos:e[8]?.[0]?.c||0,ventasRecientes:e[6]||[],topProductos:(e[9]||[]).map(e=>({nombre:e.productos,cantidad:e.qty})),productosBajoStock:(e[10]||[]).map(e=>({...e,stockActual:e.stock_actual})),tecRecientes:e[11]||[],creditosActivos:e[12]?.[0]?.c||0,totalReventas:e[13]?.[0]?.c||0,labels7d:t,ventas7d:n}},Ie=async()=>((await p(`SELECT * FROM egresos ORDER BY fecha DESC`))[0]||[]).map(e=>({...e,id:e.id_gasto})),Le=e=>p({sql:`INSERT INTO egresos VALUES (?,?,?,?,?,?,?)`,args:m([`EGR-${Date.now()}`,new Date().toISOString(),e.categoria,e.concepto,e.responsable,e.monto,``])}),Re=async()=>((await p(`SELECT * FROM nominas ORDER BY fecha DESC`))[0]||[]).map(e=>({...e,id:e.id_nomina})),ze=e=>p({sql:`INSERT INTO nominas VALUES (?,?,?,?,?,?,?,?,?,?)`,args:m([`NOM-${Date.now()}`,e.fecha||new Date().toISOString(),e.empleado,e.periodo,e.salario_base,e.deducciones,e.bonificaciones,e.total_pagar,e.estado||`Pendiente`,e.notas||``])}),Be=e=>p({sql:`DELETE FROM nominas WHERE id_nomina = ?`,args:[{type:`text`,value:e}]}),Ve=async()=>(await p(`SELECT * FROM tareas ORDER BY date(fecha_vencimiento) ASC`))[0]||[],He=e=>p({sql:`INSERT INTO tareas VALUES (?,?,?,?,?,?,?,?,?)`,args:m([`T-${Date.now()}`,e.tarea,e.fecha_inicio,e.fecha_vencimiento,e.prioridad,e.estado||`Pendiente`,e.responsable,e.notas||``,e.color||`#4f46e5`])}),Ue=(e,t)=>p({sql:`UPDATE tareas SET estado = ? WHERE id = ?`,args:[{type:`text`,value:t},{type:`text`,value:e}]}),We=e=>p({sql:`DELETE FROM tareas WHERE id = ?`,args:[{type:`text`,value:e}]});function h(e,t=`success`){let n={success:`bg-green-600`,error:`bg-error`,info:`bg-tertiary`,warning:`bg-amber-500`},r={success:`check_circle`,error:`error`,info:`info`,warning:`warning`},i=document.createElement(`div`);i.className=`fixed bottom-4 right-4 z-[9999] toast-animate`,i.innerHTML=`
  ...
  `).join(``)}}function Bt(e){let t=document.getElementById(`dash-top-productos`);if(t){if(!e||e.length===0){t.innerHTML=`<p class="p-4 text-center text-xs text-on-surface-variant italic">Sin ventas aún</p>`;return}t.innerHTML=e.map((e,t)=>`
  ...
  `).join(``)}}function Vt(e){let t=document.getElementById(`dash-stock-alertas`);if(t){if(!e||e.length===0){t.innerHTML=`<p class="p-4 text-center text-xs text-on-surface-variant italic">Stock ok</p>`;return}t.innerHTML=e.map(e=>`
  ...
  `).join(``)}}function Ht(e){let t=document.getElementById(`dash-tec-list`);if(t){if(!e||e.length===0){t.innerHTML=`<p class="p-5 text-sm text-on-surface-variant text-center">Sin servicios</p>`;return}t.innerHTML=e.map(e=>`
  ...
  `).join(``)}}function Ut(e){let t=document.getElementById(`dash-tasks-list`);if(!t)return;let n=(e||[]).filter(e=>e.estado!==`Completada`).slice(0,5);if(n.length===0){t.innerHTML=`<p class="p-8 text-center text-xs text-on-surface-variant italic">No hay pendientes</p>`;return}t.innerHTML=n.map(e=>`
  ...
  `).join(``)}function Wt(e,t){let n=document.getElementById(`dash-chart`);if(!n)return;if(!t||t.length===0||t.every(e=>e===0)){n.innerHTML=`<div class="flex-1 flex items-center justify-center text-on-surface-variant text-xs italic opacity-50">Sin ventas</div>`;return}let r=Math.max(...t,1);n.innerHTML=t.map((t,n)=>{let i=Math.max(t/r*100,5);return`
    ...
    `}).join(``)}var Gt=[],Kt=!1,qt=null,Jt,Yt,Xt,Zt,Qt,$t,en;async function tn(){if(nn(),!Kt)try{Gt=await de(),Kt=!0}catch(e){console.error(`Error loading clients for selector`,e)}}function nn(){document.getElementById(`customer-selector-modal`)||(document.body.insertAdjacentHTML(`beforeend`,`
  ...
  `),t.document.close(),setTimeout(()=>{t.print(),t.close()},500)}async function ti(){let e=document.getElementById(`sales-history-list`);if(e)try{e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-on-surface-variant italic text-sm">Cargando todas las ventas...</td></tr>`,Zr=await Ce(),Qr=[...Zr],ni()}catch(t){e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-error italic text-sm">Error: ${t.message}</td></tr>`}}function ni(){let e=document.getElementById(`sales-history-list`);if(e){if(Qr.length===0){e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-on-surface-variant italic text-sm">No se encontraron ventas</td></tr>`;return}e.innerHTML=Qr.map((e,t)=>{let n=new Date(e.fecha).toLocaleDateString(`es-CO`,{day:`2-digit`,month:`short`}),r=new Intl.NumberFormat(`es-CO`).format(e.total||0);return`
  ...
  `,n.classList.remove(`hidden`),n.classList.add(`flex`)};var ri=[],ii=!1;function ai(){return async()=>{oi(),ii||=(await si(),!0)}}function oi(){let e=document.getElementById(`task-new-btn`),t=document.getElementById(`task-modal-close`),n=document.getElementById(`task-modal-backdrop`),r=document.getElementById(`task-form`);e?.replaceWith(e.cloneNode(!0)),t?.replaceWith(t.cloneNode(!0)),n?.replaceWith(n.cloneNode(!0)),r?.replaceWith(r.cloneNode(!0)),document.getElementById(`task-new-btn`)?.addEventListener(`click`,di),document.getElementById(`task-modal-close`)?.addEventListener(`click`,fi),document.getElementById(`task-modal-backdrop`)?.addEventListener(`click`,fi),document.getElementById(`task-form`)?.addEventListener(`submit`,pi)}async function si(){let e=document.getElementById(`task-list`);try{let e=await Ve(),t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`);ri=t.rol===`Técnico de reparación`||t.rol===`Vendedor`?e.filter(e=>e.responsable===t.nombre):e,ci()}catch(t){e.innerHTML=`<li class="p-8 text-center text-error">Error: ${t.message}</li>`}}function ci(){let e=document.getElementById(`task-list`);if(!ri||ri.length===0){e.innerHTML=`<li class="p-12 text-center text-on-surface-variant italic text-sm">No hay tareas. ¡Buen trabajo!</li>`;return}e.innerHTML=ri.map(e=>{let t=e.estado===`Completada`;return`
      <li class="flex items-center gap-4 p-4 hover:bg-surface-container-low transition-colors group">
        ...
        <div class="text-right">
          <span class="text-[9px] font-black px-2 py-0.5 rounded-full ${li(e.prioridad)}">${e.prioridad}</span>
          <p class="text-[10px] text-on-surface-variant mt-1">${ui(e.fecha_vencimiento)}</p>
        </div>
    ...
    `}),e.innerHTML=r}var Y=[],yi=!1,bi=!1,xi=null;function Si(){return async()=>{yi||=(await Ci(),Ti(),!0),wi(Y)}}async function Ci(){let e=document.getElementById(`user-table-body`);try{e&&(e.innerHTML=`<tr><td colspan="5" class="p-8 text-center opacity-50">Cargando equipo...</td></tr>`),Y=await se()}catch{h(`Error al cargar usuarios`,`error`),Y=[]}}function wi(e){let t=document.getElementById(`user-table-body`);if(t){if(e.length===0){t.innerHTML=`<tr><td colspan="5" class="p-10 text-center opacity-40 italic">No hay usuarios registrados</td></tr>`;return}t.innerHTML=e.map(e=>`
  ...
  `).join(``)}}function Ti(){document.getElementById(`user-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();wi(Y.filter(e=>e.nombre.toLowerCase().includes(t)||e.email.toLowerCase().includes(t)))}),document.getElementById(`user-new-btn`)?.addEventListener(`click`,()=>Ei()),document.getElementById(`user-modal-close`)?.addEventListener(`click`,Di),document.getElementById(`user-modal-backdrop`)?.addEventListener(`click`,Di),document.getElementById(`user-form`)?.addEventListener(`submit`,Oi),window.userEdit=e=>{let t=Y.find(t=>t.email===e);t&&Ei(t)},window.userDelete=async e=>{if(e===JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).email)return h(`No puedes eliminarte a ti mismo`,`warning`);if(confirm(`¿Eliminar al usuario ${e}?`))try{await ue(e),h(`Usuario eliminado`,`success`),await Ci(),wi(Y)}catch(e){h(e.message,`error`)}}}function Ei(e=null){xi=e?e.email:null,document.getElementById(`user-form`).reset(),document.getElementById(`user-modal-title`).textContent=e?`Editar Usuario`:`Nuevo Usuario`,e&&(document.getElementById(`user-input-name`).value=e.nombre,document.getElementById(`user-input-email`).value=e.email,document.getElementById(`user-input-password`).value=e.password,document.getElementById(`user-input-rol`).value=e.rol,document.getElementById(`user-input-estado`).value=e.estado);let t=document.getElementById(`user-modal`);t.classList.remove(`hidden`),t.classList.add(`flex`)}function Di(){let e=document.getElementById(`user-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)}async function Oi(e){if(e.preventDefault(),bi)return;bi=!0;let t=document.getElementById(`user-save-btn`);t.disabled=!0,t.innerHTML=`Guardando...`;let n=e=>document.getElementById(e).value.trim(),r=n(`user-input-email`).toLowerCase(),i=[r,n(`user-input-password`),n(`user-input-name`),n(`user-input-rol`),n(`user-input-estado`)];try{xi?await le(xi,r,[i[1],i[2],i[3],i[4]]):await ce(i),h(xi?`Actualizado`:`Creado`,`success`),Di(),await Ci(),wi(Y)}catch(e){h(e.message,`error`)}finally{bi=!1,t.disabled=!1,t.innerHTML=`Guardar Usuario`}}var X=[],ki=!1,Ai=!1,ji,Mi,Ni,Pi,Fi,Ii,Li,Ri,zi,Bi,Vi,Hi,Ui,Wi,Gi;function Ki(){return async()=>{qi(),ki||=(await Ji(),Xi(),!0),Yi(X)}}function qi(){ji=document.getElementById(`ped-table-body`),Mi=document.getElementById(`ped-search`),Ni=document.getElementById(`ped-filter-status`),Pi=document.getElementById(`ped-new-btn`),Fi=document.getElementById(`ped-modal`),Ii=document.getElementById(`ped-modal-close`),Li=document.getElementById(`ped-modal-backdrop`),Ri=document.getElementById(`ped-form`),zi=document.getElementById(`ped-save-btn`),Bi=document.getElementById(`ped-id`),Vi=document.getElementById(`ped-producto`),Hi=document.getElementById(`ped-categoria`),Ui=document.getElementById(`ped-proveedor`),Wi=document.getElementById(`ped-costo`),Gi=document.getElementById(`ped-precio`)}async function Ji(){try{ji&&(ji.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando reventas...</td></tr>`),X=await Oe()}catch(e){h(`Error cargando reventas: `+e.message,`error`),X=[]}}function Yi(e){if(!ji)return;if(e.length===0){ji.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron reventas</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;ji.innerHTML=e.map(e=>{let n=Number(e.costo||0),r=Number(e.precio||0),i=Number(e.utilidad||r-n),a=e=>new Intl.NumberFormat(`es-CO`).format(e);return`
    ...
    `}).join(``)}function Xi(){let e=()=>{if(!Mi)return;let e=Mi.value.toLowerCase().trim(),t=Ni?Ni.value:``;Yi(X.filter(n=>{let r=(n.producto||``).toLowerCase().includes(e)||(n.id||``).toLowerCase().includes(e),i=t?n.categoria===t:!0;return r&&i}))};Mi?.addEventListener(`input`,e),Ni?.addEventListener(`change`,e);let t=e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))};Wi?.addEventListener(`input`,t),Gi?.addEventListener(`input`,t),Pi?.addEventListener(`click`,()=>Zi(null)),Ii?.addEventListener(`click`,Qi),Li?.addEventListener(`click`,Qi),zi?.addEventListener(`click`,$i),window.pedDelete=async e=>{if(confirm(`¿Eliminar la reventa ${e}?`))try{let t=await Ae(e);t&&t.success&&(h(`Eliminada`,`success`),await Ji(),Yi(X))}catch(e){h(e.message,`error`)}}}function Zi(e){Fi&&(Ri?.reset(),e&&(Bi&&(Bi.value=e.id),Vi&&(Vi.value=e.producto||``),Wi&&(Wi.value=e.costo||0),Gi&&(Gi.value=e.precio||0)),Fi.classList.remove(`hidden`),Fi.classList.add(`flex`))}function Qi(){Fi?.classList.add(`hidden`),Fi?.classList.remove(`flex`)}async function $i(){if(!Ai){Ai=!0;try{(await ke({producto:Vi?.value.trim(),categoria:Hi?.value,proveedor:Ui?.value.trim(),costo:parseInt(Wi?.value.replace(/\D/g,``))||0,precio:parseInt(Gi?.value.replace(/\D/g,``))||0})).success&&(h(`Guardado`,`success`),Qi(),await Ji(),Yi(X))}catch(e){h(e.message,`error`)}finally{Ai=!1}}}var Z=[],ea=!1,ta=!1,na=null,Q={};function ra(){return async()=>{ea||=(await ia(),sa(),!0),aa(Z)}}async function ia(){let e=document.getElementById(`tech-grid`);try{e&&(e.innerHTML=`<p class="col-span-full text-center p-10 opacity-50 italic">Cargando servicios...</p>`),Z=await je()}catch{h(`Error al cargar datos`,`error`),Z=[]}}function aa(e){let t=document.getElementById(`tech-grid`);if(!t)return;if(e.length===0){t.innerHTML=`<p class="col-span-full text-center p-20 opacity-30 italic text-sm">No hay órdenes de servicio activas</p>`;return}let n=e=>new Intl.NumberFormat(`es-CO`).format(e||0);t.innerHTML=e.map(e=>{let t=(e.precio_final||0)-(e.abono||0);return`
      <div class="bg-surface-container-lowest border border-surface-variant rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group relative">
        <div class="flex justify-between items-start mb-3">
          ...
          <span class="px-2 py-0.5 rounded-full text-[10px] font-bold ${oa(e.estado)}">${e.estado}</span>
        </div>
    ...
    `}).join(``)}function oa(e){return{Ingresado:`bg-slate-100 text-slate-700`,"En Revisión":`bg-blue-100 text-blue-700`,"En Taller":`bg-blue-100 text-blue-700`,Reparado:`bg-green-100 text-green-700`,Entregado:`bg-emerald-600 text-white`,"Sin Arreglo":`bg-red-100 text-red-700`}[e]||`bg-slate-100 text-slate-600`}function sa(){document.getElementById(`tech-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();aa(Z.filter(e=>e.cliente.toLowerCase().includes(t)||e.id_orden.toLowerCase().includes(t)||e.equipo.toLowerCase().includes(t)))}),document.getElementById(`tech-new-btn`)?.addEventListener(`click`,()=>ca()),document.getElementById(`tech-modal-close`)?.addEventListener(`click`,la),document.getElementById(`tech-modal-backdrop`)?.addEventListener(`click`,la),document.getElementById(`tech-form`)?.addEventListener(`submit`,ua),[`recepcion`,`resultado`].forEach(e=>{let t=document.getElementById(`tech-preview-${e}-btn`),n=document.getElementById(`tech-img-${e}`),r=document.getElementById(`tech-preview-${e}-wrap`),i=document.getElementById(`tech-preview-${e}`),a=document.getElementById(`tech-remove-${e}`);t?.addEventListener(`click`,()=>n?.click()),n?.addEventListener(`change`,n=>{let a=n.target.files[0];if(!a)return;let o=new FileReader;o.onload=n=>{Q[e]={file:a,base64:n.target.result.split(`,`)[1],mime:a.type},i.src=n.target.result,t.classList.add(`hidden`),r.classList.remove(`hidden`)},o.readAsDataURL(a)}),a?.addEventListener(`click`,()=>{n.value=``,delete Q[e],i.src=``,r.classList.add(`hidden`),t.classList.remove(`hidden`)})}),window.techEdit=e=>{let t=Z.find(t=>t.id_orden===e);t&&ca(t)},window.techPrint=e=>{let t=Z.find(t=>t.id_orden===e);if(!t)return;let n=e=>new Intl.NumberFormat(`es-CO`).format(e||0),r=(t.precio_final||0)-(t.abono||0),i=new Date().toLocaleDateString(`es-CO`,{year:`numeric`,month:`2-digit`,day:`2-digit`,hour:`2-digit`,minute:`2-digit`}),a=`
      <html>
      <head>
    ...
    `,o=window.open(``,``,`width=400,height=600`);o?(o.document.open(),o.document.write(a),o.document.close(),o.focus(),setTimeout(()=>{o.print(),o.close()},250)):h(`Por favor permite las ventanas emergentes para imprimir`,`warning`)},window.techDelete=async e=>{if(confirm(`¿Eliminar orden ${e}?`))try{(await Pe(e)).success&&(h(`Orden eliminada`,`success`),await ia(),aa(Z))}catch(e){h(e.message,`error`)}}}function ca(e=null){if(na=e?e.id_orden:null,document.getElementById(`tech-form`).reset(),document.getElementById(`tech-modal-title`).textContent=e?`Editar Orden`:`Ingreso a Servicio Técnico`,Q={},[`recepcion`,`resultado`].forEach(e=>{document.getElementById(`tech-preview-${e}-btn`)?.classList.remove(`hidden`),document.getElementById(`tech-preview-${e}-wrap`)?.classList.add(`hidden`),document.getElementById(`tech-preview-${e}`).src=``}),e&&(document.getElementById(`tech-cliente`).value=e.cliente,document.getElementById(`tech-equipo`).value=e.equipo,document.getElementById(`tech-falla`).value=e.falla,document.getElementById(`tech-costo`).value=new Intl.NumberFormat(`es-CO`).format(e.precio_final||0),document.getElementById(`tech-estado`).value=e.estado,e.evidencias))try{let t=JSON.parse(e.evidencias);[`recepcion`,`resultado`].forEach(e=>{t[e]&&(document.getElementById(`tech-preview-${e}`).src=t[e],document.getElementById(`tech-preview-${e}-btn`).classList.add(`hidden`),document.getElementById(`tech-preview-${e}-wrap`).classList.remove(`hidden`),Q[e]={url:t[e]})})}catch{}let t=document.getElementById(`tech-modal`);t.classList.remove(`hidden`),t.classList.add(`flex`)}function la(){let e=document.getElementById(`tech-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)}async function ua(e){if(e.preventDefault(),ta)return;ta=!0;let t=document.getElementById(`tech-save-btn`);t.disabled=!0;let n=na||`ST-${Date.now().toString().slice(-6)}`;t.innerHTML=`<span class="material-symbols-outlined animate-spin">progress_activity</span> Subiendo...`;let r={};for(let e of[`recepcion`,`resultado`])if(Q[e])if(Q[e].base64)try{let t=await ie(Q[e].base64,`${n}_${e}`,Q[e].mime);t&&(r[e]=t)}catch(e){console.error(`Error upload evidencia`,e)}else Q[e].url&&(r[e]=Q[e].url);let i=[n,document.getElementById(`tech-cliente`).value.trim(),`310`,document.getElementById(`tech-equipo`).value.trim(),`S/N`,document.getElementById(`tech-falla`).value.trim(),`0000`,``,0,0,parseInt(document.getElementById(`tech-costo`).value.replace(/\D/g,``))||0,document.getElementById(`tech-estado`).value,JSON.stringify(r)];try{(na?await Ne(na,i):await Me(i)).success&&(h(na?`Actualizado`:`Ingresado`,`success`),la(),await ia(),aa(Z))}catch(e){h(e.message,`error`)}finally{ta=!1,t.disabled=!1,t.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}}var da=[],fa=!1,pa=!1,ma,ha,ga,_a,va,ya,ba,xa,Sa,Ca,wa,Ta,Ea;function Da(){return async()=>{Oa(),fa||=(await ka(),ja(),!0),Aa(da)}}function Oa(){ma=document.getElementById(`exp-table-body`),ha=document.getElementById(`exp-search`),ga=document.getElementById(`exp-filter-cat`),_a=document.getElementById(`exp-new-btn`),va=document.getElementById(`exp-modal`),ya=document.getElementById(`exp-modal-close`),ba=document.getElementById(`exp-modal-backdrop`),xa=document.getElementById(`exp-form`),Sa=document.getElementById(`exp-save-btn`),Ca=document.getElementById(`exp-monto`),wa=document.getElementById(`exp-categoria`),Ta=document.getElementById(`exp-concepto`),Ea=document.getElementById(`exp-responsable`)}async function ka(){try{ma.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando egresos...</td></tr>`,da=await Ie()}catch(e){h(`Error cargando egresos: `+e.message,`error`),da=[]}}function Aa(e){if(e.length===0){ma.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron egresos</td></tr>`;return}ma.innerHTML=e.map(e=>`
      <tr class="hover:bg-surface-container-low transition-colors">
        <td class="px-4 py-3">
    ...
    `).join(``)}function ja(){let e=()=>{let e=ha.value.toLowerCase().trim(),t=ga.value;Aa(da.filter(n=>{let r=(n.concepto||``).toLowerCase().includes(e)||(n.responsable||``).toLowerCase().includes(e)||(n.id||``).toLowerCase().includes(e),i=t?n.categoria===t:!0;return r&&i}))};ha.addEventListener(`input`,e),ga.addEventListener(`change`,e),Ca.addEventListener(`input`,e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))}),_a.addEventListener(`click`,()=>Ma()),ya.addEventListener(`click`,Na),ba.addEventListener(`click`,Na),Sa.addEventListener(`click`,Pa)}function Ma(){xa.reset();try{let e=localStorage.getItem(`adminpro_user`);if(e){let t=JSON.parse(e);Ea.value=t.nombre||t.email}}catch{Ea.value=`Sistema`}va.classList.remove(`hidden`),va.classList.add(`flex`),Ca.focus()}function Na(){va.classList.add(`hidden`),va.classList.remove(`flex`)}async function Pa(){if(!xa.checkValidity()){xa.reportValidity();return}if(!pa){pa=!0,Sa.textContent=`Registrando...`,Sa.disabled=!0;try{let e=await Le({monto:parseInt(Ca.value.replace(/\D/g,``))||0,categoria:wa.value,concepto:Ta.value.trim(),responsable:Ea.value});e&&e.success?(h(`Egreso registrado`,`success`),Na(),await ka(),Aa(da)):h(e?.mensaje||`Error al registrar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{pa=!1,Sa.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,Sa.disabled=!1}}}var Fa=[],Ia=[];function La(){return async()=>{await Ra(),Ua()}}async function Ra(){try{let e=document.getElementById(`nom-list`);e&&(e.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-sm text-on-surface-variant">Cargando nóminas...</td></tr>`),[Fa,Ia]=await Promise.all([Re(),se()]),za(Fa),Ba(Fa)}catch{h(`Error al cargar nóminas`,`error`)}}function za(e){let t=document.getElementById(`nom-list`);if(t){if(!e||e.length===0){t.innerHTML=`
      <tr>
    ...
    `;let t=document.getElementById(`mobile-drawer-grid`);t&&(t.innerHTML=Qa.flatMap(e=>e.items).filter(e=>!e.roles||e.roles.includes(i)).map(e=>`
      ...
      `).join(``))}else Qa.forEach(e=>{let t=e.items.filter(e=>!e.roles||e.roles.includes(i));t.length!==0&&(a+=`<p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500 mt-6 mb-2 px-4 italic">${e.label}</p>`,t.forEach(e=>{a+=`
        ...
        `}))});r.innerHTML=a,n&&(document.getElementById(`mobile-more-btn`)?.addEventListener(`click`,()=>$a(!0)),document.getElementById(`mobile-drawer-close`)?.addEventListener(`click`,()=>$a(!1)),document.getElementById(`mobile-drawer-backdrop`)?.addEventListener(`click`,()=>$a(!1)),document.querySelectorAll(`#mobile-drawer-grid [data-nav]`).forEach(e=>{e.addEventListener(`click`,()=>{$a(!1),o(e.dataset.nav)})})),r.querySelectorAll(`[data-nav]`).forEach(e=>{e.addEventListener(`click`,()=>o(e.dataset.nav))})}function to(e){document.querySelectorAll(`#desktop-nav [data-nav]`).forEach(t=>{let n=t.dataset.nav===e;t.classList.toggle(`bg-primary`,n),t.classList.toggle(`text-white`,n),t.classList.toggle(`shadow-lg`,n),t.classList.toggle(`text-slate-400`,!n)}),document.querySelectorAll(`#mobile-nav [data-nav]`).forEach(t=>{let n=t.dataset.nav===e;t.classList.toggle(`text-primary`,n),t.classList.toggle(`text-on-surface-variant`,!n)});let t=`AdminPro`;Qa.forEach(n=>{let r=n.items.find(t=>t.id===e);r&&(t=r.label)});let n=document.getElementById(`header-title`);n&&(n.textContent=t)}function no(e,t){document.getElementById(`login-screen`).classList.add(`hidden`),document.getElementById(`app-shell`).classList.remove(`hidden`);let n=document.getElementById(`user-name`);n&&(n.textContent=e||`Usuario`),eo(`desktop-nav`,t,!1),eo(`mobile-nav`,t,!0);let s=t||`Vendedor`;a(e=>{let t=!1,n=!1;if(Qa.forEach(r=>{let i=r.items.find(t=>t.id===e);i&&(n=!0,(!i.roles||i.roles.includes(s))&&(t=!0))}),n&&!t)return h(`No tienes permiso para acceder a este módulo`,`warning`),`dashboard`}),document.querySelectorAll(`[data-goto]`).forEach(e=>{let t=e.dataset.goto,n=!1;Qa.forEach(e=>{let r=e.items.find(e=>e.id===t);r&&(!r.roles||r.roles.includes(s))&&(n=!0)}),n?e.classList.remove(`hidden`):e.classList.add(`hidden`)}),i(to),r(`inventory`,Mt()),r(`dashboard`,Ft()),r(`pos`,Pn()),r(`imei`,lr()),r(`clients`,Nr()),r(`credits`,Ur()),r(`sales-history`,$r()),r(`tasks`,ai()),r(`calendar`,gi()),r(`users`,Si()),r(`reventas`,Ki()),r(`technical`,ra()),r(`expenses`,Da()),r(`nominas`,La()),r(`settings`,Wa()),o(`dashboard`)}function ro(e){let t=document.getElementById(`step-credentials`),n=document.getElementById(`step-pin`);t&&t.classList.toggle(`hidden`,e!==`credentials`),n&&n.classList.toggle(`hidden`,e!==`pin`)}function io(){document.getElementById(`app-shell`).classList.add(`hidden`),document.getElementById(`login-screen`).classList.remove(`hidden`),ro(`credentials`)}async function ao(){await ee(),Za(),io()}window.addEventListener(`session-expired`,()=>{Za(),io()}),document.getElementById(`login-form`)?.addEventListener(`submit`,oo),document.getElementById(`pin-form`)?.addEventListener(`submit`,so),document.getElementById(`back-to-login`)?.addEventListener(`click`,()=>ro(`credentials`)),document.getElementById(`logout-btn`)?.addEventListener(`click`,ao);async function oo(e){e.preventDefault();let t=document.getElementById(`login-btn`),n=document.getElementById(`login-email`).value.trim(),r=document.getElementById(`login-pwd`).value.trim();t.disabled=!0,t.textContent=`Verificando...`;try{let e=await ae(n,r);e.success&&e.step===`pin`?(qa=n,document.getElementById(`pin-hint`).textContent=`Enviamos un PIN a ${n}`,ro(`pin`),document.getElementById(`login-pin`).focus()):h(e.mensaje||`Credenciales incorrectas`,`error`)}catch{h(`Error de conexión`,`error`)}finally{t.disabled=!1,t.textContent=`Ingresar`}}async function so(e){e.preventDefault();let t=document.getElementById(`pin-btn`),n=document.getElementById(`login-pin`).value.trim();t.disabled=!0,t.textContent=`Verificando...`;try{let e=await oe(qa,n);e.success&&e.token?(Xa({email:e.email,nombre:e.nombre,rol:e.rol},e.token),no(e.nombre,e.rol)):h(e.mensaje||`PIN incorrecto`,`error`)}catch{h(`Error`,`error`)}finally{t.disabled=!1,t.textContent=`Verificar`}}var co=Ya();co&&co.token&&f()?(d(co.token),no(co.nombre,co.rol)):(Za(),io());