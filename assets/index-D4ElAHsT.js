(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e={},t=null,n=null;function r(t,n){e[t]=n}function i(e){t=e}function a(e){n=e}async function o(r){if(n){let e=n(r);if(e===!1)return;typeof e==`string`&&(r=e)}document.querySelectorAll(`[data-view]`).forEach(e=>{e.classList.add(`hidden`)});let i=document.querySelector(`[data-view="${r}"]`);if(!i)return console.warn(`View not found:`,r);i.classList.remove(`hidden`),t&&t(r),e[r]&&await e[r]()}var s=`https://adminpro-adminpro.aws-us-west-2.turso.io/v2/pipeline`,c=`eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3NzcyOTIwOTMsImlkIjoiMDE5ZGNlZGItOTIwMS03NGVkLWIwZGYtZjg4MTQ3NjlhODcxIiwicmlkIjoiNWIwMWViNTctYTgxYS00OTI0LWIzMDUtZjk1Y2EwMjUzNmRkIn0.oruUZmv_ZLWlKA2ctQnghAD5PIiJSIeR4nzbZia-q-f1r12IHhLv1hDw9CsReABIceaVRHPS52JMZ4j3lcZ1Bw`,l=`https://script.google.com/macros/s/AKfycbxxs-PUyNqqALN2-azEHaViv5PM1r5oteanvr2Sfydic-bBQJrGWj00R0FO7UAlP4Ug/exec`,u=localStorage.getItem(`adminpro_gas_token`)||``,d=e=>{u=e,localStorage.setItem(`adminpro_gas_token`,e)},f=()=>u,ee=()=>{localStorage.clear(),location.reload()};async function p(e){let t=Array.isArray(e)?e.map(e=>typeof e==`string`?{type:`execute`,stmt:{sql:e}}:e.type?e:{type:`execute`,stmt:e}):[{type:`execute`,stmt:typeof e==`string`?{sql:e}:e}],n=await(await fetch(s,{method:`POST`,headers:{Authorization:`Bearer ${c}`,"Content-Type":`application/json`},body:JSON.stringify({requests:t})})).json();if(n.error)throw Error(n.error.message);return(n.results||[]).map(e=>{if(!e.response||!e.response.result)return[];let{cols:t,rows:n}=e.response.result;return n.map(e=>{let n={};return t.forEach((t,r)=>{n[t.name]=e[r].value}),n})})}var m=e=>e.map(e=>({type:typeof e==`number`?`float`:`text`,value:String(e)}));async function te(e){return await(await fetch(l,{method:`POST`,mode:`cors`,body:JSON.stringify(e)})).json()}var ne=async(e,t,n)=>(await te({action:`uploadFoto`,token:u,base64Data:e,fileName:t,mimeType:n})).url||``,re=async(e,t)=>(await te({action:`uploadSignature`,token:u,base64Data:e,fileName:t})).url||``,ie=async(e,t,n)=>(await te({action:`uploadEvidencia`,token:u,base64Data:e,fileName:t,mimeType:n})).url||``,ae=async(e,t)=>{let n=await p({sql:`SELECT * FROM usuarios WHERE email = ? AND password = ? AND estado = 'Activo'`,args:[{type:`text`,value:e.toLowerCase()},{type:`text`,value:t}]});return!n[0]||n[0].length===0?{success:!1,mensaje:`Credenciales incorrectas`}:await(await fetch(`${l}?action=login&email=${encodeURIComponent(e)}&password=${t}`)).json()},oe=async(e,t)=>{let n=await(await fetch(`${l}?action=verifyPin&email=${encodeURIComponent(e)}&pin=${t}`)).json();if(n.success){d(n.token);let t={...(await p({sql:`SELECT nombre, rol, email FROM usuarios WHERE email = ?`,args:[{type:`text`,value:e.toLowerCase()}]}))[0]?.[0]||{nombre:`Usuario`},token:n.token};return localStorage.setItem(`adminpro_user`,JSON.stringify(t)),{success:!0,...t}}return n},se=async()=>(await p(`SELECT * FROM usuarios ORDER BY nombre ASC`))[0]||[],ce=e=>p({sql:`INSERT INTO usuarios (email, password, nombre, rol, estado) VALUES (?,?,?,?,?)`,args:m(e)}),le=(e,t,n)=>p({sql:`UPDATE usuarios SET email=?, password=?, nombre=?, rol=?, estado=? WHERE email=?`,args:[{type:`text`,value:t},...m(n),{type:`text`,value:e}]}),ue=e=>p({sql:`DELETE FROM usuarios WHERE email = ?`,args:[{type:`text`,value:e}]}),de=async()=>((await p(`SELECT * FROM clientes ORDER BY nombre ASC`))[0]||[]).map(e=>({...e,cedula:e.id})),fe=e=>p({sql:`INSERT INTO clientes VALUES (?,?,?,?,?,?,?)`,args:m([e.cedula,e.nombre,e.telefono,e.direccion,e.email,e.tipo,new Date().toISOString()])}),pe=(e,t)=>p({sql:`UPDATE clientes SET id=?, nombre=?, telefono=?, direccion=?, email=?, tipo=? WHERE id=?`,args:[...m([t.cedula,t.nombre,t.telefono,t.direccion,t.email,t.tipo]),{type:`text`,value:e}]}),me=e=>p({sql:`DELETE FROM clientes WHERE id = ?`,args:[{type:`text`,value:e}]}),he=async()=>(await p(`SELECT * FROM inventario ORDER BY id DESC`))[0].map(e=>({...e,stockActual:e.stock_actual,stockMinimo:e.stock_minimo,precioVenta:e.precio_venta,costo:e.costo})),ge=e=>p({sql:`INSERT INTO inventario VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),_e=(e,t)=>p({sql:`UPDATE inventario SET id=?, nombre=?, marca=?, categoria=?, tipo=?, costo=?, precio_venta=?, stock_minimo=?, stock_actual=?, ubicacion=?, sku=?, imagen=? WHERE id=?`,args:[...m(t),{type:`text`,value:e}]}),ve=e=>p({sql:`DELETE FROM inventario WHERE id = ?`,args:[{type:`text`,value:e}]}),ye=async()=>(await p(`SELECT * FROM equipos`))[0]||[],be=e=>p({sql:`INSERT INTO equipos VALUES (?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),xe=(e,t)=>p({sql:`UPDATE equipos SET imei1=?, imei2=?, id_producto=?, marca=?, nombre=?, proveedor=?, costo=?, venta=?, estado=?, fecha_ingreso=? WHERE imei1=?`,args:[...m(t),{type:`text`,value:e}]}),Se=e=>p({sql:`DELETE FROM equipos WHERE imei1 = ?`,args:[{type:`text`,value:e}]}),Ce=async()=>(await p(`SELECT * FROM ventas ORDER BY fecha DESC`))[0]||[],we=async e=>{let t=`FAC-${Date.now()}`,n=new Date().toISOString(),r=[{sql:`INSERT INTO ventas VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:[{type:`text`,value:t},{type:`text`,value:n},{type:`text`,value:e.cedula||``},{type:`text`,value:e.cliente||``},{type:`text`,value:e.direccion||``},{type:`text`,value:e.productoNombre||``},{type:`text`,value:String(e.total||0)},{type:`text`,value:String(e.items?.length||1)},{type:`text`,value:e.imeis||`N/A`},{type:`float`,value:e.subtotal||0},{type:`float`,value:e.descuento||0},{type:`float`,value:e.total||0},{type:`text`,value:e.metodo||``},{type:`text`,value:e.vendedor||``},{type:`text`,value:e.firmaVendedor||``},{type:`text`,value:e.firmaComprador||``},{type:`text`,value:e.evidencia||``},{type:`text`,value:e.ciudad||``},{type:`text`,value:e.telefono||``},{type:`text`,value:e.tipoFactura||`fisica`}]}];return e.items&&Array.isArray(e.items)?e.items.forEach(e=>{r.push({sql:`UPDATE inventario SET stock_actual = stock_actual - ? WHERE id = ?`,args:[{type:`float`,value:parseFloat(e.qty||1)},{type:`text`,value:e.id}]})}):r.push({sql:`UPDATE inventario SET stock_actual = stock_actual - 1 WHERE id = ?`,args:[{type:`text`,value:e.productoId||``}]}),await p(r),{success:!0,idFactura:t}},Te=async()=>(await p(`SELECT * FROM creditos`))[0].map(e=>({...e,id:e.id_credito,idFactura:e.id_factura_ref,abonado:e.total_abonado,saldo:e.saldo_pendiente,total:e.valor_total})),Ee=e=>p({sql:`INSERT INTO creditos (id_credito, cliente, telefono, id_factura_ref, fecha_deuda, tipo, valor_total, total_abonado, saldo_pendiente, estado, detalle) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,args:m([Date.now().toString(),e.cliente,e.telefono,e.idFactura,new Date().toISOString(),e.tipo||`Crédito`,e.total,0,e.total,`Activo`,e.detalle])}),De=(e,t)=>p({sql:`UPDATE creditos SET id_credito=?, cliente=?, telefono=?, id_factura_ref=?, fecha_deuda=?, tipo=?, valor_total=?, total_abonado=?, saldo_pendiente=?, estado=?, fecha_cancelacion=?, detalle=?, historial_abonos=? WHERE id_credito=?`,args:[...m(t),{type:`text`,value:e}]}),Oe=async()=>(await p(`SELECT * FROM reventas ORDER BY fecha DESC`))[0].map(e=>({...e,id:e.id_reventa,producto:e.producto,costo:e.costo_proveedor,precio:e.precio_venta,utilidad:e.utilidad})),ke=e=>p({sql:`INSERT INTO reventas VALUES (?,?,?,?,?,?,?,?)`,args:m([`REV-${Date.now()}`,new Date().toISOString(),e.producto,e.categoria,e.costo,e.precio,e.proveedor,e.precio-e.costo])}),Ae=e=>p({sql:`DELETE FROM reventas WHERE id_reventa = ?`,args:[{type:`text`,value:e}]}),je=async()=>(await p(`SELECT * FROM servicio_tecnico ORDER BY id_orden DESC`))[0]||[],Me=e=>p({sql:`INSERT INTO servicio_tecnico VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),Ne=(e,t)=>p({sql:`UPDATE servicio_tecnico SET id_orden=?, cliente=?, telefono=?, equipo=?, imei_serie=?, falla=?, clave_patron=?, repuestos=?, costo_taller=?, abono=?, precio_final=?, estado=?, evidencias=? WHERE id_orden=?`,args:[...m(t),{type:`text`,value:e}]}),Pe=e=>p({sql:`DELETE FROM servicio_tecnico WHERE id_orden = ?`,args:[{type:`text`,value:e}]}),Fe=async()=>{let e=await p([`SELECT COUNT(*) as c FROM inventario`,`SELECT COUNT(*) as c FROM clientes`,`SELECT SUM(total) as s FROM ventas WHERE date(fecha) = date('now')`,`SELECT SUM(monto) as s FROM egresos WHERE date(fecha) = date('now')`,`SELECT SUM(stock_actual) as s FROM inventario`,`SELECT COUNT(*) as c FROM inventario WHERE stock_actual <= 1`,`SELECT * FROM ventas ORDER BY fecha DESC LIMIT 8`,`SELECT date(fecha) as d, SUM(total) as m FROM ventas WHERE date(fecha) >= date('now','-7 days') GROUP BY d`,`SELECT COUNT(*) as c FROM equipos`,`SELECT productos, COUNT(*) as qty FROM ventas GROUP BY productos ORDER BY qty DESC LIMIT 5`,`SELECT id, nombre, stock_actual, stock_minimo FROM inventario WHERE stock_actual <= 1 LIMIT 5`,`SELECT id_orden, cliente, equipo, estado FROM servicio_tecnico ORDER BY id_orden DESC LIMIT 5`,`SELECT COUNT(*) as c FROM creditos WHERE estado != 'Pagado' AND estado != 'Cancelado'`,`SELECT COUNT(*) as c FROM reventas`]),t=[],n=[];for(let r=6;r>=0;r--){let i=new Date;i.setDate(i.getDate()-r);let a=i.toISOString().slice(0,10);t.push(i.toLocaleDateString(`es-CO`,{weekday:`short`,day:`numeric`}));let o=(e[7]||[]).find(e=>e.d===a);n.push(o?o.m:0)}return{ingresosHoy:e[2]?.[0]?.s||0,egresosHoy:e[3]?.[0]?.s||0,utilidad:(e[2]?.[0]?.s||0)-(e[3]?.[0]?.s||0),totalProductos:e[0]?.[0]?.c||0,totalClientes:e[1]?.[0]?.c||0,totalStock:e[4]?.[0]?.s||0,stockCritico:e[5]?.[0]?.c||0,totalEquipos:e[8]?.[0]?.c||0,ventasRecientes:e[6]||[],topProductos:(e[9]||[]).map(e=>({nombre:e.productos,cantidad:e.qty})),productosBajoStock:(e[10]||[]).map(e=>({...e,stockActual:e.stock_actual})),tecRecientes:e[11]||[],creditosActivos:e[12]?.[0]?.c||0,totalReventas:e[13]?.[0]?.c||0,labels7d:t,ventas7d:n}},Ie=async()=>((await p(`SELECT * FROM egresos ORDER BY fecha DESC`))[0]||[]).map(e=>({...e,id:e.id_gasto})),Le=e=>p({sql:`INSERT INTO egresos VALUES (?,?,?,?,?,?,?)`,args:m([`EGR-${Date.now()}`,new Date().toISOString(),e.categoria,e.concepto,e.responsable,e.monto,``])}),Re=async()=>((await p(`SELECT * FROM nominas ORDER BY fecha DESC`))[0]||[]).map(e=>({...e,id:e.id_nomina})),ze=e=>p({sql:`INSERT INTO nominas VALUES (?,?,?,?,?,?,?,?,?,?)`,args:m([`NOM-${Date.now()}`,e.fecha||new Date().toISOString(),e.empleado,e.periodo,e.salario_base,e.deducciones,e.bonificaciones,e.total_pagar,e.estado||`Pendiente`,e.notas||``])}),Be=e=>p({sql:`DELETE FROM nominas WHERE id_nomina = ?`,args:[{type:`text`,value:e}]}),Ve=async()=>(await p(`SELECT * FROM tareas ORDER BY date(fecha_vencimiento) ASC`))[0]||[],He=e=>p({sql:`INSERT INTO tareas VALUES (?,?,?,?,?,?,?,?,?)`,args:m([`T-${Date.now()}`,e.tarea,e.fecha_inicio,e.fecha_vencimiento,e.prioridad,e.estado||`Pendiente`,e.responsable,e.notas||``,e.color||`#4f46e5`])}),Ue=(e,t)=>p({sql:`UPDATE tareas SET estado = ? WHERE id = ?`,args:[{type:`text`,value:t},{type:`text`,value:e}]}),We=e=>p({sql:`DELETE FROM tareas WHERE id = ?`,args:[{type:`text`,value:e}]});function h(e,t=`success`){let n={success:`bg-green-600`,error:`bg-error`,info:`bg-tertiary`,warning:`bg-amber-500`},r={success:`check_circle`,error:`error`,info:`info`,warning:`warning`},i=document.createElement(`div`);i.className=`fixed bottom-4 right-4 z-[9999] toast-animate`,i.innerHTML=`
  ...
  `).join(``)}function Wt(e,t){let n=document.getElementById(`dash-chart`);if(!n)return;if(!t||t.length===0||t.every(e=>e===0)){n.innerHTML=`<div class="flex-1 flex items-center justify-center text-on-surface-variant text-xs italic opacity-50">Sin ventas</div>`;return}let r=Math.max(...t,1);n.innerHTML=t.map((t,n)=>{let i=Math.max(t/r*100,5);return`
    ...
    `}).join(``)}var O=[],Gt=!1,Kt=null,qt,Jt,Yt,Xt,Zt,Qt,$t;async function en(){if(tn(),!Gt)try{O=await de(),Gt=!0}catch(e){console.error(`Error loading clients for selector`,e)}}function tn(){document.getElementById(`customer-selector-modal`)||(document.body.insertAdjacentHTML(`beforeend`,`
  ...
  `),qt=document.getElementById(`customer-selector-modal`),Jt=document.getElementById(`cs-backdrop`),Yt=document.getElementById(`cs-close`),Xt=document.getElementById(`cs-search`),Zt=document.getElementById(`cs-new-btn`),Qt=document.getElementById(`cs-results`),$t=document.getElementById(`cs-new-form-area`),Yt.addEventListener(`click`,sn),Jt.addEventListener(`click`,sn),Xt.addEventListener(`input`,nn),Zt.addEventListener(`click`,()=>{$t.classList.toggle(`hidden`),document.getElementById(`cs-new-doc`).focus()}),document.getElementById(`cs-cancel-new`).addEventListener(`click`,()=>{$t.classList.add(`hidden`)}),document.getElementById(`cs-save-new`).addEventListener(`click`,async()=>{let e=document.getElementById(`cs-new-doc`).value.trim(),t=document.getElementById(`cs-new-nom`).value.trim(),n=document.getElementById(`cs-new-tel`).value.trim(),r=document.getElementById(`cs-new-email`).value.trim(),i=document.getElementById(`cs-new-dir`).value.trim(),a=document.getElementById(`cs-new-tipo`).value;if(!e||!t){h(`Documento y Nombre son obligatorios`,`warning`);return}let o=document.getElementById(`cs-save-new`);o.disabled=!0,o.textContent=`...`;try{let o=await fe({documento:e,cedula:e,nombre:t,telefono:n,direccion:i,email:r,tipo:a});if(o&&o.success){h(`Cliente creado`,`success`);let o={cedula:e,documento:e,nombre:t,telefono:n,direccion:i,email:r,tipo:a,id:e};O.push(o),$t.classList.add(`hidden`),document.getElementById(`cs-new-doc`).value=``,document.getElementById(`cs-new-nom`).value=``,document.getElementById(`cs-new-tel`).value=``,document.getElementById(`cs-new-email`).value=``,document.getElementById(`cs-new-dir`).value=``,document.getElementById(`cs-new-tipo`).value=`General`,an(o)}else h(o.mensaje||`Error al crear`,`error`)}catch{h(`Error de conexión`,`error`)}finally{o.disabled=!1,o.textContent=`Guardar`}}))}function nn(){let e=Xt.value.toLowerCase().trim();if(!e){Qt.innerHTML=`<li class="p-4 text-center text-sm text-on-surface-variant">Escribe para buscar...</li>`;return}rn(O.filter(t=>(t.cedula||t.documento||``).toLowerCase().includes(e)||t.nombre&&t.nombre.toLowerCase().includes(e)||t.telefono&&t.telefono.toLowerCase().includes(e)).slice(0,20))}function rn(e){if(e.length===0){Qt.innerHTML=`<li class="p-4 text-center text-sm text-on-surface-variant">No se encontraron clientes.</li>`;return}Qt.innerHTML=e.map(e=>{let t=e.cedula||e.documento||``;return`
    <li>
  ...
  `}).join(``),document.querySelectorAll(`.cs-item-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.doc,n=O.find(e=>(e.cedula||e.documento||``)===t);n&&(n.documento=t,an(n))})})}function an(e){Kt&&Kt(e),sn()}async function on(e){await en(),Kt=e,Xt.value=``,O.length>0?rn([...O].reverse().slice(0,20)):Qt.innerHTML=`<li class="p-4 text-center text-sm text-on-surface-variant">No hay clientes. Crea uno nuevo.</li>`,$t.classList.add(`hidden`),qt.classList.remove(`hidden`),qt.classList.add(`flex`),setTimeout(()=>Xt.focus(),100)}function sn(){qt.classList.add(`hidden`),qt.classList.remove(`flex`),Kt=null}var k=[],A=[],cn=!1,ln=!1,un=null,j,dn,fn,pn,mn,hn,gn,_n,vn,yn,bn,M,xn,N,Sn,Cn,wn,Tn,En,Dn,On,kn;function An(){return async()=>{jn(),cn||=(await Mn(),Pn(),Rn(N,e=>e),Rn(Sn,e=>e),!0),Nn(k),P()}}function jn(){j=document.getElementById(`pos-search`),dn=document.getElementById(`pos-products-grid`),fn=document.getElementById(`pos-cart-items`),pn=document.getElementById(`pos-subtotal`),mn=document.getElementById(`pos-descuento`),hn=document.getElementById(`pos-total`),document.getElementById(`pos-pay-btn`),gn=document.getElementById(`pos-cliente-doc`),_n=document.getElementById(`pos-cliente-nombre`),vn=document.getElementById(`pos-checkout-modal`),yn=document.getElementById(`pos-checkout-close`),bn=document.getElementById(`pos-checkout-cancel`),M=document.getElementById(`pos-checkout-confirm`),xn=document.getElementById(`pos-evidencia-file`),N=document.getElementById(`pos-canvas-cliente`),Sn=document.getElementById(`pos-canvas-vendedor`),Cn=document.getElementById(`pos-cliente-direccion`),wn=document.getElementById(`pos-cliente-ciudad`),Tn=document.getElementById(`pos-cliente-tel`),En=document.getElementById(`pos-digital-fields`),Dn=document.getElementById(`pos-signatures-container`),On=document.getElementById(`pos-imei-container`),kn=document.getElementById(`pos-imei-list`)}async function Mn(){try{k=(await he()).filter(e=>e.stockActual>0)}catch{k=[]}}function Nn(e){if(e.length===0){dn.innerHTML=`<p class="p-4 col-span-full text-center opacity-50 italic text-sm">Sin stock disponible</p>`;return}dn.innerHTML=e.map(e=>`
  ...
  `).join(``)}function Pn(){j.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();Nn(t?k.filter(e=>e.nombre.toLowerCase().includes(t)||e.sku.toLowerCase().includes(t)):k)}),document.addEventListener(`barcodeScanned`,e=>{let t=document.querySelector(`[data-view="pos"]`);if(!t||t.classList.contains(`hidden`))return;let n=e.detail,r=k.find(e=>e.sku===n||e.id===n);r?(window.posAddToCart(r.id),h(`✅ ${r.nombre} agregado`,`success`),document.activeElement===j&&(j.value=``,Nn(k),j.blur())):h(`Código ${n} no encontrado`,`warning`)}),document.getElementById(`pos-scan-btn`)?.addEventListener(`click`,()=>{vt({title:`Escanear`,onScan:e=>{j.value=e,j.dispatchEvent(new Event(`input`));let t=k.find(t=>t.sku===e||t.id===e);t?(window.posAddToCart(t.id),h(`✅ ${t.nombre} agregado`,`success`),setTimeout(()=>{j.value===e&&(j.value=``,j.dispatchEvent(new Event(`input`)))},1500)):h(`Código ${e} no encontrado en inventario`,`warning`)}})}),document.getElementById(`pos-reventa-btn`)?.addEventListener(`click`,()=>{window.__posReventaMode=!0,o(`inventory`),setTimeout(()=>{window.inventoryView&&window.inventoryView.openNuevo&&window.inventoryView.openNuevo(!0)},150)}),document.getElementById(`pos-select-client-btn`)?.addEventListener(`click`,()=>{on(e=>{_n.value=e.nombre,gn.value=e.documento})}),document.querySelectorAll(`input[name="pos-billing-type"]`).forEach(e=>{e.addEventListener(`change`,e=>{let t=e.target.value===`digital`,n=document.getElementById(`pos-evidencia-container`);t?(n.classList.add(`hidden`),En.classList.remove(`hidden`),Dn.classList.remove(`hidden`),Ln()):(n.classList.remove(`hidden`),En.classList.add(`hidden`),Dn.classList.add(`hidden`),On.classList.add(`hidden`))})}),window.posAddToCart=e=>{let t=k.find(t=>t.id===e);if(!t)return;let n=A.find(t=>t.id===e);if(n){if(n.qty>=t.stockActual)return h(`Sin stock`,`warning`);n.qty++}else A.push({...t,qty:1,precioManual:0});P(),setTimeout(()=>{let e=fn.querySelectorAll(`input[oninput*="posUpdatePrice"]`);e.length>0&&e[e.length-1].focus()},100)},window.posRemoveItem=e=>{A=A.filter(t=>t.id!==e),P()},window.posUpdateQty=(e,t)=>{let n=A.find(t=>t.id===e),r=k.find(t=>t.id===e);n&&(n.qty+=t,n.qty<=0?window.posRemoveItem(e):n.qty>r.stockActual&&(n.qty=r.stockActual),P())},window.posUpdatePrice=(e,t)=>{let n=A.find(t=>t.id===e),r=Number(t.value.replace(/\D/g,``));n&&(n.precioManual=r,t.value=new Intl.NumberFormat(`es-CO`).format(r),Fn())},mn.addEventListener(`input`,P);let e=e=>{if(A.length!==0){if(!_n.value)return h(`Nombre cliente ok?`,`warning`);un=e,zn()}};document.getElementById(`pos-pay-btn-venta`)?.addEventListener(`click`,()=>e(`venta`)),document.getElementById(`pos-pay-btn-credito`)?.addEventListener(`click`,()=>e(`credito`)),document.getElementById(`pos-pay-btn-separe`)?.addEventListener(`click`,()=>e(`separe`)),yn.addEventListener(`click`,Bn),bn.addEventListener(`click`,Bn),M.addEventListener(`click`,Vn)}function P(){if(A.length===0){fn.innerHTML=`
      ...
      </div>`,In(0);return}let e=0;fn.innerHTML=A.map(t=>{let n=t.precioManual||0;return e+=n*t.qty,`
    ...
    </div>`}).join(``),In(e)}function Fn(){let e=0;A.forEach(t=>e+=(t.precioManual||t.precioVenta||0)*t.qty),In(e)}function In(e){let t=parseFloat(mn.value)||0;pn.textContent=`$${new Intl.NumberFormat(`es-CO`).format(e)}`,hn.textContent=`$${new Intl.NumberFormat(`es-CO`).format(Math.max(0,e-t))}`}function Ln(){let e=A.filter(e=>{let t=k.find(t=>t.id===e.id);return t&&(t.categoria===`Celulares`||t.nombre.toLowerCase().includes(`celular`)||t.nombre.toLowerCase().includes(`teléfono`))});e.length>0?(On.classList.remove(`hidden`),kn.innerHTML=e.map(e=>`
    ...
    `).join(``)):(On.classList.add(`hidden`),kn.innerHTML=``)}function Rn(e,t){let n=e.getContext(`2d`);t(n);let r=t=>{let n=e.getBoundingClientRect(),r=(t.touches?t.touches[0].clientX:t.clientX)-n.left,i=(t.touches?t.touches[0].clientY:t.clientY)-n.top;return{x:r*(e.width/n.width),y:i*(e.height/n.height)}},i=!1,a=e=>{i=!0,n.beginPath();let{x:t,y:a}=r(e);n.moveTo(t,a),e.preventDefault()},o=e=>{if(!i)return;let{x:t,y:a}=r(e);n.lineTo(t,a),n.stroke(),e.preventDefault()};e.addEventListener(`mousedown`,a),e.addEventListener(`mousemove`,o),e.addEventListener(`mouseup`,()=>i=!1),e.addEventListener(`touchstart`,a,{passive:!1}),e.addEventListener(`touchmove`,o,{passive:!1}),e.addEventListener(`touchend`,()=>i=!1),e.nextElementSibling.addEventListener(`click`,()=>n.clearRect(0,0,e.width,e.height))}function zn(){vn.classList.remove(`hidden`),vn.classList.add(`flex`),[N,Sn].forEach(e=>{e.width=e.offsetWidth,e.height=e.offsetHeight;let t=e.getContext(`2d`);t.lineWidth=2,t.lineCap=`round`,t.clearRect(0,0,e.width,e.height)}),Cn.value=``,wn.value=``,Tn.value=``,document.querySelector(`input[name="pos-billing-type"][value="fisica"]`).checked=!0,document.getElementById(`pos-evidencia-container`).classList.remove(`hidden`),En.classList.add(`hidden`),Dn.classList.add(`hidden`),On.classList.add(`hidden`)}function Bn(){vn.classList.add(`hidden`),vn.classList.remove(`flex`)}async function Vn(){if(ln)return;ln=!0;let e=document.querySelector(`input[name="pos-billing-type"]:checked`).value;M.textContent=`Procesando...`,M.disabled=!0;try{let t=``,n=``,r=``;if(e===`digital`){let e=document.createElement(`canvas`);if(e.width=N.width,e.height=N.height,N.toDataURL()!==e.toDataURL()){let e=await re(N.toDataURL(`image/png`),`FirmaCliente_${Date.now()}.png`);t=typeof e==`string`?e:e?.url||``}if(Sn.toDataURL()!==e.toDataURL()){let e=await re(Sn.toDataURL(`image/png`),`FirmaVendedor_${Date.now()}.png`);n=typeof e==`string`?e:e?.url||``}}if(e===`fisica`)if(xn.files[0]){M.textContent=`Subiendo evidencia...`;let e=xn.files[0],t=await ie(await new Promise(t=>{let n=new FileReader;n.onload=e=>t(e.target.result),n.readAsDataURL(e)}),e.name,e.type);r=typeof t==`string`?t:t?.url||``}else{h(`Por favor sube la foto de la factura física`,`warning`),ln=!1,M.textContent=`Confirmar y Facturar`,M.disabled=!1;return}let i=document.querySelectorAll(`.pos-imei-input`),a={};i.forEach(e=>{let t=e.dataset.id;e.value.trim()&&(a[t]||(a[t]=[]),a[t].push(e.value.trim()))}),M.textContent=`Registrando venta...`;let o=Number(pn.textContent.replace(/\D/g,``)),s=Number(hn.textContent.replace(/\D/g,``)),c=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`),l={cedula:gn.value.trim(),cliente:_n.value.trim(),direccion:Cn.value.trim(),ciudad:wn.value.trim(),telefono:Tn.value.trim(),productoNombre:A.map(e=>`${e.nombre} (x${e.qty})`).join(`, `),productoId:A[0]?.id,items:A.map(e=>({id:e.id,qty:e.qty})),subtotal:o,descuento:Number(mn.value)||0,total:s,metodo:document.getElementById(`pos-metodo-pago`).value,vendedor:c.nombre||`Vendedor`,firmaComprador:t,firmaVendedor:n,evidencia:r,tipoFactura:e,imeis:JSON.stringify(a),emisor:{nombre:`CLAROCELL.COM`,propietario:`Yeison Rangel Rangel`,nit:`1193400777-2`,direccion:`Calle 12 No. 10 - 108, Maicao - La Guajira`,contacto:`3016807310`,correo:`yeison0021@hotmail.com`}},u=await we(l);u.success?(un!==`venta`&&await Ee({cliente:l.cliente,telefono:l.cedula,idFactura:u.idFactura,total:s,detalle:l.productoNombre,tipo:un===`separe`?`Plan Separe`:`Crédito`}),h(`Venta Exitosa`,`success`),A=[],P(),gn.value=``,_n.value=``,Bn(),Mn().then(()=>Nn(k))):h(`Error al guardar`,`error`)}catch(e){h(e.message,`error`)}finally{ln=!1,M.textContent=`Confirmar y Facturar`,M.disabled=!1}}var F=[],Hn=!1,Un=!1,Wn,Gn,Kn,qn,Jn,Yn,Xn,Zn,I,L,Qn,R,$n,er,z,B,tr,nr;function rr(){return async()=>{ir(),Hn||=(await ar(),sr(),!0),or(F)}}function ir(){Wn=document.getElementById(`imei-table-body`),Gn=document.getElementById(`imei-search`),Kn=document.getElementById(`imei-filter-status`),qn=document.getElementById(`imei-new-btn`),Jn=document.getElementById(`imei-modal`),Yn=document.getElementById(`imei-modal-close`),Xn=document.getElementById(`imei-modal-backdrop`),Zn=document.getElementById(`imei-form`),I=document.getElementById(`imei-save-btn`),L=document.getElementById(`imei-1`),Qn=document.getElementById(`imei-2`),R=document.getElementById(`imei-nombre`),$n=document.getElementById(`imei-marca`),er=document.getElementById(`imei-proveedor`),z=document.getElementById(`imei-costo`),B=document.getElementById(`imei-venta`),tr=document.getElementById(`imei-estado`),nr=document.getElementById(`imei-original`)}async function ar(){try{Wn.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando equipos...</td></tr>`,F=await ye();let e=await he();R.innerHTML=`<option value="">Seleccione un equipo...</option>`+e.map(e=>`<option value="${e.nombre}" data-marca="${e.marca}" data-costo="${e.costo}" data-precio="${e.precioVenta}" data-prov="${e.proveedor||``}" data-id="${e.id}">${e.nombre}${e.marca?` (`+e.marca+`)`:``}</option>`).join(``)}catch(e){h(`Error cargando equipos: `+e.message,`error`),F=[]}}function or(e){if(e.length===0){Wn.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron equipos</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;Wn.innerHTML=e.map(e=>{e.estado;let n=e.estado===`Disponible`?`bg-green-100 text-green-800`:e.estado===`Vendido`?`bg-red-100 text-red-800`:`bg-yellow-100 text-yellow-800`;return`
    ...
    `}).join(``)}function sr(){let e=()=>{let e=Gn.value.toLowerCase().trim(),t=Kn.value;or(F.filter(n=>{let r=(n.imei1||``).toLowerCase().includes(e)||(n.imei2||``).toLowerCase().includes(e)||(n.nombre||``).toLowerCase().includes(e),i=t?n.estado===t:!0;return r&&i}))},t=e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))};z.addEventListener(`input`,t),B.addEventListener(`input`,t),R.addEventListener(`change`,e=>{let t=e.target.selectedOptions[0];if(t&&t.value){$n.value=t.dataset.marca||``,er.value=t.dataset.prov||``;let e=parseInt(String(t.dataset.costo||`0`).replace(/\D/g,``))||0,n=parseInt(String(t.dataset.precio||`0`).replace(/\D/g,``))||0;z.value=new Intl.NumberFormat(`es-CO`).format(e),B.value=new Intl.NumberFormat(`es-CO`).format(n)}else $n.value=``,er.value=``,z.value=``,B.value=``}),Gn.addEventListener(`input`,e),Kn.addEventListener(`change`,e),document.getElementById(`imei-scan-btn`)?.addEventListener(`click`,()=>{vt({title:`Escanear IMEI`,onScan:t=>{Gn.value=t,e(),h(`IMEI: ${t}`,`info`)}})}),document.getElementById(`imei-scan-1`)?.addEventListener(`click`,()=>{vt({title:`Escanear IMEI 1`,onScan:e=>{L.value=e,h(`IMEI 1: ${e}`,`success`)}})}),document.getElementById(`imei-scan-2`)?.addEventListener(`click`,()=>{vt({title:`Escanear IMEI 2`,onScan:e=>{Qn.value=e,h(`IMEI 2: ${e}`,`success`)}})}),qn.addEventListener(`click`,()=>cr(null)),Yn.addEventListener(`click`,lr),Xn.addEventListener(`click`,lr),I.addEventListener(`click`,ur),window.imeiEdit=e=>{let t=F.find(t=>t.imei1==e);t&&cr(t)},window.imeiDelete=async t=>{if(confirm(`¿Eliminar el equipo con IMEI ${t}?`))try{h(`Eliminando...`,`info`);let n=await Se(t);n&&n.success?(h(`Equipo eliminado`,`success`),await ar(),e()):h(n.mensaje||`Error al eliminar`,`error`)}catch(e){h(`Error: `+e.message,`error`)}}}function cr(e){Zn.reset(),e?(nr.value=e?e.imei1:``,L.value=e?e.imei1:``,Qn.value=e?e.imei2:``,R.value=e?e.nombre:``,$n.value=e?e.marca:``,er.value=e?e.proveedor:``,z.value=e&&e.costo?new Intl.NumberFormat(`es-CO`).format(parseInt(String(e.costo).replace(/\D/g,``))||0):``,B.value=e&&e.venta?new Intl.NumberFormat(`es-CO`).format(parseInt(String(e.venta).replace(/\D/g,``))||0):``,tr.value=e?e.estado:`Disponible`,document.getElementById(`imei-modal-title`).textContent=`Editar Equipo`):(nr.value=``,tr.value=`Disponible`,document.getElementById(`imei-modal-title`).textContent=`Registrar Equipo`),Jn.classList.remove(`hidden`),Jn.classList.add(`flex`)}function lr(){Jn.classList.add(`hidden`),Jn.classList.remove(`flex`)}async function ur(){if(!L.value.trim()){h(`El IMEI Principal es obligatorio`,`warning`),L.focus();return}if(!R.value){h(`Debe seleccionar un equipo del inventario`,`warning`),R.focus();return}if(!Un){Un=!0,I.textContent=`Guardando...`,I.disabled=!0;try{let e=nr.value,t={imei1:L.value.trim(),imei2:Qn.value.trim(),nombre:R.value.trim(),marca:$n.value.trim(),proveedor:er.value.trim(),costo:parseInt(z.value.replace(/\D/g,``))||0,venta:parseInt(B.value.replace(/\D/g,``))||0,estado:tr.value},n;n=e?await xe(e,t):await be(t),n&&n.success?(h(`Equipo guardado`,`success`),lr(),await ar(),or(F)):h(n?.mensaje||`Error al guardar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{Un=!1,I.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,I.disabled=!1}}}var V=[],dr=!1,fr=!1,pr,mr,hr,gr,_r,vr,yr,H,br,xr,Sr,Cr,wr,Tr,Er;function Dr(){return async()=>{Or(),dr||=(await kr(),jr(),!0),Ar(V)}}function Or(){pr=document.getElementById(`cli-table-body`),mr=document.getElementById(`cli-search`),hr=document.getElementById(`cli-new-btn`),gr=document.getElementById(`cli-modal`),_r=document.getElementById(`cli-modal-close`),vr=document.getElementById(`cli-modal-backdrop`),yr=document.getElementById(`cli-form`),H=document.getElementById(`cli-save-btn`),br=document.getElementById(`cli-doc`),xr=document.getElementById(`cli-nombre`),Sr=document.getElementById(`cli-tel`),Cr=document.getElementById(`cli-email`),wr=document.getElementById(`cli-dir`),Tr=document.getElementById(`cli-tipo`),Er=document.getElementById(`cli-original`)}async function kr(){try{pr.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando clientes...</td></tr>`,V=await de()}catch(e){h(`Error cargando clientes: `+e.message,`error`),V=[]}}function Ar(e){if(e.length===0){pr.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron clientes</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;pr.innerHTML=e.map(e=>{let n=e.tipo===`VIP`?`bg-amber-100 text-amber-800`:e.tipo===`Empresa`?`bg-blue-100 text-blue-800`:e.tipo===`Mayorista`?`bg-purple-100 text-purple-800`:`bg-surface-container text-on-surface-variant`;return`
      <tr class="hover:bg-surface-container-low transition-colors">
    ...
    `}).join(``)}function jr(){let e=()=>{let e=mr.value.toLowerCase().trim();Ar(V.filter(t=>(t.cedula||``).toLowerCase().includes(e)||(t.nombre||``).toLowerCase().includes(e)||(t.telefono||``).toLowerCase().includes(e)))};mr.addEventListener(`input`,e),hr.addEventListener(`click`,()=>Mr(null)),_r.addEventListener(`click`,Nr),vr.addEventListener(`click`,Nr),H.addEventListener(`click`,Pr),window.cliEdit=e=>{let t=V.find(t=>t.cedula==e);t&&Mr(t)},window.cliDelete=async t=>{if(confirm(`¿Eliminar al cliente con documento ${t}?`))try{h(`Eliminando...`,`info`);let n=await me(t);n&&n.success?(h(`Cliente eliminado`,`success`),await kr(),e()):h(n.mensaje||`Error al eliminar`,`error`)}catch(e){h(`Error: `+e.message,`error`)}}}function Mr(e){yr.reset(),e?(Er.value=e.cedula,br.value=e.cedula,xr.value=e.nombre||``,Sr.value=e.telefono||``,Cr.value=e.email||``,wr.value=e.direccion||``,Tr.value=e.tipo||`Normal`,document.getElementById(`cli-modal-title`).textContent=`Editar Cliente`):(Er.value=``,Tr.value=`Normal`,document.getElementById(`cli-modal-title`).textContent=`Nuevo Cliente`),gr.classList.remove(`hidden`),gr.classList.add(`flex`)}function Nr(){gr.classList.add(`hidden`),gr.classList.remove(`flex`)}async function Pr(){if(!yr.checkValidity()){yr.reportValidity();return}if(!fr){fr=!0,H.textContent=`Guardando...`,H.disabled=!0;try{let e=Er.value,t={cedula:br.value.trim(),nombre:xr.value.trim(),telefono:Sr.value.trim(),email:Cr.value.trim(),direccion:wr.value.trim(),tipo:Tr.value},n;n=e?await pe(e,t):await fe(t),n&&n.success?(h(`Cliente guardado`,`success`),Nr(),await kr(),Ar(V)):h(n?.mensaje||`Error al guardar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{fr=!1,H.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,H.disabled=!1}}}var U=[],Fr=!1,Ir=!1;function Lr(){return async()=>{Fr||=(await Rr(),Wr(),!0),Ur(U)}}async function Rr(){let e=document.getElementById(`cred-table-body`);e&&(e.innerHTML=`<tr><td colspan="9" class="p-6 text-center text-on-surface-variant">Cargando créditos...</td></tr>`);try{U=await Te()}catch(e){h(`Error cargando créditos: `+e.message,`error`),U=[]}}var W=e=>`$`+new Intl.NumberFormat(`es-CO`).format(Math.round(e||0));function zr(e,t){if(!e)return 0;let n=e=>{if(!e)return null;let t=String(e).split(`/`);return t.length===3?new Date(t[2],t[1]-1,t[0]):new Date(e)},r=n(e),i=t?n(t):new Date;return!r||isNaN(r)?0:Math.max(0,Math.floor((i-r)/864e5))}function Br(e){return e?e.split(`;`).filter(Boolean).map(e=>{let t=e.split(`|`);return{fecha:t[0]||``,monto:parseFloat(t[1])||0,nota:t[2]||``}}):[]}function Vr(e){return e.map(e=>`${e.fecha}|${e.monto}|${e.nota}`).join(`;`)}function Hr(e){let t=0,n=0;e.forEach(e=>{e.estado!==`Cancelado`&&(t+=e.saldo||0),n+=e.abonado||0});let r=document.getElementById(`cred-stat-total`),i=document.getElementById(`cred-stat-recaudo`);r&&(r.textContent=W(t)),i&&(i.textContent=W(n))}function Ur(e){Hr(U);let t=document.getElementById(`cred-table-body`);if(t){if(e.length===0){t.innerHTML=`<tr><td colspan="9" class="p-6 text-center text-on-surface-variant">No se encontraron créditos</td></tr>`;return}t.innerHTML=e.map(e=>{let t=e.estado===`Cancelado`||e.estado===`Entregado`,n=zr(e.fecha,t?e.fechaCancelacion:null),r=t?`bg-green-100 text-green-800`:e.estado===`En Mora`?`bg-red-100 text-red-800`:`bg-orange-100 text-orange-800`,i=e.tipo===`Plan Separe`?`bg-emerald-100 text-emerald-800 border-emerald-200`:`bg-blue-100 text-blue-800 border-blue-200`,a=t?`<span class="text-[10px] text-on-surface-variant">Pagó en ${n}d</span>`:`<span class="text-[10px] font-bold ${n>30?`text-red-500`:`text-orange-500`}">${n} días</span>`,o=String(e.telefono||``).replace(/\D/g,``),s=`https://wa.me/57${o}?text=${encodeURIComponent(`Hola ${e.cliente}, le recordamos que tiene un saldo pendiente de ${W(e.saldo)} con nosotros. Gracias.`)}`;return`
      <tr class="hover:bg-surface-container-low transition-colors ${t?`opacity-70`:``}">
        ...
        <td class="px-4 py-3 text-sm font-medium">${W(e.total)}</td>
        <td class="px-4 py-3 text-sm font-medium text-green-600">${W(e.abonado)}</td>
        <td class="px-4 py-3 text-sm font-black text-error">${W(e.saldo)}</td>
    ...
    `}).join(``)}}function Wr(){let e=document.getElementById(`cred-search`),t=document.getElementById(`cred-filter-status`),n=document.getElementById(`cred-filter-tipo`),r=()=>{let r=(e?.value||``).toLowerCase().trim(),i=t?.value||``,a=n?.value||``;Ur(U.filter(e=>{let t=(e.cliente||``).toLowerCase().includes(r)||(e.idFactura||``).toLowerCase().includes(r),n=i?e.estado===i:!0,o=a?(e.tipo||`Crédito`)===a:!0;return t&&n&&o}))};e?.addEventListener(`input`,r),t?.addEventListener(`change`,r),n?.addEventListener(`change`,r);let i=e=>{let t=e.target.value.replace(/\D/g,``);e.target.value=t?new Intl.NumberFormat(`es-CO`).format(parseInt(t)):``};document.getElementById(`cred-new-total`)?.addEventListener(`input`,i),document.getElementById(`cred-new-abono`)?.addEventListener(`input`,i),document.getElementById(`cred-monto-abono`)?.addEventListener(`input`,i);let a=document.getElementById(`cred-modal`),o=document.getElementById(`cred-modal-close`),s=document.getElementById(`cred-modal-backdrop`),c=document.getElementById(`cred-save-btn`),l=()=>{a?.classList.add(`hidden`),a?.classList.remove(`flex`)};o?.addEventListener(`click`,l),s?.addEventListener(`click`,l),window.credAddAbono=e=>{let t=U.find(t=>t.id==e);if(!t)return;document.getElementById(`cred-id`).value=t.id,document.getElementById(`cred-cliente-name`).textContent=t.cliente,document.getElementById(`cred-saldo-actual`).textContent=W(t.saldo);let n=Br(t.historialAbonos),r=document.getElementById(`cred-historial`);r&&(r.innerHTML=n.length===0?`<p class="text-xs text-on-surface-variant">Sin abonos anteriores</p>`:n.map(e=>`
            <div class="flex justify-between items-center py-1 border-b border-surface-variant text-xs">
              ...
              <span class="font-bold text-green-600">${W(e.monto)}</span>
              <span class="text-on-surface-variant">${e.nota||``}</span>
            </div>`).join(``)),document.getElementById(`cred-monto-abono`).value=``,document.getElementById(`cred-nota-abono`).value=``,a?.classList.remove(`hidden`),a?.classList.add(`flex`),document.getElementById(`cred-monto-abono`)?.focus()},c?.addEventListener(`click`,async()=>{let e=document.getElementById(`cred-id`).value,t=parseInt((document.getElementById(`cred-monto-abono`).value||``).replace(/\D/g,``))||0,n=(document.getElementById(`cred-nota-abono`)?.value||``).trim();if(!t||t<=0){h(`Ingresa un monto válido`,`warning`);return}if(!Ir){Ir=!0,c.textContent=`Aplicando...`,c.disabled=!0;try{let r=U.find(t=>t.id==e),i=(r.abonado||0)+t,a=Math.max(0,(r.total||0)-i),o=a<=0,s=Br(r.historialAbonos);s.push({fecha:new Date().toLocaleDateString(`es-CO`),monto:t,nota:n});let c=r.tipo===`Plan Separe`,u=c?`Separado`:`Activo`,d=c?`Entregado`:`Cancelado`,f=await De(e,{...r,abonado:i,saldo:a,estado:o||r.estado===`Cancelado`||r.estado===`Entregado`?d:u,fechaCancelacion:o?new Date().toLocaleDateString(`es-CO`):r.fechaCancelacion||``,historialAbonos:Vr(s)});f?.success?(h(o?`✅ ¡Crédito cancelado!`:`Abono registrado`,`success`),l(),Fr=!1,await Rr(),Ur(U)):h(f?.mensaje||`Error al guardar`,`error`)}catch(e){h(`Error: `+e.message,`error`)}finally{Ir=!1,c.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Aplicar Abono`,c.disabled=!1}}});let u=document.getElementById(`cred-new-modal`),d=()=>{u?.classList.add(`hidden`),u?.classList.remove(`flex`)};document.getElementById(`cred-new-btn`)?.addEventListener(`click`,()=>{document.getElementById(`cred-new-form`)?.reset(),document.getElementById(`cred-new-cliente`).value=``,document.getElementById(`cred-new-cliente-doc`).value=``,u?.classList.remove(`hidden`),u?.classList.add(`flex`)}),document.getElementById(`cred-new-close`)?.addEventListener(`click`,d),document.getElementById(`cred-new-backdrop`)?.addEventListener(`click`,d),document.getElementById(`cred-select-client-btn`)?.addEventListener(`click`,()=>{on(e=>{document.getElementById(`cred-new-cliente`).value=e.nombre,document.getElementById(`cred-new-cliente-doc`).value=e.cedula||e.documento||e.telefono||``})}),document.getElementById(`cred-save-new-btn`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`cred-new-cliente`).value.trim(),t=document.getElementById(`cred-new-cliente-doc`).value.trim(),n=parseInt((document.getElementById(`cred-new-total`).value||``).replace(/\D/g,``))||0,r=parseInt((document.getElementById(`cred-new-abono`)?.value||``).replace(/\D/g,``))||0,i=document.getElementById(`cred-new-detalle`).value.trim();if(!e||!n){h(`Cliente y monto son requeridos`,`warning`);return}let a=document.getElementById(`cred-save-new-btn`);a.disabled=!0,a.textContent=`Guardando...`;try{let a=await Ee({cliente:e,telefono:t,total:n,detalle:i,historialAbonos:r>0?Vr([{fecha:new Date().toLocaleDateString(`es-CO`),monto:r,nota:`Abono inicial`}]):``});a?.success?(h(`Crédito creado`,`success`),d(),Fr=!1,await Rr(),Ur(U)):h(a?.mensaje||`Error al crear crédito`,`error`)}catch(e){h(`Error: `+e.message,`error`)}finally{a.disabled=!1,a.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}})}var Gr=[],Kr=[];function qr(){return async()=>{document.getElementById(`sales-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();Kr=Gr.filter(e=>(e.id_factura||``).toLowerCase().includes(t)||(e.cliente||``).toLowerCase().includes(t)||(e.cedula||``).toLowerCase().includes(t)),Yr()});let e=()=>{let e=document.getElementById(`sale-detail-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)};document.getElementById(`sale-detail-close`)?.addEventListener(`click`,e),document.getElementById(`sale-detail-backdrop`)?.addEventListener(`click`,e),await Jr()}}async function Jr(){let e=document.getElementById(`sales-history-list`);if(e)try{e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-on-surface-variant italic text-sm">Cargando todas las ventas...</td></tr>`,Gr=await Ce(),Kr=[...Gr],Yr()}catch(t){e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-error italic text-sm">Error: ${t.message}</td></tr>`}}function Yr(){let e=document.getElementById(`sales-history-list`);if(e){if(Kr.length===0){e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-on-surface-variant italic text-sm">No se encontraron ventas</td></tr>`;return}e.innerHTML=Kr.map((e,t)=>{let n=new Date(e.fecha).toLocaleDateString(`es-CO`,{day:`2-digit`,month:`short`}),r=new Intl.NumberFormat(`es-CO`).format(e.total||0);return`
    ...
    `}).join(``)}}window.viewSaleDetail=e=>{let t=Gr.find(t=>t.id_factura===e);if(!t)return;let n=document.getElementById(`sale-detail-modal`),r=document.getElementById(`sale-detail-content`),i=e=>new Intl.NumberFormat(`es-CO`).format(e||0);r.innerHTML=`
    <div class="space-y-6">
  ...
  `,n.classList.remove(`hidden`),n.classList.add(`flex`)};var Xr=[],Zr=!1;function Qr(){return async()=>{$r(),Zr||=(await ei(),!0)}}function $r(){let e=document.getElementById(`task-new-btn`),t=document.getElementById(`task-modal-close`),n=document.getElementById(`task-modal-backdrop`),r=document.getElementById(`task-form`);e?.replaceWith(e.cloneNode(!0)),t?.replaceWith(t.cloneNode(!0)),n?.replaceWith(n.cloneNode(!0)),r?.replaceWith(r.cloneNode(!0)),document.getElementById(`task-new-btn`)?.addEventListener(`click`,ii),document.getElementById(`task-modal-close`)?.addEventListener(`click`,ai),document.getElementById(`task-modal-backdrop`)?.addEventListener(`click`,ai),document.getElementById(`task-form`)?.addEventListener(`submit`,oi)}async function ei(){let e=document.getElementById(`task-list`);try{let e=await Ve(),t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`);Xr=t.rol===`Técnico de reparación`||t.rol===`Vendedor`?e.filter(e=>e.responsable===t.nombre):e,ti()}catch(t){e.innerHTML=`<li class="p-8 text-center text-error">Error: ${t.message}</li>`}}function ti(){let e=document.getElementById(`task-list`);if(!Xr||Xr.length===0){e.innerHTML=`<li class="p-12 text-center text-on-surface-variant italic text-sm">No hay tareas. ¡Buen trabajo!</li>`;return}e.innerHTML=Xr.map(e=>{let t=e.estado===`Completada`;return`
      <li class="flex items-center gap-4 p-4 hover:bg-surface-container-low transition-colors group">
        ...
        <div class="text-right">
          <span class="text-[9px] font-black px-2 py-0.5 rounded-full ${ni(e.prioridad)}">${e.prioridad}</span>
          <p class="text-[10px] text-on-surface-variant mt-1">${ri(e.fecha_vencimiento)}</p>
        </div>
    ...
    `}).join(``)}function ni(e){return e===`Alta`?`bg-red-100 text-red-700`:e===`Media`?`bg-blue-100 text-blue-700`:`bg-slate-100 text-slate-700`}function ri(e){return e?new Date(e+`T12:00:00`).toLocaleDateString(`es-CO`,{day:`numeric`,month:`short`}):`—`}function ii(){let e=document.getElementById(`task-modal`);document.getElementById(`task-form`).reset(),document.getElementById(`task-input-date`).value=new Date().toISOString().slice(0,10),e.classList.remove(`hidden`),e.classList.add(`flex`)}function ai(){document.getElementById(`task-modal`).classList.add(`hidden`),document.getElementById(`task-modal`).classList.remove(`flex`)}async function oi(e){e.preventDefault();let t=document.getElementById(`task-save-btn`);t.disabled=!0,t.innerHTML=`Guardando...`;let n={tarea:document.getElementById(`task-input-title`).value.trim(),fecha_inicio:new Date().toISOString().slice(0,10),fecha_vencimiento:document.getElementById(`task-input-date`).value,prioridad:document.getElementById(`task-input-priority`).value,responsable:JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).nombre||`Admin`,notas:document.getElementById(`task-input-notes`).value.trim(),estado:`Pendiente`,color:si(document.getElementById(`task-input-priority`).value)};try{await He(n)&&(h(`Tarea creada`,`success`),ai(),await ei())}catch{h(`Error al guardar`,`error`)}finally{t.disabled=!1,t.innerHTML=`<span class="material-symbols-outlined text-[20px]">save</span> Guardar Tarea`}}function si(e){return e===`Alta`?`#ef4444`:e===`Media`?`#3b82f6`:`#64748b`}window.toggleTaskStatus=async(e,t)=>{let n=t===`Completada`?`Pendiente`:`Completada`;try{await Ue(e,n),await ei()}catch(e){console.error(e)}},window.deleteTask=async e=>{if(confirm(`¿Eliminar tarea?`))try{await We(e),await ei()}catch(e){console.error(e)}};var G=[],ci=!1;function li(){return async()=>{ci||=(await ui(),!0),di()}}async function ui(){let e=document.getElementById(`calendar-container`);e&&(e.innerHTML=`<div class="p-10 text-center text-on-surface-variant italic animate-pulse">Recopilando actividad global...</div>`);try{let e=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`),t=e.rol===`Administrador`,n=[Ce().catch(()=>[]),Ve().catch(()=>[]),Oe().catch(()=>[])];t&&(n.push(Ie().catch(()=>[])),n.push(Re().catch(()=>[])));let[r,i,a,o=[],s=[]]=await Promise.all(n);G=[],r.forEach(e=>{e.fecha&&G.push({fecha:new Date(e.fecha),tipo:`Venta`,icono:`point_of_sale`,color:`text-green-600`,bg:`bg-green-100`,border:`border-green-200`,titulo:`Factura: ${e.id_factura||`N/A`}`,subtitulo:`Cliente: ${e.cliente||`Consumidor Final`}`,monto:e.total,signo:`+`,usuario:e.vendedor||`Admin`})}),i.forEach(e=>{e.fecha_vencimiento&&G.push({fecha:new Date(e.fecha_vencimiento),tipo:`Tarea`,icono:`task_alt`,color:`text-indigo-600`,bg:`bg-indigo-100`,border:`border-indigo-200`,titulo:e.tarea,subtitulo:`Resp: ${e.responsable||`Sin asignar`}`,monto:null,usuario:e.responsable||`Admin`})}),a.forEach(e=>{e.fecha&&G.push({fecha:new Date(e.fecha),tipo:`Reventa`,icono:`storefront`,color:`text-purple-600`,bg:`bg-purple-100`,border:`border-purple-200`,titulo:`Reventa: ${e.producto}`,subtitulo:`Proveedor: ${e.proveedor||`N/A`}`,monto:e.utilidad,signo:`+`,usuario:e.vendedor||`Admin`})}),o.forEach(e=>{e.fecha&&G.push({fecha:new Date(e.fecha),tipo:`Egreso`,icono:`payments`,color:`text-red-600`,bg:`bg-red-100`,border:`border-red-200`,titulo:`Gasto: ${e.concepto}`,subtitulo:`Cat: ${e.categoria}`,monto:e.monto,signo:`-`})}),s.forEach(e=>{e.fecha&&G.push({fecha:new Date(e.fecha),tipo:`Nómina`,icono:`request_quote`,color:`text-orange-600`,bg:`bg-orange-100`,border:`border-orange-200`,titulo:`Pago: ${e.empleado}`,subtitulo:`Período: ${e.periodo}`,monto:e.total_pagar,signo:`-`})}),G.sort((e,t)=>t.fecha-e.fecha),t||(G=G.filter(t=>t.usuario===e.nombre))}catch(t){console.error(t),e&&(e.innerHTML=`<div class="p-10 text-center text-error font-bold">Error cargando actividad: ${t.message}</div>`)}}function di(){let e=document.getElementById(`calendar-container`);if(!e)return;if(G.length===0){e.innerHTML=`
    ...
    `}),e.innerHTML=r}var K=[],fi=!1,pi=!1,mi=null;function hi(){return async()=>{fi||=(await gi(),vi(),!0),_i(K)}}async function gi(){let e=document.getElementById(`user-table-body`);try{e&&(e.innerHTML=`<tr><td colspan="5" class="p-8 text-center opacity-50">Cargando equipo...</td></tr>`),K=await se()}catch{h(`Error al cargar usuarios`,`error`),K=[]}}function _i(e){let t=document.getElementById(`user-table-body`);if(t){if(e.length===0){t.innerHTML=`<tr><td colspan="5" class="p-10 text-center opacity-40 italic">No hay usuarios registrados</td></tr>`;return}t.innerHTML=e.map(e=>`
  ...
  `).join(``)}}function vi(){document.getElementById(`user-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();_i(K.filter(e=>e.nombre.toLowerCase().includes(t)||e.email.toLowerCase().includes(t)))}),document.getElementById(`user-new-btn`)?.addEventListener(`click`,()=>yi()),document.getElementById(`user-modal-close`)?.addEventListener(`click`,bi),document.getElementById(`user-modal-backdrop`)?.addEventListener(`click`,bi),document.getElementById(`user-form`)?.addEventListener(`submit`,xi),window.userEdit=e=>{let t=K.find(t=>t.email===e);t&&yi(t)},window.userDelete=async e=>{if(e===JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).email)return h(`No puedes eliminarte a ti mismo`,`warning`);if(confirm(`¿Eliminar al usuario ${e}?`))try{await ue(e),h(`Usuario eliminado`,`success`),await gi(),_i(K)}catch(e){h(e.message,`error`)}}}function yi(e=null){mi=e?e.email:null,document.getElementById(`user-form`).reset(),document.getElementById(`user-modal-title`).textContent=e?`Editar Usuario`:`Nuevo Usuario`,e&&(document.getElementById(`user-input-name`).value=e.nombre,document.getElementById(`user-input-email`).value=e.email,document.getElementById(`user-input-password`).value=e.password,document.getElementById(`user-input-rol`).value=e.rol,document.getElementById(`user-input-estado`).value=e.estado);let t=document.getElementById(`user-modal`);t.classList.remove(`hidden`),t.classList.add(`flex`)}function bi(){let e=document.getElementById(`user-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)}async function xi(e){if(e.preventDefault(),pi)return;pi=!0;let t=document.getElementById(`user-save-btn`);t.disabled=!0,t.innerHTML=`Guardando...`;let n=e=>document.getElementById(e).value.trim(),r=n(`user-input-email`).toLowerCase(),i=[r,n(`user-input-password`),n(`user-input-name`),n(`user-input-rol`),n(`user-input-estado`)];try{mi?await le(mi,r,[i[1],i[2],i[3],i[4]]):await ce(i),h(mi?`Actualizado`:`Creado`,`success`),bi(),await gi(),_i(K)}catch(e){h(e.message,`error`)}finally{pi=!1,t.disabled=!1,t.innerHTML=`Guardar Usuario`}}var q=[],Si=!1,Ci=!1,J,wi,Ti,Ei,Y,Di,Oi,ki,Ai,ji,Mi,Ni,Pi,Fi,Ii;function Li(){return async()=>{Ri(),Si||=(await zi(),Vi(),!0),Bi(q)}}function Ri(){J=document.getElementById(`ped-table-body`),wi=document.getElementById(`ped-search`),Ti=document.getElementById(`ped-filter-status`),Ei=document.getElementById(`ped-new-btn`),Y=document.getElementById(`ped-modal`),Di=document.getElementById(`ped-modal-close`),Oi=document.getElementById(`ped-modal-backdrop`),ki=document.getElementById(`ped-form`),Ai=document.getElementById(`ped-save-btn`),ji=document.getElementById(`ped-id`),Mi=document.getElementById(`ped-producto`),Ni=document.getElementById(`ped-categoria`),Pi=document.getElementById(`ped-proveedor`),Fi=document.getElementById(`ped-costo`),Ii=document.getElementById(`ped-precio`)}async function zi(){try{J&&(J.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando reventas...</td></tr>`),q=await Oe()}catch(e){h(`Error cargando reventas: `+e.message,`error`),q=[]}}function Bi(e){if(!J)return;if(e.length===0){J.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron reventas</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;J.innerHTML=e.map(e=>{let n=Number(e.costo||0),r=Number(e.precio||0),i=Number(e.utilidad||r-n),a=e=>new Intl.NumberFormat(`es-CO`).format(e);return`
    ...
    `}).join(``)}function Vi(){let e=()=>{if(!wi)return;let e=wi.value.toLowerCase().trim(),t=Ti?Ti.value:``;Bi(q.filter(n=>{let r=(n.producto||``).toLowerCase().includes(e)||(n.id||``).toLowerCase().includes(e),i=t?n.categoria===t:!0;return r&&i}))};wi?.addEventListener(`input`,e),Ti?.addEventListener(`change`,e);let t=e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))};Fi?.addEventListener(`input`,t),Ii?.addEventListener(`input`,t),Ei?.addEventListener(`click`,()=>Hi(null)),Di?.addEventListener(`click`,Ui),Oi?.addEventListener(`click`,Ui),Ai?.addEventListener(`click`,Wi),window.pedDelete=async e=>{if(confirm(`¿Eliminar la reventa ${e}?`))try{let t=await Ae(e);t&&t.success&&(h(`Eliminada`,`success`),await zi(),Bi(q))}catch(e){h(e.message,`error`)}}}function Hi(e){Y&&(ki?.reset(),e&&(ji&&(ji.value=e.id),Mi&&(Mi.value=e.producto||``),Fi&&(Fi.value=e.costo||0),Ii&&(Ii.value=e.precio||0)),Y.classList.remove(`hidden`),Y.classList.add(`flex`))}function Ui(){Y?.classList.add(`hidden`),Y?.classList.remove(`flex`)}async function Wi(){if(!Ci){Ci=!0;try{(await ke({producto:Mi?.value.trim(),categoria:Ni?.value,proveedor:Pi?.value.trim(),costo:parseInt(Fi?.value.replace(/\D/g,``))||0,precio:parseInt(Ii?.value.replace(/\D/g,``))||0})).success&&(h(`Guardado`,`success`),Ui(),await zi(),Bi(q))}catch(e){h(e.message,`error`)}finally{Ci=!1}}}var X=[],Gi=!1,Ki=!1,qi=null,Z={};function Ji(){return async()=>{Gi||=(await Yi(),Qi(),!0),Xi(X)}}async function Yi(){let e=document.getElementById(`tech-grid`);try{e&&(e.innerHTML=`<p class="col-span-full text-center p-10 opacity-50 italic">Cargando servicios...</p>`),X=await je()}catch{h(`Error al cargar datos`,`error`),X=[]}}function Xi(e){let t=document.getElementById(`tech-grid`);if(!t)return;if(e.length===0){t.innerHTML=`<p class="col-span-full text-center p-20 opacity-30 italic text-sm">No hay órdenes de servicio activas</p>`;return}let n=e=>new Intl.NumberFormat(`es-CO`).format(e||0);t.innerHTML=e.map(e=>{let t=(e.precio_final||0)-(e.abono||0);return`
    ...
    `}).join(``)}function Zi(e){return{Ingresado:`bg-slate-100 text-slate-700`,"En Revisión":`bg-blue-100 text-blue-700`,"En Taller":`bg-blue-100 text-blue-700`,Reparado:`bg-green-100 text-green-700`,Entregado:`bg-emerald-600 text-white`,"Sin Arreglo":`bg-red-100 text-red-700`}[e]||`bg-slate-100 text-slate-600`}function Qi(){document.getElementById(`tech-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();Xi(X.filter(e=>e.cliente.toLowerCase().includes(t)||e.id_orden.toLowerCase().includes(t)||e.equipo.toLowerCase().includes(t)))}),document.getElementById(`tech-new-btn`)?.addEventListener(`click`,()=>$i()),document.getElementById(`tech-modal-close`)?.addEventListener(`click`,ea),document.getElementById(`tech-modal-backdrop`)?.addEventListener(`click`,ea),document.getElementById(`tech-form`)?.addEventListener(`submit`,ta),[`recepcion`,`resultado`].forEach(e=>{let t=document.getElementById(`tech-preview-${e}-btn`),n=document.getElementById(`tech-img-${e}`),r=document.getElementById(`tech-preview-${e}-wrap`),i=document.getElementById(`tech-preview-${e}`),a=document.getElementById(`tech-remove-${e}`);t?.addEventListener(`click`,()=>n?.click()),n?.addEventListener(`change`,n=>{let a=n.target.files[0];if(!a)return;let o=new FileReader;o.onload=n=>{Z[e]={file:a,base64:n.target.result.split(`,`)[1],mime:a.type},i.src=n.target.result,t.classList.add(`hidden`),r.classList.remove(`hidden`)},o.readAsDataURL(a)}),a?.addEventListener(`click`,()=>{n.value=``,delete Z[e],i.src=``,r.classList.add(`hidden`),t.classList.remove(`hidden`)})}),window.techEdit=e=>{let t=X.find(t=>t.id_orden===e);t&&$i(t)},window.techPrint=e=>{let t=X.find(t=>t.id_orden===e);if(!t)return;let n=e=>new Intl.NumberFormat(`es-CO`).format(e||0),r=(t.precio_final||0)-(t.abono||0),i=new Date().toLocaleDateString(`es-CO`,{year:`numeric`,month:`2-digit`,day:`2-digit`,hour:`2-digit`,minute:`2-digit`}),a=`
      <html>
      <head>
    ...
    `,o=window.open(``,``,`width=400,height=600`);o?(o.document.open(),o.document.write(a),o.document.close(),o.focus(),setTimeout(()=>{o.print(),o.close()},250)):h(`Por favor permite las ventanas emergentes para imprimir`,`warning`)},window.techDelete=async e=>{if(confirm(`¿Eliminar orden ${e}?`))try{(await Pe(e)).success&&(h(`Orden eliminada`,`success`),await Yi(),Xi(X))}catch(e){h(e.message,`error`)}}}function $i(e=null){if(qi=e?e.id_orden:null,document.getElementById(`tech-form`).reset(),document.getElementById(`tech-modal-title`).textContent=e?`Editar Orden`:`Ingreso a Servicio Técnico`,Z={},[`recepcion`,`resultado`].forEach(e=>{document.getElementById(`tech-preview-${e}-btn`)?.classList.remove(`hidden`),document.getElementById(`tech-preview-${e}-wrap`)?.classList.add(`hidden`),document.getElementById(`tech-preview-${e}`).src=``}),e&&(document.getElementById(`tech-cliente`).value=e.cliente,document.getElementById(`tech-equipo`).value=e.equipo,document.getElementById(`tech-falla`).value=e.falla,document.getElementById(`tech-costo`).value=new Intl.NumberFormat(`es-CO`).format(e.precio_final||0),document.getElementById(`tech-estado`).value=e.estado,e.evidencias))try{let t=JSON.parse(e.evidencias);[`recepcion`,`resultado`].forEach(e=>{t[e]&&(document.getElementById(`tech-preview-${e}`).src=t[e],document.getElementById(`tech-preview-${e}-btn`).classList.add(`hidden`),document.getElementById(`tech-preview-${e}-wrap`).classList.remove(`hidden`),Z[e]={url:t[e]})})}catch{}let t=document.getElementById(`tech-modal`);t.classList.remove(`hidden`),t.classList.add(`flex`)}function ea(){let e=document.getElementById(`tech-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)}async function ta(e){if(e.preventDefault(),Ki)return;Ki=!0;let t=document.getElementById(`tech-save-btn`);t.disabled=!0;let n=qi||`ST-${Date.now().toString().slice(-6)}`;t.innerHTML=`<span class="material-symbols-outlined animate-spin">progress_activity</span> Subiendo...`;let r={};for(let e of[`recepcion`,`resultado`])if(Z[e])if(Z[e].base64)try{let t=await ie(Z[e].base64,`${n}_${e}`,Z[e].mime);t&&(r[e]=t)}catch(e){console.error(`Error upload evidencia`,e)}else Z[e].url&&(r[e]=Z[e].url);let i=[n,document.getElementById(`tech-cliente`).value.trim(),`310`,document.getElementById(`tech-equipo`).value.trim(),`S/N`,document.getElementById(`tech-falla`).value.trim(),`0000`,``,0,0,parseInt(document.getElementById(`tech-costo`).value.replace(/\D/g,``))||0,document.getElementById(`tech-estado`).value,JSON.stringify(r)];try{(qi?await Ne(qi,i):await Me(i)).success&&(h(qi?`Actualizado`:`Ingresado`,`success`),ea(),await Yi(),Xi(X))}catch(e){h(e.message,`error`)}finally{Ki=!1,t.disabled=!1,t.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}}var na=[],ra=!1,ia=!1,aa,oa,sa,ca,la,ua,da,fa,Q,pa,ma,ha,ga;function _a(){return async()=>{va(),ra||=(await ya(),xa(),!0),ba(na)}}function va(){aa=document.getElementById(`exp-table-body`),oa=document.getElementById(`exp-search`),sa=document.getElementById(`exp-filter-cat`),ca=document.getElementById(`exp-new-btn`),la=document.getElementById(`exp-modal`),ua=document.getElementById(`exp-modal-close`),da=document.getElementById(`exp-modal-backdrop`),fa=document.getElementById(`exp-form`),Q=document.getElementById(`exp-save-btn`),pa=document.getElementById(`exp-monto`),ma=document.getElementById(`exp-categoria`),ha=document.getElementById(`exp-concepto`),ga=document.getElementById(`exp-responsable`)}async function ya(){try{aa.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando egresos...</td></tr>`,na=await Ie()}catch(e){h(`Error cargando egresos: `+e.message,`error`),na=[]}}function ba(e){if(e.length===0){aa.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron egresos</td></tr>`;return}aa.innerHTML=e.map(e=>`
      <tr class="hover:bg-surface-container-low transition-colors">
        <td class="px-4 py-3">
    ...
    `).join(``)}function xa(){let e=()=>{let e=oa.value.toLowerCase().trim(),t=sa.value;ba(na.filter(n=>{let r=(n.concepto||``).toLowerCase().includes(e)||(n.responsable||``).toLowerCase().includes(e)||(n.id||``).toLowerCase().includes(e),i=t?n.categoria===t:!0;return r&&i}))};oa.addEventListener(`input`,e),sa.addEventListener(`change`,e),pa.addEventListener(`input`,e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))}),ca.addEventListener(`click`,()=>Sa()),ua.addEventListener(`click`,Ca),da.addEventListener(`click`,Ca),Q.addEventListener(`click`,wa)}function Sa(){fa.reset();try{let e=localStorage.getItem(`adminpro_user`);if(e){let t=JSON.parse(e);ga.value=t.nombre||t.email}}catch{ga.value=`Sistema`}la.classList.remove(`hidden`),la.classList.add(`flex`),pa.focus()}function Ca(){la.classList.add(`hidden`),la.classList.remove(`flex`)}async function wa(){if(!fa.checkValidity()){fa.reportValidity();return}if(!ia){ia=!0,Q.textContent=`Registrando...`,Q.disabled=!0;try{let e=await Le({monto:parseInt(pa.value.replace(/\D/g,``))||0,categoria:ma.value,concepto:ha.value.trim(),responsable:ga.value});e&&e.success?(h(`Egreso registrado`,`success`),Ca(),await ya(),ba(na)):h(e?.mensaje||`Error al registrar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{ia=!1,Q.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,Q.disabled=!1}}}var Ta=[],Ea=[];function Da(){return async()=>{await Oa(),Na()}}async function Oa(){try{let e=document.getElementById(`nom-list`);e&&(e.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-sm text-on-surface-variant">Cargando nóminas...</td></tr>`),[Ta,Ea]=await Promise.all([Re(),se()]),ka(Ta),Aa(Ta)}catch{h(`Error al cargar nóminas`,`error`)}}function ka(e){let t=document.getElementById(`nom-list`);if(t){if(!e||e.length===0){t.innerHTML=`
      <tr>
    ...
    `}).join(``),document.querySelectorAll(`.nom-del-btn`).forEach(e=>{e.addEventListener(`click`,async e=>{let t=e.currentTarget.dataset.id;if(confirm(`¿Estás seguro de eliminar este registro de nómina?`))try{await Be(t),h(`Nómina eliminada`,`success`),Oa()}catch{h(`Error al eliminar`,`error`)}})})}}function Aa(e){let t=new Date,n=t.getMonth(),r=t.getFullYear(),i=0,a=0;e.forEach(e=>{let t=new Date(e.fecha);t.getMonth()===n&&t.getFullYear()===r&&e.estado!==`Anulado`&&(i+=parseFloat(e.total_pagar)||0),e.estado===`Pendiente`&&(a+=parseFloat(e.total_pagar)||0)});let o=document.getElementById(`nom-stat-mes`),s=document.getElementById(`nom-stat-pendiente`);o&&(o.textContent=`$`+i.toLocaleString(`es-CO`)),s&&(s.textContent=`$`+a.toLocaleString(`es-CO`))}function ja(){let e=parseFloat(document.getElementById(`nom-base`).value)||0,t=parseFloat(document.getElementById(`nom-bonos`).value)||0,n=parseFloat(document.getElementById(`nom-deduc`).value)||0,r=e+t-n;document.getElementById(`nom-total-calc`).textContent=`$`+r.toLocaleString(`es-CO`)}var Ma=!1;function Na(){if(Ma)return;Ma=!0;let e=document.getElementById(`nom-modal`),t=document.getElementById(`nom-form`);document.getElementById(`nom-new-btn`)?.addEventListener(`click`,()=>{t.reset();let n=document.getElementById(`nom-empleado`);n&&(n.innerHTML=`<option value="">Seleccione empleado...</option>`+Ea.map(e=>`<option value="${e.nombre}">${e.nombre} (${e.rol})</option>`).join(``)),ja(),e.classList.remove(`hidden`),e.classList.add(`flex`)}),document.getElementById(`nom-modal-close`)?.addEventListener(`click`,()=>{e.classList.add(`hidden`),e.classList.remove(`flex`)}),document.getElementById(`nom-modal-cancel`)?.addEventListener(`click`,()=>{e.classList.add(`hidden`),e.classList.remove(`flex`)}),document.getElementById(`nom-base`)?.addEventListener(`input`,ja),document.getElementById(`nom-bonos`)?.addEventListener(`input`,ja),document.getElementById(`nom-deduc`)?.addEventListener(`input`,ja),document.getElementById(`nom-save-btn`)?.addEventListener(`click`,async()=>{if(!t.checkValidity()){t.reportValidity();return}let n=parseFloat(document.getElementById(`nom-base`).value)||0,r=parseFloat(document.getElementById(`nom-bonos`).value)||0,i=parseFloat(document.getElementById(`nom-deduc`).value)||0,a=n+r-i,o={empleado:document.getElementById(`nom-empleado`).value,periodo:document.getElementById(`nom-periodo`).value,salario_base:n,bonificaciones:r,deducciones:i,total_pagar:a,estado:document.getElementById(`nom-estado`).value,notas:document.getElementById(`nom-notas`).value},s=document.getElementById(`nom-save-btn`);s.disabled=!0,s.innerHTML=`<span class="material-symbols-outlined text-[18px] animate-spin">refresh</span> Guardando...`;try{await ze(o),h(`Nómina registrada exitosamente`,`success`),e.classList.add(`hidden`),e.classList.remove(`flex`),Oa()}catch(e){h(`Error al guardar: `+e.message,`error`)}finally{s.disabled=!1,s.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}})}function Pa(){return()=>{Fa(),Ia()}}function Fa(){let e=document.getElementById(`set-avatar`),t=document.getElementById(`set-name`),n=document.getElementById(`set-role`),r=document.getElementById(`set-email`);try{let i=localStorage.getItem(`adminproSession`),a=localStorage.getItem(`adminpro_user`),o=i?JSON.parse(i):a?JSON.parse(a):null;o?(t&&(t.textContent=o.nombre||`Usuario`),n&&(n.textContent=o.rol||`Administrador`),r&&(r.textContent=o.email||`No disponible`),e&&(e.textContent=(o.nombre?o.nombre.charAt(0):`U`).toUpperCase())):console.warn(`No se encontró sesión activa para cargar el perfil.`)}catch(e){console.error(`Error loading profile`,e)}let i=document.getElementById(`set-theme-toggle`);i&&(i.checked=document.documentElement.classList.contains(`dark`))}function Ia(){let e=document.getElementById(`set-logout-btn`);e?.replaceWith(e.cloneNode(!0)),document.getElementById(`set-logout-btn`)?.addEventListener(`click`,()=>{confirm(`¿Estás seguro de que deseas cerrar sesión?`)&&ee()});let t=document.getElementById(`set-theme-toggle`);t?.replaceWith(t.cloneNode(!0)),document.getElementById(`set-theme-toggle`)?.addEventListener(`change`,e=>{e.target.checked?(document.documentElement.classList.add(`dark`),localStorage.setItem(`adminpro_theme`,`dark`)):(document.documentElement.classList.remove(`dark`),localStorage.setItem(`adminpro_theme`,`light`))})}`serviceWorker`in navigator&&window.addEventListener(`load`,()=>{navigator.serviceWorker.register(`/adminpro/sw.js`,{scope:`/adminpro/`}).catch(e=>{console.log(`ServiceWorker registration failed: `,e)})});var La=``,$=``,Ra=null;document.addEventListener(`keydown`,e=>{if(!(e.ctrlKey||e.altKey||e.metaKey))if(e.key===`Enter`){if($.length>=3){let t=$;$=``,clearTimeout(Ra),document.dispatchEvent(new CustomEvent(`barcodeScanned`,{detail:t})),(document.activeElement?document.activeElement.tagName:``)!==`TEXTAREA`&&e.preventDefault();return}$=``}else e.key.length===1&&($+=e.key,clearTimeout(Ra),Ra=setTimeout(()=>{$=``},50))});function za(){try{let e=JSON.parse(localStorage.getItem(`adminproSession`)||`null`);return e&&Date.now()<e.expiresAt?e:null}catch{return null}}function Ba(e,t){d(t),localStorage.setItem(`adminproSession`,JSON.stringify({...e,token:t,expiresAt:Date.now()+480*60*1e3}))}function Va(){d(null),localStorage.removeItem(`adminproSession`),localStorage.removeItem(`adminpro_user`)}var Ha=[{label:`Inicio`,items:[{id:`dashboard`,label:`Dashboard`,icon:`dashboard`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]}]},{label:`Operaciones`,items:[{id:`pos`,label:`Ventas (POS)`,icon:`point_of_sale`,roles:[`Administrador`,`Vendedor`]},{id:`sales-history`,label:`Historial de Ventas`,icon:`history`,roles:[`Administrador`,`Vendedor`]},{id:`credits`,label:`Créditos`,icon:`credit_score`,roles:[`Administrador`,`Vendedor`]},{id:`expenses`,label:`Egresos`,icon:`payments`,roles:[`Administrador`]},{id:`nominas`,label:`Nóminas`,icon:`request_quote`,roles:[`Administrador`]}]},{label:`Inventario`,items:[{id:`inventory`,label:`Catálogo General`,icon:`inventory_2`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]},{id:`imei`,label:`Equipos IMEI`,icon:`phone_android`,roles:[`Administrador`,`Vendedor`]},{id:`reventas`,label:`Reventas`,icon:`storefront`,roles:[`Administrador`,`Vendedor`]}]},{label:`Servicios`,items:[{id:`technical`,label:`Servicio Técnico`,icon:`build`,roles:[`Administrador`,`Técnico de reparación`]}]},{label:`Organización`,items:[{id:`tasks`,label:`Lista de Tareas`,icon:`check_circle`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]},{id:`calendar`,label:`Actividad`,icon:`history_toggle_off`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]}]},{label:`Personas`,items:[{id:`clients`,label:`Clientes`,icon:`people`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]},{id:`users`,label:`Equipo / Usuarios`,icon:`manage_accounts`,roles:[`Administrador`]}]},{label:`Otros`,items:[{id:`settings`,label:`Ajustes`,icon:`settings`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]}]}];function Ua(e){let t=document.getElementById(`mobile-drawer`),n=document.getElementById(`mobile-drawer-backdrop`),r=document.getElementById(`mobile-drawer-content`);e?(t.classList.remove(`hidden`),setTimeout(()=>{n.classList.replace(`opacity-0`,`opacity-100`),r.classList.replace(`translate-y-full`,`translate-y-0`)},10)):(n.classList.replace(`opacity-100`,`opacity-0`),r.classList.replace(`translate-y-0`,`translate-y-full`),setTimeout(()=>t.classList.add(`hidden`),300))}function Wa(e,t,n=!1){let r=document.getElementById(e);if(!r)return;let i=t||`Vendedor`,a=``;if(n){let e=[];e=i===`Técnico de reparación`?[{id:`dashboard`,label:`Home`,icon:`dashboard`},{id:`technical`,label:`Reparar`,icon:`build`},{id:`inventory`,label:`Stock`,icon:`inventory_2`},{id:`tasks`,label:`Tareas`,icon:`check_circle`}]:[{id:`dashboard`,label:`Home`,icon:`dashboard`},{id:`pos`,label:`Venta`,icon:`point_of_sale`},{id:`inventory`,label:`Stock`,icon:`inventory_2`},{id:`tasks`,label:`Tareas`,icon:`check_circle`}],a=e.map(e=>`
    ...
    `;let t=document.getElementById(`mobile-drawer-grid`);t&&(t.innerHTML=Ha.flatMap(e=>e.items).filter(e=>!e.roles||e.roles.includes(i)).map(e=>`
      ...
      `).join(``))}else Ha.forEach(e=>{let t=e.items.filter(e=>!e.roles||e.roles.includes(i));t.length!==0&&(a+=`<p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500 mt-6 mb-2 px-4 italic">${e.label}</p>`,t.forEach(e=>{a+=`
        ...
        `}))});r.innerHTML=a,n&&(document.getElementById(`mobile-more-btn`)?.addEventListener(`click`,()=>Ua(!0)),document.getElementById(`mobile-drawer-close`)?.addEventListener(`click`,()=>Ua(!1)),document.getElementById(`mobile-drawer-backdrop`)?.addEventListener(`click`,()=>Ua(!1)),document.querySelectorAll(`#mobile-drawer-grid [data-nav]`).forEach(e=>{e.addEventListener(`click`,()=>{Ua(!1),o(e.dataset.nav)})})),r.querySelectorAll(`[data-nav]`).forEach(e=>{e.addEventListener(`click`,()=>o(e.dataset.nav))})}function Ga(e){document.querySelectorAll(`#desktop-nav [data-nav]`).forEach(t=>{let n=t.dataset.nav===e;t.classList.toggle(`bg-primary`,n),t.classList.toggle(`text-white`,n),t.classList.toggle(`shadow-lg`,n),t.classList.toggle(`text-slate-400`,!n)}),document.querySelectorAll(`#mobile-nav [data-nav]`).forEach(t=>{let n=t.dataset.nav===e;t.classList.toggle(`text-primary`,n),t.classList.toggle(`text-on-surface-variant`,!n)});let t=`AdminPro`;Ha.forEach(n=>{let r=n.items.find(t=>t.id===e);r&&(t=r.label)});let n=document.getElementById(`header-title`);n&&(n.textContent=t)}function Ka(e,t){document.getElementById(`login-screen`).classList.add(`hidden`),document.getElementById(`app-shell`).classList.remove(`hidden`);let n=document.getElementById(`user-name`);n&&(n.textContent=e||`Usuario`),Wa(`desktop-nav`,t,!1),Wa(`mobile-nav`,t,!0);let s=t||`Vendedor`;a(e=>{let t=!1,n=!1;if(Ha.forEach(r=>{let i=r.items.find(t=>t.id===e);i&&(n=!0,(!i.roles||i.roles.includes(s))&&(t=!0))}),n&&!t)return h(`No tienes permiso para acceder a este módulo`,`warning`),`dashboard`}),document.querySelectorAll(`[data-goto]`).forEach(e=>{let t=e.dataset.goto,n=!1;Ha.forEach(e=>{let r=e.items.find(e=>e.id===t);r&&(!r.roles||r.roles.includes(s))&&(n=!0)}),n?e.classList.remove(`hidden`):e.classList.add(`hidden`)}),i(Ga),r(`inventory`,Mt()),r(`dashboard`,Ft()),r(`pos`,An()),r(`imei`,rr()),r(`clients`,Dr()),r(`credits`,Lr()),r(`sales-history`,qr()),r(`tasks`,Qr()),r(`calendar`,li()),r(`users`,hi()),r(`reventas`,Li()),r(`technical`,Ji()),r(`expenses`,_a()),r(`nominas`,Da()),r(`settings`,Pa()),o(`dashboard`)}function qa(e){let t=document.getElementById(`step-credentials`),n=document.getElementById(`step-pin`);t&&t.classList.toggle(`hidden`,e!==`credentials`),n&&n.classList.toggle(`hidden`,e!==`pin`)}function Ja(){document.getElementById(`app-shell`).classList.add(`hidden`),document.getElementById(`login-screen`).classList.remove(`hidden`),qa(`credentials`)}async function Ya(){await ee(),Va(),Ja()}window.addEventListener(`session-expired`,()=>{Va(),Ja()}),document.getElementById(`login-form`)?.addEventListener(`submit`,Xa),document.getElementById(`pin-form`)?.addEventListener(`submit`,Za),document.getElementById(`back-to-login`)?.addEventListener(`click`,()=>qa(`credentials`)),document.getElementById(`logout-btn`)?.addEventListener(`click`,Ya);async function Xa(e){e.preventDefault();let t=document.getElementById(`login-btn`),n=document.getElementById(`login-email`).value.trim(),r=document.getElementById(`login-pwd`).value.trim();t.disabled=!0,t.textContent=`Verificando...`;try{let e=await ae(n,r);e.success&&e.step===`pin`?(La=n,document.getElementById(`pin-hint`).textContent=`Enviamos un PIN a ${n}`,qa(`pin`),document.getElementById(`login-pin`).focus()):h(e.mensaje||`Credenciales incorrectas`,`error`)}catch{h(`Error de conexión`,`error`)}finally{t.disabled=!1,t.textContent=`Ingresar`}}async function Za(e){e.preventDefault();let t=document.getElementById(`pin-btn`),n=document.getElementById(`login-pin`).value.trim();t.disabled=!0,t.textContent=`Verificando...`;try{let e=await oe(La,n);e.success&&e.token?(Ba({email:e.email,nombre:e.nombre,rol:e.rol},e.token),Ka(e.nombre,e.rol)):h(e.mensaje||`PIN incorrecto`,`error`)}catch{h(`Error`,`error`)}finally{t.disabled=!1,t.textContent=`Verificar`}}var Qa=za();Qa&&Qa.token&&f()?(d(Qa.token),Ka(Qa.nombre,Qa.rol)):(Va(),Ja());