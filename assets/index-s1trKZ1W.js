(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e={},t=null,n=null;function r(t,n){e[t]=n}function i(e){t=e}function a(e){n=e}async function o(r){if(n){let e=n(r);if(e===!1)return;typeof e==`string`&&(r=e)}document.querySelectorAll(`[data-view]`).forEach(e=>{e.classList.add(`hidden`)});let i=document.querySelector(`[data-view="${r}"]`);if(!i)return console.warn(`View not found:`,r);i.classList.remove(`hidden`),t&&t(r),e[r]&&await e[r]()}var s=`https://adminpro-adminpro.aws-us-west-2.turso.io/v2/pipeline`,c=`eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3NzcyOTIwOTMsImlkIjoiMDE5ZGNlZGItOTIwMS03NGVkLWIwZGYtZjg4MTQ3NjlhODcxIiwicmlkIjoiNWIwMWViNTctYTgxYS00OTI0LWIzMDUtZjk1Y2EwMjUzNmRkIn0.oruUZmv_ZLWlKA2ctQnghAD5PIiJSIeR4nzbZia-q-f1r12IHhLv1hDw9CsReABIceaVRHPS52JMZ4j3lcZ1Bw`,l=`https://script.google.com/macros/s/AKfycbxxs-PUyNqqALN2-azEHaViv5PM1r5oteanvr2Sfydic-bBQJrGWj00R0FO7UAlP4Ug/exec`,u=localStorage.getItem(`adminpro_gas_token`)||``,d=e=>{u=e,localStorage.setItem(`adminpro_gas_token`,e)},f=()=>u,ee=()=>{localStorage.clear(),location.reload()};async function p(e){let t=Array.isArray(e)?e.map(e=>typeof e==`string`?{type:`execute`,stmt:{sql:e}}:e.type?e:{type:`execute`,stmt:e}):[{type:`execute`,stmt:typeof e==`string`?{sql:e}:e}],n=await(await fetch(s,{method:`POST`,headers:{Authorization:`Bearer ${c}`,"Content-Type":`application/json`},body:JSON.stringify({requests:t})})).json();if(n.error)throw Error(n.error.message);return(n.results||[]).map(e=>{if(!e.response||!e.response.result)return[];let{cols:t,rows:n}=e.response.result;return n.map(e=>{let n={};return t.forEach((t,r)=>{n[t.name]=e[r].value}),n})})}var m=e=>e.map(e=>({type:typeof e==`number`?`float`:`text`,value:String(e)}));async function te(e){return await(await fetch(l,{method:`POST`,mode:`cors`,body:JSON.stringify(e)})).json()}var ne=async(e,t,n)=>(await te({action:`uploadFoto`,token:u,base64Data:e,fileName:t,mimeType:n})).url||``,re=async(e,t)=>(await te({action:`uploadSignature`,token:u,base64Data:e,fileName:t})).url||``,ie=async(e,t,n)=>(await te({action:`uploadEvidencia`,token:u,base64Data:e,fileName:t,mimeType:n})).url||``,ae=async(e,t)=>{let n=await p({sql:`SELECT * FROM usuarios WHERE email = ? AND password = ? AND estado = 'Activo'`,args:[{type:`text`,value:e.toLowerCase()},{type:`text`,value:t}]});return!n[0]||n[0].length===0?{success:!1,mensaje:`Credenciales incorrectas`}:await(await fetch(`${l}?action=login&email=${encodeURIComponent(e)}&password=${t}`)).json()},oe=async(e,t)=>{let n=await(await fetch(`${l}?action=verifyPin&email=${encodeURIComponent(e)}&pin=${t}`)).json();if(n.success){d(n.token);let t={...(await p({sql:`SELECT nombre, rol, email FROM usuarios WHERE email = ?`,args:[{type:`text`,value:e.toLowerCase()}]}))[0]?.[0]||{nombre:`Usuario`},token:n.token};return localStorage.setItem(`adminpro_user`,JSON.stringify(t)),{success:!0,...t}}return n},se=async()=>(await p(`SELECT * FROM usuarios ORDER BY nombre ASC`))[0]||[],ce=e=>p({sql:`INSERT INTO usuarios (email, password, nombre, rol, estado) VALUES (?,?,?,?,?)`,args:m(e)}),le=(e,t,n)=>p({sql:`UPDATE usuarios SET email=?, password=?, nombre=?, rol=?, estado=? WHERE email=?`,args:[{type:`text`,value:t},...m(n),{type:`text`,value:e}]}),ue=e=>p({sql:`DELETE FROM usuarios WHERE email = ?`,args:[{type:`text`,value:e}]}),de=async()=>((await p(`SELECT * FROM clientes ORDER BY nombre ASC`))[0]||[]).map(e=>({...e,cedula:e.id})),fe=e=>p({sql:`INSERT INTO clientes VALUES (?,?,?,?,?,?,?)`,args:m([e.cedula,e.nombre,e.telefono,e.direccion,e.email,e.tipo,new Date().toISOString()])}),pe=(e,t)=>p({sql:`UPDATE clientes SET id=?, nombre=?, telefono=?, direccion=?, email=?, tipo=? WHERE id=?`,args:[...m([t.cedula,t.nombre,t.telefono,t.direccion,t.email,t.tipo]),{type:`text`,value:e}]}),me=e=>p({sql:`DELETE FROM clientes WHERE id = ?`,args:[{type:`text`,value:e}]}),he=async()=>(await p(`SELECT * FROM inventario ORDER BY id DESC`))[0].map(e=>({...e,stockActual:e.stock_actual,stockMinimo:e.stock_minimo,precioVenta:e.precio_venta,costo:e.costo})),ge=e=>p({sql:`INSERT INTO inventario VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),_e=(e,t)=>p({sql:`UPDATE inventario SET id=?, nombre=?, marca=?, categoria=?, tipo=?, costo=?, precio_venta=?, stock_minimo=?, stock_actual=?, ubicacion=?, sku=?, imagen=? WHERE id=?`,args:[...m(t),{type:`text`,value:e}]}),ve=e=>p({sql:`DELETE FROM inventario WHERE id = ?`,args:[{type:`text`,value:e}]}),ye=async()=>(await p(`SELECT * FROM equipos`))[0]||[],be=e=>p({sql:`INSERT INTO equipos VALUES (?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),xe=(e,t)=>p({sql:`UPDATE equipos SET imei1=?, imei2=?, id_producto=?, marca=?, nombre=?, proveedor=?, costo=?, venta=?, estado=?, fecha_ingreso=? WHERE imei1=?`,args:[...m(t),{type:`text`,value:e}]}),Se=e=>p({sql:`DELETE FROM equipos WHERE imei1 = ?`,args:[{type:`text`,value:e}]}),Ce=async()=>(await p(`SELECT * FROM ventas ORDER BY fecha DESC`))[0]||[],we=async e=>{let t=`FAC-${Date.now()}`,n=new Date().toISOString();return await p([{sql:`INSERT INTO ventas VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:[{type:`text`,value:t},{type:`text`,value:n},{type:`text`,value:e.cedula||``},{type:`text`,value:e.cliente||``},{type:`text`,value:e.direccion||``},{type:`text`,value:e.productoNombre||``},{type:`text`,value:String(e.total||0)},{type:`text`,value:`1`},{type:`text`,value:e.imei||`N/A`},{type:`float`,value:e.subtotal||0},{type:`float`,value:e.descuento||0},{type:`float`,value:e.total||0},{type:`text`,value:e.metodo||``},{type:`text`,value:e.vendedor||``},{type:`text`,value:``},{type:`text`,value:e.firmaComprador||``},{type:`text`,value:e.evidencia||``}]},{sql:`UPDATE inventario SET stock_actual = stock_actual - 1 WHERE id = ?`,args:[{type:`text`,value:e.productoId||``}]}]),{success:!0,idFactura:t}},Te=async()=>(await p(`SELECT * FROM creditos`))[0].map(e=>({...e,id:e.id_credito,idFactura:e.id_factura_ref,abonado:e.total_abonado,saldo:e.saldo_pendiente,total:e.valor_total})),Ee=e=>p({sql:`INSERT INTO creditos (id_credito, cliente, telefono, id_factura_ref, fecha_deuda, tipo, valor_total, total_abonado, saldo_pendiente, estado, detalle) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,args:m([Date.now().toString(),e.cliente,e.telefono,e.idFactura,new Date().toISOString(),e.tipo||`Crédito`,e.total,0,e.total,`Activo`,e.detalle])}),De=(e,t)=>p({sql:`UPDATE creditos SET id_credito=?, cliente=?, telefono=?, id_factura_ref=?, fecha_deuda=?, tipo=?, valor_total=?, total_abonado=?, saldo_pendiente=?, estado=?, fecha_cancelacion=?, detalle=?, historial_abonos=? WHERE id_credito=?`,args:[...m(t),{type:`text`,value:e}]}),Oe=async()=>(await p(`SELECT * FROM reventas ORDER BY fecha DESC`))[0].map(e=>({...e,id:e.id_reventa,producto:e.producto,costo:e.costo_proveedor,precio:e.precio_venta,utilidad:e.utilidad})),ke=e=>p({sql:`INSERT INTO reventas VALUES (?,?,?,?,?,?,?,?)`,args:m([`REV-${Date.now()}`,new Date().toISOString(),e.producto,e.categoria,e.costo,e.precio,e.proveedor,e.precio-e.costo])}),Ae=e=>p({sql:`DELETE FROM reventas WHERE id_reventa = ?`,args:[{type:`text`,value:e}]}),je=async()=>(await p(`SELECT * FROM servicio_tecnico ORDER BY id_orden DESC`))[0]||[],Me=e=>p({sql:`INSERT INTO servicio_tecnico VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),Ne=(e,t)=>p({sql:`UPDATE servicio_tecnico SET id_orden=?, cliente=?, telefono=?, equipo=?, imei_serie=?, falla=?, clave_patron=?, repuestos=?, costo_taller=?, abono=?, precio_final=?, estado=?, evidencias=? WHERE id_orden=?`,args:[...m(t),{type:`text`,value:e}]}),Pe=e=>p({sql:`DELETE FROM servicio_tecnico WHERE id_orden = ?`,args:[{type:`text`,value:e}]}),Fe=async()=>{let e=await p([`SELECT COUNT(*) as c FROM inventario`,`SELECT COUNT(*) as c FROM clientes`,`SELECT SUM(total) as s FROM ventas WHERE date(fecha) = date('now')`,`SELECT SUM(monto) as s FROM egresos WHERE date(fecha) = date('now')`,`SELECT SUM(stock_actual) as s FROM inventario`,`SELECT COUNT(*) as c FROM inventario WHERE stock_actual <= 1`,`SELECT * FROM ventas ORDER BY fecha DESC LIMIT 8`,`SELECT date(fecha) as d, SUM(total) as m FROM ventas WHERE date(fecha) >= date('now','-7 days') GROUP BY d`,`SELECT COUNT(*) as c FROM equipos`,`SELECT productos, COUNT(*) as qty FROM ventas GROUP BY productos ORDER BY qty DESC LIMIT 5`,`SELECT id, nombre, stock_actual, stock_minimo FROM inventario WHERE stock_actual <= 1 LIMIT 5`,`SELECT id_orden, cliente, equipo, estado FROM servicio_tecnico ORDER BY id_orden DESC LIMIT 5`,`SELECT COUNT(*) as c FROM creditos WHERE estado != 'Pagado' AND estado != 'Cancelado'`,`SELECT COUNT(*) as c FROM reventas`]),t=[],n=[];for(let r=6;r>=0;r--){let i=new Date;i.setDate(i.getDate()-r);let a=i.toISOString().slice(0,10);t.push(i.toLocaleDateString(`es-CO`,{weekday:`short`,day:`numeric`}));let o=(e[7]||[]).find(e=>e.d===a);n.push(o?o.m:0)}return{ingresosHoy:e[2]?.[0]?.s||0,egresosHoy:e[3]?.[0]?.s||0,utilidad:(e[2]?.[0]?.s||0)-(e[3]?.[0]?.s||0),totalProductos:e[0]?.[0]?.c||0,totalClientes:e[1]?.[0]?.c||0,totalStock:e[4]?.[0]?.s||0,stockCritico:e[5]?.[0]?.c||0,totalEquipos:e[8]?.[0]?.c||0,ventasRecientes:e[6]||[],topProductos:(e[9]||[]).map(e=>({nombre:e.productos,cantidad:e.qty})),productosBajoStock:(e[10]||[]).map(e=>({...e,stockActual:e.stock_actual})),tecRecientes:e[11]||[],creditosActivos:e[12]?.[0]?.c||0,totalReventas:e[13]?.[0]?.c||0,labels7d:t,ventas7d:n}},Ie=async()=>((await p(`SELECT * FROM egresos ORDER BY fecha DESC`))[0]||[]).map(e=>({...e,id:e.id_gasto})),Le=e=>p({sql:`INSERT INTO egresos VALUES (?,?,?,?,?,?,?)`,args:m([`EGR-${Date.now()}`,new Date().toISOString(),e.categoria,e.concepto,e.responsable,e.monto,``])}),Re=async()=>((await p(`SELECT * FROM nominas ORDER BY fecha DESC`))[0]||[]).map(e=>({...e,id:e.id_nomina})),ze=e=>p({sql:`INSERT INTO nominas VALUES (?,?,?,?,?,?,?,?,?,?)`,args:m([`NOM-${Date.now()}`,e.fecha||new Date().toISOString(),e.empleado,e.periodo,e.salario_base,e.deducciones,e.bonificaciones,e.total_pagar,e.estado||`Pendiente`,e.notas||``])}),Be=e=>p({sql:`DELETE FROM nominas WHERE id_nomina = ?`,args:[{type:`text`,value:e}]}),Ve=async()=>(await p(`SELECT * FROM tareas ORDER BY date(fecha_vencimiento) ASC`))[0]||[],He=e=>p({sql:`INSERT INTO tareas VALUES (?,?,?,?,?,?,?,?,?)`,args:m([`T-${Date.now()}`,e.tarea,e.fecha_inicio,e.fecha_vencimiento,e.prioridad,e.estado||`Pendiente`,e.responsable,e.notas||``,e.color||`#4f46e5`])}),Ue=(e,t)=>p({sql:`UPDATE tareas SET estado = ? WHERE id = ?`,args:[{type:`text`,value:t},{type:`text`,value:e}]}),We=e=>p({sql:`DELETE FROM tareas WHERE id = ?`,args:[{type:`text`,value:e}]});function h(e,t=`success`){let n={success:`bg-green-600`,error:`bg-error`,info:`bg-tertiary`,warning:`bg-amber-500`},r={success:`check_circle`,error:`error`,info:`info`,warning:`warning`},i=document.createElement(`div`);i.className=`fixed bottom-4 right-4 z-[9999] toast-animate`,i.innerHTML=`
  ...
  `).join(``)}function En(){j.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();Tn(t?k.filter(e=>e.nombre.toLowerCase().includes(t)||e.sku.toLowerCase().includes(t)):k)}),document.addEventListener(`barcodeScanned`,e=>{let t=document.querySelector(`[data-view="pos"]`);if(!t||t.classList.contains(`hidden`))return;let n=e.detail,r=k.find(e=>e.sku===n||e.id===n);r?(window.posAddToCart(r.id),h(`✅ ${r.nombre} agregado`,`success`),document.activeElement===j&&(j.value=``,Tn(k),j.blur())):h(`Código ${n} no encontrado`,`warning`)}),document.getElementById(`pos-scan-btn`)?.addEventListener(`click`,()=>{vt({title:`Escanear`,onScan:e=>{j.value=e,j.dispatchEvent(new Event(`input`));let t=k.find(t=>t.sku===e||t.id===e);t?(window.posAddToCart(t.id),h(`✅ ${t.nombre} agregado`,`success`),setTimeout(()=>{j.value===e&&(j.value=``,j.dispatchEvent(new Event(`input`)))},1500)):h(`Código ${e} no encontrado en inventario`,`warning`)}})}),document.getElementById(`pos-reventa-btn`)?.addEventListener(`click`,()=>{window.__posReventaMode=!0,o(`inventory`),setTimeout(()=>{window.inventoryView&&window.inventoryView.openNuevo&&window.inventoryView.openNuevo(!0)},150)}),document.getElementById(`pos-select-client-btn`)?.addEventListener(`click`,()=>{on(e=>{_n.value=e.nombre,gn.value=e.documento})}),document.querySelectorAll(`input[name="pos-billing-type"]`).forEach(e=>{e.addEventListener(`change`,e=>{let t=document.getElementById(`pos-evidencia-container`);e.target.value===`digital`?t.classList.add(`hidden`):t.classList.remove(`hidden`)})}),window.posAddToCart=e=>{let t=k.find(t=>t.id===e);if(!t)return;let n=A.find(t=>t.id===e);if(n){if(n.qty>=t.stockActual)return h(`Sin stock`,`warning`);n.qty++}else A.push({...t,qty:1,precioManual:0});F(),setTimeout(()=>{let e=fn.querySelectorAll(`input[oninput*="posUpdatePrice"]`);e.length>0&&e[e.length-1].focus()},100)},window.posRemoveItem=e=>{A=A.filter(t=>t.id!==e),F()},window.posUpdateQty=(e,t)=>{let n=A.find(t=>t.id===e),r=k.find(t=>t.id===e);n&&(n.qty+=t,n.qty<=0?window.posRemoveItem(e):n.qty>r.stockActual&&(n.qty=r.stockActual),F())},window.posUpdatePrice=(e,t)=>{let n=A.find(t=>t.id===e),r=Number(t.value.replace(/\D/g,``));n&&(n.precioManual=r,t.value=new Intl.NumberFormat(`es-CO`).format(r),Dn())},mn.addEventListener(`input`,F);let e=e=>{if(A.length!==0){if(!_n.value)return h(`Nombre cliente ok?`,`warning`);un=e,An()}};document.getElementById(`pos-pay-btn-venta`)?.addEventListener(`click`,()=>e(`venta`)),document.getElementById(`pos-pay-btn-credito`)?.addEventListener(`click`,()=>e(`credito`)),document.getElementById(`pos-pay-btn-separe`)?.addEventListener(`click`,()=>e(`separe`)),yn.addEventListener(`click`,jn),bn.addEventListener(`click`,jn),M.addEventListener(`click`,Mn)}function F(){if(A.length===0){fn.innerHTML=`
      ...
      </div>`,On(0);return}let e=0;fn.innerHTML=A.map(t=>{let n=t.precioManual||0;return e+=n*t.qty,`
    ...
    </div>`}).join(``),On(e)}function Dn(){let e=0;A.forEach(t=>e+=(t.precioManual||t.precioVenta||0)*t.qty),On(e)}function On(e){let t=parseFloat(mn.value)||0;pn.textContent=`$${new Intl.NumberFormat(`es-CO`).format(e)}`,hn.textContent=`$${new Intl.NumberFormat(`es-CO`).format(Math.max(0,e-t))}`}function kn(){P=N.getContext(`2d`);let e=e=>{let t=N.getBoundingClientRect(),n=(e.touches?e.touches[0].clientX:e.clientX)-t.left,r=(e.touches?e.touches[0].clientY:e.clientY)-t.top;return{x:n*(N.width/t.width),y:r*(N.height/t.height)}},t=!1,n=n=>{t=!0,P.beginPath();let{x:r,y:i}=e(n);P.moveTo(r,i),n.preventDefault()},r=n=>{if(!t)return;let{x:r,y:i}=e(n);P.lineTo(r,i),P.stroke(),n.preventDefault()};N.addEventListener(`mousedown`,n),N.addEventListener(`mousemove`,r),N.addEventListener(`mouseup`,()=>t=!1),N.addEventListener(`touchstart`,n,{passive:!1}),N.addEventListener(`touchmove`,r,{passive:!1}),N.addEventListener(`touchend`,()=>t=!1),N.nextElementSibling.addEventListener(`click`,()=>P.clearRect(0,0,N.width,N.height))}function An(){vn.classList.remove(`hidden`),vn.classList.add(`flex`),N.width=N.offsetWidth,N.height=N.offsetHeight,P.lineWidth=2,P.lineCap=`round`,P.clearRect(0,0,N.width,N.height)}function jn(){vn.classList.add(`hidden`),vn.classList.remove(`flex`)}async function Mn(){if(ln)return;ln=!0;let e=document.querySelector(`input[name="pos-billing-type"]:checked`).value;M.textContent=`Subiendo archivos...`,M.disabled=!0;try{let t=``,n=``,r=document.createElement(`canvas`);if(r.width=N.width,r.height=N.height,N.toDataURL()!==r.toDataURL()){let e=await re(N.toDataURL(`image/png`),`Firma_${Date.now()}.png`);t=typeof e==`string`?e:e?.url||``}if(e===`fisica`)if(xn.files[0]){M.textContent=`Subiendo evidencia...`;let e=xn.files[0],t=await ie(await new Promise(t=>{let n=new FileReader;n.onload=e=>t(e.target.result),n.readAsDataURL(e)}),e.name,e.type);n=typeof t==`string`?t:t?.url||``}else{h(`Por favor sube la foto de la factura física`,`warning`),ln=!1,M.textContent=`Confirmar y Facturar`,M.disabled=!1;return}M.textContent=`Registrando venta...`;let i=Number(pn.textContent.replace(/\D/g,``)),a=Number(hn.textContent.replace(/\D/g,``)),o=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`),s={cedula:gn.value.trim(),cliente:_n.value.trim(),productoNombre:A.map(e=>`${e.nombre} (x${e.qty})`).join(`, `),productoId:A[0]?.id,subtotal:i,descuento:Number(mn.value)||0,total:a,metodo:document.getElementById(`pos-metodo-pago`).value,vendedor:o.nombre||`Vendedor`,firmaComprador:t,evidencia:n,tipoFactura:e},c=await we(s);c.success?(un!==`venta`&&await Ee({cliente:s.cliente,telefono:s.cedula,idFactura:c.idFactura,total:a,detalle:s.productoNombre,tipo:un===`separe`?`Plan Separe`:`Crédito`}),h(`Venta Exitosa`,`success`),A=[],F(),gn.value=``,_n.value=``,jn(),wn().then(()=>Tn(k))):h(`Error al guardar`,`error`)}catch(e){h(e.message,`error`)}finally{ln=!1,M.textContent=`Confirmar y Facturar`,M.disabled=!1}}var I=[],Nn=!1,Pn=!1,Fn,In,Ln,Rn,zn,Bn,Vn,Hn,L,R,Un,z,Wn,Gn,B,V,Kn,qn;function Jn(){return async()=>{Yn(),Nn||=(await Xn(),Qn(),!0),Zn(I)}}function Yn(){Fn=document.getElementById(`imei-table-body`),In=document.getElementById(`imei-search`),Ln=document.getElementById(`imei-filter-status`),Rn=document.getElementById(`imei-new-btn`),zn=document.getElementById(`imei-modal`),Bn=document.getElementById(`imei-modal-close`),Vn=document.getElementById(`imei-modal-backdrop`),Hn=document.getElementById(`imei-form`),L=document.getElementById(`imei-save-btn`),R=document.getElementById(`imei-1`),Un=document.getElementById(`imei-2`),z=document.getElementById(`imei-nombre`),Wn=document.getElementById(`imei-marca`),Gn=document.getElementById(`imei-proveedor`),B=document.getElementById(`imei-costo`),V=document.getElementById(`imei-venta`),Kn=document.getElementById(`imei-estado`),qn=document.getElementById(`imei-original`)}async function Xn(){try{Fn.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando equipos...</td></tr>`,I=await ye();let e=await he();z.innerHTML=`<option value="">Seleccione un equipo...</option>`+e.map(e=>`<option value="${e.nombre}" data-marca="${e.marca}" data-costo="${e.costo}" data-precio="${e.precioVenta}" data-prov="${e.proveedor||``}" data-id="${e.id}">${e.nombre}${e.marca?` (`+e.marca+`)`:``}</option>`).join(``)}catch(e){h(`Error cargando equipos: `+e.message,`error`),I=[]}}function Zn(e){if(e.length===0){Fn.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron equipos</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;Fn.innerHTML=e.map(e=>{e.estado;let n=e.estado===`Disponible`?`bg-green-100 text-green-800`:e.estado===`Vendido`?`bg-red-100 text-red-800`:`bg-yellow-100 text-yellow-800`;return`
    ...
    `}).join(``)}function Qn(){let e=()=>{let e=In.value.toLowerCase().trim(),t=Ln.value;Zn(I.filter(n=>{let r=(n.imei1||``).toLowerCase().includes(e)||(n.imei2||``).toLowerCase().includes(e)||(n.nombre||``).toLowerCase().includes(e),i=t?n.estado===t:!0;return r&&i}))},t=e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))};B.addEventListener(`input`,t),V.addEventListener(`input`,t),z.addEventListener(`change`,e=>{let t=e.target.selectedOptions[0];if(t&&t.value){Wn.value=t.dataset.marca||``,Gn.value=t.dataset.prov||``;let e=parseInt(String(t.dataset.costo||`0`).replace(/\D/g,``))||0,n=parseInt(String(t.dataset.precio||`0`).replace(/\D/g,``))||0;B.value=new Intl.NumberFormat(`es-CO`).format(e),V.value=new Intl.NumberFormat(`es-CO`).format(n)}else Wn.value=``,Gn.value=``,B.value=``,V.value=``}),In.addEventListener(`input`,e),Ln.addEventListener(`change`,e),document.getElementById(`imei-scan-btn`)?.addEventListener(`click`,()=>{vt({title:`Escanear IMEI`,onScan:t=>{In.value=t,e(),h(`IMEI: ${t}`,`info`)}})}),document.getElementById(`imei-scan-1`)?.addEventListener(`click`,()=>{vt({title:`Escanear IMEI 1`,onScan:e=>{R.value=e,h(`IMEI 1: ${e}`,`success`)}})}),document.getElementById(`imei-scan-2`)?.addEventListener(`click`,()=>{vt({title:`Escanear IMEI 2`,onScan:e=>{Un.value=e,h(`IMEI 2: ${e}`,`success`)}})}),Rn.addEventListener(`click`,()=>$n(null)),Bn.addEventListener(`click`,er),Vn.addEventListener(`click`,er),L.addEventListener(`click`,tr),window.imeiEdit=e=>{let t=I.find(t=>t.imei1==e);t&&$n(t)},window.imeiDelete=async t=>{if(confirm(`¿Eliminar el equipo con IMEI ${t}?`))try{h(`Eliminando...`,`info`);let n=await Se(t);n&&n.success?(h(`Equipo eliminado`,`success`),await Xn(),e()):h(n.mensaje||`Error al eliminar`,`error`)}catch(e){h(`Error: `+e.message,`error`)}}}function $n(e){Hn.reset(),e?(qn.value=e?e.imei1:``,R.value=e?e.imei1:``,Un.value=e?e.imei2:``,z.value=e?e.nombre:``,Wn.value=e?e.marca:``,Gn.value=e?e.proveedor:``,B.value=e&&e.costo?new Intl.NumberFormat(`es-CO`).format(parseInt(String(e.costo).replace(/\D/g,``))||0):``,V.value=e&&e.venta?new Intl.NumberFormat(`es-CO`).format(parseInt(String(e.venta).replace(/\D/g,``))||0):``,Kn.value=e?e.estado:`Disponible`,document.getElementById(`imei-modal-title`).textContent=`Editar Equipo`):(qn.value=``,Kn.value=`Disponible`,document.getElementById(`imei-modal-title`).textContent=`Registrar Equipo`),zn.classList.remove(`hidden`),zn.classList.add(`flex`)}function er(){zn.classList.add(`hidden`),zn.classList.remove(`flex`)}async function tr(){if(!R.value.trim()){h(`El IMEI Principal es obligatorio`,`warning`),R.focus();return}if(!z.value){h(`Debe seleccionar un equipo del inventario`,`warning`),z.focus();return}if(!Pn){Pn=!0,L.textContent=`Guardando...`,L.disabled=!0;try{let e=qn.value,t={imei1:R.value.trim(),imei2:Un.value.trim(),nombre:z.value.trim(),marca:Wn.value.trim(),proveedor:Gn.value.trim(),costo:parseInt(B.value.replace(/\D/g,``))||0,venta:parseInt(V.value.replace(/\D/g,``))||0,estado:Kn.value},n;n=e?await xe(e,t):await be(t),n&&n.success?(h(`Equipo guardado`,`success`),er(),await Xn(),Zn(I)):h(n?.mensaje||`Error al guardar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{Pn=!1,L.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,L.disabled=!1}}}var H=[],nr=!1,rr=!1,ir,ar,or,sr,cr,lr,ur,U,dr,fr,pr,mr,hr,gr,_r;function vr(){return async()=>{yr(),nr||=(await br(),Sr(),!0),xr(H)}}function yr(){ir=document.getElementById(`cli-table-body`),ar=document.getElementById(`cli-search`),or=document.getElementById(`cli-new-btn`),sr=document.getElementById(`cli-modal`),cr=document.getElementById(`cli-modal-close`),lr=document.getElementById(`cli-modal-backdrop`),ur=document.getElementById(`cli-form`),U=document.getElementById(`cli-save-btn`),dr=document.getElementById(`cli-doc`),fr=document.getElementById(`cli-nombre`),pr=document.getElementById(`cli-tel`),mr=document.getElementById(`cli-email`),hr=document.getElementById(`cli-dir`),gr=document.getElementById(`cli-tipo`),_r=document.getElementById(`cli-original`)}async function br(){try{ir.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando clientes...</td></tr>`,H=await de()}catch(e){h(`Error cargando clientes: `+e.message,`error`),H=[]}}function xr(e){if(e.length===0){ir.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron clientes</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;ir.innerHTML=e.map(e=>{let n=e.tipo===`VIP`?`bg-amber-100 text-amber-800`:e.tipo===`Empresa`?`bg-blue-100 text-blue-800`:e.tipo===`Mayorista`?`bg-purple-100 text-purple-800`:`bg-surface-container text-on-surface-variant`;return`
      <tr class="hover:bg-surface-container-low transition-colors">
    ...
    `}).join(``)}function Sr(){let e=()=>{let e=ar.value.toLowerCase().trim();xr(H.filter(t=>(t.cedula||``).toLowerCase().includes(e)||(t.nombre||``).toLowerCase().includes(e)||(t.telefono||``).toLowerCase().includes(e)))};ar.addEventListener(`input`,e),or.addEventListener(`click`,()=>Cr(null)),cr.addEventListener(`click`,wr),lr.addEventListener(`click`,wr),U.addEventListener(`click`,Tr),window.cliEdit=e=>{let t=H.find(t=>t.cedula==e);t&&Cr(t)},window.cliDelete=async t=>{if(confirm(`¿Eliminar al cliente con documento ${t}?`))try{h(`Eliminando...`,`info`);let n=await me(t);n&&n.success?(h(`Cliente eliminado`,`success`),await br(),e()):h(n.mensaje||`Error al eliminar`,`error`)}catch(e){h(`Error: `+e.message,`error`)}}}function Cr(e){ur.reset(),e?(_r.value=e.cedula,dr.value=e.cedula,fr.value=e.nombre||``,pr.value=e.telefono||``,mr.value=e.email||``,hr.value=e.direccion||``,gr.value=e.tipo||`Normal`,document.getElementById(`cli-modal-title`).textContent=`Editar Cliente`):(_r.value=``,gr.value=`Normal`,document.getElementById(`cli-modal-title`).textContent=`Nuevo Cliente`),sr.classList.remove(`hidden`),sr.classList.add(`flex`)}function wr(){sr.classList.add(`hidden`),sr.classList.remove(`flex`)}async function Tr(){if(!ur.checkValidity()){ur.reportValidity();return}if(!rr){rr=!0,U.textContent=`Guardando...`,U.disabled=!0;try{let e=_r.value,t={cedula:dr.value.trim(),nombre:fr.value.trim(),telefono:pr.value.trim(),email:mr.value.trim(),direccion:hr.value.trim(),tipo:gr.value},n;n=e?await pe(e,t):await fe(t),n&&n.success?(h(`Cliente guardado`,`success`),wr(),await br(),xr(H)):h(n?.mensaje||`Error al guardar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{rr=!1,U.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,U.disabled=!1}}}var W=[],Er=!1,Dr=!1;function Or(){return async()=>{Er||=(await kr(),Fr(),!0),Pr(W)}}async function kr(){let e=document.getElementById(`cred-table-body`);e&&(e.innerHTML=`<tr><td colspan="9" class="p-6 text-center text-on-surface-variant">Cargando créditos...</td></tr>`);try{W=await Te()}catch(e){h(`Error cargando créditos: `+e.message,`error`),W=[]}}var G=e=>`$`+new Intl.NumberFormat(`es-CO`).format(Math.round(e||0));function Ar(e,t){if(!e)return 0;let n=e=>{if(!e)return null;let t=String(e).split(`/`);return t.length===3?new Date(t[2],t[1]-1,t[0]):new Date(e)},r=n(e),i=t?n(t):new Date;return!r||isNaN(r)?0:Math.max(0,Math.floor((i-r)/864e5))}function jr(e){return e?e.split(`;`).filter(Boolean).map(e=>{let t=e.split(`|`);return{fecha:t[0]||``,monto:parseFloat(t[1])||0,nota:t[2]||``}}):[]}function Mr(e){return e.map(e=>`${e.fecha}|${e.monto}|${e.nota}`).join(`;`)}function Nr(e){let t=0,n=0;e.forEach(e=>{e.estado!==`Cancelado`&&(t+=e.saldo||0),n+=e.abonado||0});let r=document.getElementById(`cred-stat-total`),i=document.getElementById(`cred-stat-recaudo`);r&&(r.textContent=G(t)),i&&(i.textContent=G(n))}function Pr(e){Nr(W);let t=document.getElementById(`cred-table-body`);if(t){if(e.length===0){t.innerHTML=`<tr><td colspan="9" class="p-6 text-center text-on-surface-variant">No se encontraron créditos</td></tr>`;return}t.innerHTML=e.map(e=>{let t=e.estado===`Cancelado`||e.estado===`Entregado`,n=Ar(e.fecha,t?e.fechaCancelacion:null),r=t?`bg-green-100 text-green-800`:e.estado===`En Mora`?`bg-red-100 text-red-800`:`bg-orange-100 text-orange-800`,i=e.tipo===`Plan Separe`?`bg-emerald-100 text-emerald-800 border-emerald-200`:`bg-blue-100 text-blue-800 border-blue-200`,a=t?`<span class="text-[10px] text-on-surface-variant">Pagó en ${n}d</span>`:`<span class="text-[10px] font-bold ${n>30?`text-red-500`:`text-orange-500`}">${n} días</span>`,o=String(e.telefono||``).replace(/\D/g,``),s=`https://wa.me/57${o}?text=${encodeURIComponent(`Hola ${e.cliente}, le recordamos que tiene un saldo pendiente de ${G(e.saldo)} con nosotros. Gracias.`)}`;return`
    ...
    `}).join(``)}}function Fr(){let e=document.getElementById(`cred-search`),t=document.getElementById(`cred-filter-status`),n=document.getElementById(`cred-filter-tipo`),r=()=>{let r=(e?.value||``).toLowerCase().trim(),i=t?.value||``,a=n?.value||``;Pr(W.filter(e=>{let t=(e.cliente||``).toLowerCase().includes(r)||(e.idFactura||``).toLowerCase().includes(r),n=i?e.estado===i:!0,o=a?(e.tipo||`Crédito`)===a:!0;return t&&n&&o}))};e?.addEventListener(`input`,r),t?.addEventListener(`change`,r),n?.addEventListener(`change`,r);let i=e=>{let t=e.target.value.replace(/\D/g,``);e.target.value=t?new Intl.NumberFormat(`es-CO`).format(parseInt(t)):``};document.getElementById(`cred-new-total`)?.addEventListener(`input`,i),document.getElementById(`cred-new-abono`)?.addEventListener(`input`,i),document.getElementById(`cred-monto-abono`)?.addEventListener(`input`,i);let a=document.getElementById(`cred-modal`),o=document.getElementById(`cred-modal-close`),s=document.getElementById(`cred-modal-backdrop`),c=document.getElementById(`cred-save-btn`),l=()=>{a?.classList.add(`hidden`),a?.classList.remove(`flex`)};o?.addEventListener(`click`,l),s?.addEventListener(`click`,l),window.credAddAbono=e=>{let t=W.find(t=>t.id==e);if(!t)return;document.getElementById(`cred-id`).value=t.id,document.getElementById(`cred-cliente-name`).textContent=t.cliente,document.getElementById(`cred-saldo-actual`).textContent=G(t.saldo);let n=jr(t.historialAbonos),r=document.getElementById(`cred-historial`);r&&(r.innerHTML=n.length===0?`<p class="text-xs text-on-surface-variant">Sin abonos anteriores</p>`:n.map(e=>`
            ...
            </div>`).join(``)),document.getElementById(`cred-monto-abono`).value=``,document.getElementById(`cred-nota-abono`).value=``,a?.classList.remove(`hidden`),a?.classList.add(`flex`),document.getElementById(`cred-monto-abono`)?.focus()},c?.addEventListener(`click`,async()=>{let e=document.getElementById(`cred-id`).value,t=parseInt((document.getElementById(`cred-monto-abono`).value||``).replace(/\D/g,``))||0,n=(document.getElementById(`cred-nota-abono`)?.value||``).trim();if(!t||t<=0){h(`Ingresa un monto válido`,`warning`);return}if(!Dr){Dr=!0,c.textContent=`Aplicando...`,c.disabled=!0;try{let r=W.find(t=>t.id==e),i=(r.abonado||0)+t,a=Math.max(0,(r.total||0)-i),o=a<=0,s=jr(r.historialAbonos);s.push({fecha:new Date().toLocaleDateString(`es-CO`),monto:t,nota:n});let c=r.tipo===`Plan Separe`,u=c?`Separado`:`Activo`,d=c?`Entregado`:`Cancelado`,f=await De(e,{...r,abonado:i,saldo:a,estado:o||r.estado===`Cancelado`||r.estado===`Entregado`?d:u,fechaCancelacion:o?new Date().toLocaleDateString(`es-CO`):r.fechaCancelacion||``,historialAbonos:Mr(s)});f?.success?(h(o?`✅ ¡Crédito cancelado!`:`Abono registrado`,`success`),l(),Er=!1,await kr(),Pr(W)):h(f?.mensaje||`Error al guardar`,`error`)}catch(e){h(`Error: `+e.message,`error`)}finally{Dr=!1,c.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Aplicar Abono`,c.disabled=!1}}});let u=document.getElementById(`cred-new-modal`),d=()=>{u?.classList.add(`hidden`),u?.classList.remove(`flex`)};document.getElementById(`cred-new-btn`)?.addEventListener(`click`,()=>{document.getElementById(`cred-new-form`)?.reset(),document.getElementById(`cred-new-cliente`).value=``,document.getElementById(`cred-new-cliente-doc`).value=``,u?.classList.remove(`hidden`),u?.classList.add(`flex`)}),document.getElementById(`cred-new-close`)?.addEventListener(`click`,d),document.getElementById(`cred-new-backdrop`)?.addEventListener(`click`,d),document.getElementById(`cred-select-client-btn`)?.addEventListener(`click`,()=>{on(e=>{document.getElementById(`cred-new-cliente`).value=e.nombre,document.getElementById(`cred-new-cliente-doc`).value=e.cedula||e.documento||e.telefono||``})}),document.getElementById(`cred-save-new-btn`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`cred-new-cliente`).value.trim(),t=document.getElementById(`cred-new-cliente-doc`).value.trim(),n=parseInt((document.getElementById(`cred-new-total`).value||``).replace(/\D/g,``))||0,r=parseInt((document.getElementById(`cred-new-abono`)?.value||``).replace(/\D/g,``))||0,i=document.getElementById(`cred-new-detalle`).value.trim();if(!e||!n){h(`Cliente y monto son requeridos`,`warning`);return}let a=document.getElementById(`cred-save-new-btn`);a.disabled=!0,a.textContent=`Guardando...`;try{let a=await Ee({cliente:e,telefono:t,total:n,detalle:i,historialAbonos:r>0?Mr([{fecha:new Date().toLocaleDateString(`es-CO`),monto:r,nota:`Abono inicial`}]):``});a?.success?(h(`Crédito creado`,`success`),d(),Er=!1,await kr(),Pr(W)):h(a?.mensaje||`Error al crear crédito`,`error`)}catch(e){h(`Error: `+e.message,`error`)}finally{a.disabled=!1,a.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}})}var Ir=[],Lr=[];function Rr(){return async()=>{document.getElementById(`sales-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();Lr=Ir.filter(e=>(e.id_factura||``).toLowerCase().includes(t)||(e.cliente||``).toLowerCase().includes(t)||(e.cedula||``).toLowerCase().includes(t)),Br()});let e=()=>{let e=document.getElementById(`sale-detail-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)};document.getElementById(`sale-detail-close`)?.addEventListener(`click`,e),document.getElementById(`sale-detail-backdrop`)?.addEventListener(`click`,e),await zr()}}async function zr(){let e=document.getElementById(`sales-history-list`);if(e)try{e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-on-surface-variant italic text-sm">Cargando todas las ventas...</td></tr>`,Ir=await Ce(),Lr=[...Ir],Br()}catch(t){e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-error italic text-sm">Error: ${t.message}</td></tr>`}}function Br(){let e=document.getElementById(`sales-history-list`);if(e){if(Lr.length===0){e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-on-surface-variant italic text-sm">No se encontraron ventas</td></tr>`;return}e.innerHTML=Lr.map((e,t)=>{let n=new Date(e.fecha).toLocaleDateString(`es-CO`,{day:`2-digit`,month:`short`}),r=new Intl.NumberFormat(`es-CO`).format(e.total||0);return`
    ...
    `}).join(``)}}window.viewSaleDetail=e=>{let t=Ir.find(t=>t.id_factura===e);if(!t)return;let n=document.getElementById(`sale-detail-modal`),r=document.getElementById(`sale-detail-content`),i=e=>new Intl.NumberFormat(`es-CO`).format(e||0);r.innerHTML=`
    <div class="space-y-6">
  ...
  `,n.classList.remove(`hidden`),n.classList.add(`flex`)};var Vr=[],Hr=!1;function Ur(){return async()=>{Wr(),Hr||=(await Gr(),!0)}}function Wr(){let e=document.getElementById(`task-new-btn`),t=document.getElementById(`task-modal-close`),n=document.getElementById(`task-modal-backdrop`),r=document.getElementById(`task-form`);e?.replaceWith(e.cloneNode(!0)),t?.replaceWith(t.cloneNode(!0)),n?.replaceWith(n.cloneNode(!0)),r?.replaceWith(r.cloneNode(!0)),document.getElementById(`task-new-btn`)?.addEventListener(`click`,Yr),document.getElementById(`task-modal-close`)?.addEventListener(`click`,Xr),document.getElementById(`task-modal-backdrop`)?.addEventListener(`click`,Xr),document.getElementById(`task-form`)?.addEventListener(`submit`,Zr)}async function Gr(){let e=document.getElementById(`task-list`);try{let e=await Ve(),t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`);Vr=t.rol===`Técnico de reparación`||t.rol===`Vendedor`?e.filter(e=>e.responsable===t.nombre):e,Kr()}catch(t){e.innerHTML=`<li class="p-8 text-center text-error">Error: ${t.message}</li>`}}function Kr(){let e=document.getElementById(`task-list`);if(!Vr||Vr.length===0){e.innerHTML=`<li class="p-12 text-center text-on-surface-variant italic text-sm">No hay tareas. ¡Buen trabajo!</li>`;return}e.innerHTML=Vr.map(e=>{let t=e.estado===`Completada`;return`
      <li class="flex items-center gap-4 p-4 hover:bg-surface-container-low transition-colors group">
        ...
        <div class="text-right">
          <span class="text-[9px] font-black px-2 py-0.5 rounded-full ${qr(e.prioridad)}">${e.prioridad}</span>
          <p class="text-[10px] text-on-surface-variant mt-1">${Jr(e.fecha_vencimiento)}</p>
        </div>
    ...
    `}),e.innerHTML=r}var q=[],ri=!1,ii=!1,ai=null;function oi(){return async()=>{ri||=(await si(),li(),!0),ci(q)}}async function si(){let e=document.getElementById(`user-table-body`);try{e&&(e.innerHTML=`<tr><td colspan="5" class="p-8 text-center opacity-50">Cargando equipo...</td></tr>`),q=await se()}catch{h(`Error al cargar usuarios`,`error`),q=[]}}function ci(e){let t=document.getElementById(`user-table-body`);if(t){if(e.length===0){t.innerHTML=`<tr><td colspan="5" class="p-10 text-center opacity-40 italic">No hay usuarios registrados</td></tr>`;return}t.innerHTML=e.map(e=>`
  ...
  `).join(``)}}function li(){document.getElementById(`user-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();ci(q.filter(e=>e.nombre.toLowerCase().includes(t)||e.email.toLowerCase().includes(t)))}),document.getElementById(`user-new-btn`)?.addEventListener(`click`,()=>ui()),document.getElementById(`user-modal-close`)?.addEventListener(`click`,di),document.getElementById(`user-modal-backdrop`)?.addEventListener(`click`,di),document.getElementById(`user-form`)?.addEventListener(`submit`,fi),window.userEdit=e=>{let t=q.find(t=>t.email===e);t&&ui(t)},window.userDelete=async e=>{if(e===JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).email)return h(`No puedes eliminarte a ti mismo`,`warning`);if(confirm(`¿Eliminar al usuario ${e}?`))try{await ue(e),h(`Usuario eliminado`,`success`),await si(),ci(q)}catch(e){h(e.message,`error`)}}}function ui(e=null){ai=e?e.email:null,document.getElementById(`user-form`).reset(),document.getElementById(`user-modal-title`).textContent=e?`Editar Usuario`:`Nuevo Usuario`,e&&(document.getElementById(`user-input-name`).value=e.nombre,document.getElementById(`user-input-email`).value=e.email,document.getElementById(`user-input-password`).value=e.password,document.getElementById(`user-input-rol`).value=e.rol,document.getElementById(`user-input-estado`).value=e.estado);let t=document.getElementById(`user-modal`);t.classList.remove(`hidden`),t.classList.add(`flex`)}function di(){let e=document.getElementById(`user-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)}async function fi(e){if(e.preventDefault(),ii)return;ii=!0;let t=document.getElementById(`user-save-btn`);t.disabled=!0,t.innerHTML=`Guardando...`;let n=e=>document.getElementById(e).value.trim(),r=n(`user-input-email`).toLowerCase(),i=[r,n(`user-input-password`),n(`user-input-name`),n(`user-input-rol`),n(`user-input-estado`)];try{ai?await le(ai,r,[i[1],i[2],i[3],i[4]]):await ce(i),h(ai?`Actualizado`:`Creado`,`success`),di(),await si(),ci(q)}catch(e){h(e.message,`error`)}finally{ii=!1,t.disabled=!1,t.innerHTML=`Guardar Usuario`}}var J=[],pi=!1,mi=!1,Y,hi,gi,_i,X,vi,yi,bi,xi,Si,Ci,wi,Ti,Ei,Di;function Oi(){return async()=>{ki(),pi||=(await Ai(),Mi(),!0),ji(J)}}function ki(){Y=document.getElementById(`ped-table-body`),hi=document.getElementById(`ped-search`),gi=document.getElementById(`ped-filter-status`),_i=document.getElementById(`ped-new-btn`),X=document.getElementById(`ped-modal`),vi=document.getElementById(`ped-modal-close`),yi=document.getElementById(`ped-modal-backdrop`),bi=document.getElementById(`ped-form`),xi=document.getElementById(`ped-save-btn`),Si=document.getElementById(`ped-id`),Ci=document.getElementById(`ped-producto`),wi=document.getElementById(`ped-categoria`),Ti=document.getElementById(`ped-proveedor`),Ei=document.getElementById(`ped-costo`),Di=document.getElementById(`ped-precio`)}async function Ai(){try{Y&&(Y.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando reventas...</td></tr>`),J=await Oe()}catch(e){h(`Error cargando reventas: `+e.message,`error`),J=[]}}function ji(e){if(!Y)return;if(e.length===0){Y.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron reventas</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;Y.innerHTML=e.map(e=>{let n=Number(e.costo||0),r=Number(e.precio||0),i=Number(e.utilidad||r-n),a=e=>new Intl.NumberFormat(`es-CO`).format(e);return`
    ...
    `}).join(``)}function Mi(){let e=()=>{if(!hi)return;let e=hi.value.toLowerCase().trim(),t=gi?gi.value:``;ji(J.filter(n=>{let r=(n.producto||``).toLowerCase().includes(e)||(n.id||``).toLowerCase().includes(e),i=t?n.categoria===t:!0;return r&&i}))};hi?.addEventListener(`input`,e),gi?.addEventListener(`change`,e);let t=e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))};Ei?.addEventListener(`input`,t),Di?.addEventListener(`input`,t),_i?.addEventListener(`click`,()=>Ni(null)),vi?.addEventListener(`click`,Pi),yi?.addEventListener(`click`,Pi),xi?.addEventListener(`click`,Fi),window.pedDelete=async e=>{if(confirm(`¿Eliminar la reventa ${e}?`))try{let t=await Ae(e);t&&t.success&&(h(`Eliminada`,`success`),await Ai(),ji(J))}catch(e){h(e.message,`error`)}}}function Ni(e){X&&(bi?.reset(),e&&(Si&&(Si.value=e.id),Ci&&(Ci.value=e.producto||``),Ei&&(Ei.value=e.costo||0),Di&&(Di.value=e.precio||0)),X.classList.remove(`hidden`),X.classList.add(`flex`))}function Pi(){X?.classList.add(`hidden`),X?.classList.remove(`flex`)}async function Fi(){if(!mi){mi=!0;try{(await ke({producto:Ci?.value.trim(),categoria:wi?.value,proveedor:Ti?.value.trim(),costo:parseInt(Ei?.value.replace(/\D/g,``))||0,precio:parseInt(Di?.value.replace(/\D/g,``))||0})).success&&(h(`Guardado`,`success`),Pi(),await Ai(),ji(J))}catch(e){h(e.message,`error`)}finally{mi=!1}}}var Z=[],Ii=!1,Li=!1,Ri=null,Q={};function zi(){return async()=>{Ii||=(await Bi(),Ui(),!0),Vi(Z)}}async function Bi(){let e=document.getElementById(`tech-grid`);try{e&&(e.innerHTML=`<p class="col-span-full text-center p-10 opacity-50 italic">Cargando servicios...</p>`),Z=await je()}catch{h(`Error al cargar datos`,`error`),Z=[]}}function Vi(e){let t=document.getElementById(`tech-grid`);if(!t)return;if(e.length===0){t.innerHTML=`<p class="col-span-full text-center p-20 opacity-30 italic text-sm">No hay órdenes de servicio activas</p>`;return}let n=e=>new Intl.NumberFormat(`es-CO`).format(e||0);t.innerHTML=e.map(e=>{let t=(e.precio_final||0)-(e.abono||0);return`
      <div class="bg-surface-container-lowest border border-surface-variant rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group relative">
        <div class="flex justify-between items-start mb-3">
          ...
          <span class="px-2 py-0.5 rounded-full text-[10px] font-bold ${Hi(e.estado)}">${e.estado}</span>
        </div>
    ...
    `}).join(``)}function Hi(e){return{Ingresado:`bg-slate-100 text-slate-700`,"En Revisión":`bg-blue-100 text-blue-700`,"En Taller":`bg-blue-100 text-blue-700`,Reparado:`bg-green-100 text-green-700`,Entregado:`bg-emerald-600 text-white`,"Sin Arreglo":`bg-red-100 text-red-700`}[e]||`bg-slate-100 text-slate-600`}function Ui(){document.getElementById(`tech-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();Vi(Z.filter(e=>e.cliente.toLowerCase().includes(t)||e.id_orden.toLowerCase().includes(t)||e.equipo.toLowerCase().includes(t)))}),document.getElementById(`tech-new-btn`)?.addEventListener(`click`,()=>Wi()),document.getElementById(`tech-modal-close`)?.addEventListener(`click`,Gi),document.getElementById(`tech-modal-backdrop`)?.addEventListener(`click`,Gi),document.getElementById(`tech-form`)?.addEventListener(`submit`,Ki),[`recepcion`,`resultado`].forEach(e=>{let t=document.getElementById(`tech-preview-${e}-btn`),n=document.getElementById(`tech-img-${e}`),r=document.getElementById(`tech-preview-${e}-wrap`),i=document.getElementById(`tech-preview-${e}`),a=document.getElementById(`tech-remove-${e}`);t?.addEventListener(`click`,()=>n?.click()),n?.addEventListener(`change`,n=>{let a=n.target.files[0];if(!a)return;let o=new FileReader;o.onload=n=>{Q[e]={file:a,base64:n.target.result.split(`,`)[1],mime:a.type},i.src=n.target.result,t.classList.add(`hidden`),r.classList.remove(`hidden`)},o.readAsDataURL(a)}),a?.addEventListener(`click`,()=>{n.value=``,delete Q[e],i.src=``,r.classList.add(`hidden`),t.classList.remove(`hidden`)})}),window.techEdit=e=>{let t=Z.find(t=>t.id_orden===e);t&&Wi(t)},window.techPrint=e=>{let t=Z.find(t=>t.id_orden===e);if(!t)return;let n=e=>new Intl.NumberFormat(`es-CO`).format(e||0),r=(t.precio_final||0)-(t.abono||0),i=new Date().toLocaleDateString(`es-CO`,{year:`numeric`,month:`2-digit`,day:`2-digit`,hour:`2-digit`,minute:`2-digit`}),a=`
      <html>
      <head>
    ...
    `,o=window.open(``,``,`width=400,height=600`);o?(o.document.open(),o.document.write(a),o.document.close(),o.focus(),setTimeout(()=>{o.print(),o.close()},250)):h(`Por favor permite las ventanas emergentes para imprimir`,`warning`)},window.techDelete=async e=>{if(confirm(`¿Eliminar orden ${e}?`))try{(await Pe(e)).success&&(h(`Orden eliminada`,`success`),await Bi(),Vi(Z))}catch(e){h(e.message,`error`)}}}function Wi(e=null){if(Ri=e?e.id_orden:null,document.getElementById(`tech-form`).reset(),document.getElementById(`tech-modal-title`).textContent=e?`Editar Orden`:`Ingreso a Servicio Técnico`,Q={},[`recepcion`,`resultado`].forEach(e=>{document.getElementById(`tech-preview-${e}-btn`)?.classList.remove(`hidden`),document.getElementById(`tech-preview-${e}-wrap`)?.classList.add(`hidden`),document.getElementById(`tech-preview-${e}`).src=``}),e&&(document.getElementById(`tech-cliente`).value=e.cliente,document.getElementById(`tech-equipo`).value=e.equipo,document.getElementById(`tech-falla`).value=e.falla,document.getElementById(`tech-costo`).value=new Intl.NumberFormat(`es-CO`).format(e.precio_final||0),document.getElementById(`tech-estado`).value=e.estado,e.evidencias))try{let t=JSON.parse(e.evidencias);[`recepcion`,`resultado`].forEach(e=>{t[e]&&(document.getElementById(`tech-preview-${e}`).src=t[e],document.getElementById(`tech-preview-${e}-btn`).classList.add(`hidden`),document.getElementById(`tech-preview-${e}-wrap`).classList.remove(`hidden`),Q[e]={url:t[e]})})}catch{}let t=document.getElementById(`tech-modal`);t.classList.remove(`hidden`),t.classList.add(`flex`)}function Gi(){let e=document.getElementById(`tech-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)}async function Ki(e){if(e.preventDefault(),Li)return;Li=!0;let t=document.getElementById(`tech-save-btn`);t.disabled=!0;let n=Ri||`ST-${Date.now().toString().slice(-6)}`;t.innerHTML=`<span class="material-symbols-outlined animate-spin">progress_activity</span> Subiendo...`;let r={};for(let e of[`recepcion`,`resultado`])if(Q[e])if(Q[e].base64)try{let t=await ie(Q[e].base64,`${n}_${e}`,Q[e].mime);t&&(r[e]=t)}catch(e){console.error(`Error upload evidencia`,e)}else Q[e].url&&(r[e]=Q[e].url);let i=[n,document.getElementById(`tech-cliente`).value.trim(),`310`,document.getElementById(`tech-equipo`).value.trim(),`S/N`,document.getElementById(`tech-falla`).value.trim(),`0000`,``,0,0,parseInt(document.getElementById(`tech-costo`).value.replace(/\D/g,``))||0,document.getElementById(`tech-estado`).value,JSON.stringify(r)];try{(Ri?await Ne(Ri,i):await Me(i)).success&&(h(Ri?`Actualizado`:`Ingresado`,`success`),Gi(),await Bi(),Vi(Z))}catch(e){h(e.message,`error`)}finally{Li=!1,t.disabled=!1,t.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}}var qi=[],Ji=!1,Yi=!1,Xi,Zi,Qi,$i,ea,ta,na,ra,ia,aa,oa,sa,ca;function la(){return async()=>{ua(),Ji||=(await da(),pa(),!0),fa(qi)}}function ua(){Xi=document.getElementById(`exp-table-body`),Zi=document.getElementById(`exp-search`),Qi=document.getElementById(`exp-filter-cat`),$i=document.getElementById(`exp-new-btn`),ea=document.getElementById(`exp-modal`),ta=document.getElementById(`exp-modal-close`),na=document.getElementById(`exp-modal-backdrop`),ra=document.getElementById(`exp-form`),ia=document.getElementById(`exp-save-btn`),aa=document.getElementById(`exp-monto`),oa=document.getElementById(`exp-categoria`),sa=document.getElementById(`exp-concepto`),ca=document.getElementById(`exp-responsable`)}async function da(){try{Xi.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando egresos...</td></tr>`,qi=await Ie()}catch(e){h(`Error cargando egresos: `+e.message,`error`),qi=[]}}function fa(e){if(e.length===0){Xi.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron egresos</td></tr>`;return}Xi.innerHTML=e.map(e=>`
      <tr class="hover:bg-surface-container-low transition-colors">
        <td class="px-4 py-3">
    ...
    `).join(``)}function pa(){let e=()=>{let e=Zi.value.toLowerCase().trim(),t=Qi.value;fa(qi.filter(n=>{let r=(n.concepto||``).toLowerCase().includes(e)||(n.responsable||``).toLowerCase().includes(e)||(n.id||``).toLowerCase().includes(e),i=t?n.categoria===t:!0;return r&&i}))};Zi.addEventListener(`input`,e),Qi.addEventListener(`change`,e),aa.addEventListener(`input`,e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))}),$i.addEventListener(`click`,()=>ma()),ta.addEventListener(`click`,ha),na.addEventListener(`click`,ha),ia.addEventListener(`click`,ga)}function ma(){ra.reset();try{let e=localStorage.getItem(`adminpro_user`);if(e){let t=JSON.parse(e);ca.value=t.nombre||t.email}}catch{ca.value=`Sistema`}ea.classList.remove(`hidden`),ea.classList.add(`flex`),aa.focus()}function ha(){ea.classList.add(`hidden`),ea.classList.remove(`flex`)}async function ga(){if(!ra.checkValidity()){ra.reportValidity();return}if(!Yi){Yi=!0,ia.textContent=`Registrando...`,ia.disabled=!0;try{let e=await Le({monto:parseInt(aa.value.replace(/\D/g,``))||0,categoria:oa.value,concepto:sa.value.trim(),responsable:ca.value});e&&e.success?(h(`Egreso registrado`,`success`),ha(),await da(),fa(qi)):h(e?.mensaje||`Error al registrar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{Yi=!1,ia.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,ia.disabled=!1}}}var _a=[],va=[];function ya(){return async()=>{await ba(),Ta()}}async function ba(){try{let e=document.getElementById(`nom-list`);e&&(e.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-sm text-on-surface-variant">Cargando nóminas...</td></tr>`),[_a,va]=await Promise.all([Re(),se()]),xa(_a),Sa(_a)}catch{h(`Error al cargar nóminas`,`error`)}}function xa(e){let t=document.getElementById(`nom-list`);if(t){if(!e||e.length===0){t.innerHTML=`
      <tr>
    ...
    `;let t=document.getElementById(`mobile-drawer-grid`);t&&(t.innerHTML=Pa.flatMap(e=>e.items).filter(e=>!e.roles||e.roles.includes(i)).map(e=>`
      ...
      `).join(``))}else Pa.forEach(e=>{let t=e.items.filter(e=>!e.roles||e.roles.includes(i));t.length!==0&&(a+=`<p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500 mt-6 mb-2 px-4 italic">${e.label}</p>`,t.forEach(e=>{a+=`
        ...
        `}))});r.innerHTML=a,n&&(document.getElementById(`mobile-more-btn`)?.addEventListener(`click`,()=>Fa(!0)),document.getElementById(`mobile-drawer-close`)?.addEventListener(`click`,()=>Fa(!1)),document.getElementById(`mobile-drawer-backdrop`)?.addEventListener(`click`,()=>Fa(!1)),document.querySelectorAll(`#mobile-drawer-grid [data-nav]`).forEach(e=>{e.addEventListener(`click`,()=>{Fa(!1),o(e.dataset.nav)})})),r.querySelectorAll(`[data-nav]`).forEach(e=>{e.addEventListener(`click`,()=>o(e.dataset.nav))})}function La(e){document.querySelectorAll(`#desktop-nav [data-nav]`).forEach(t=>{let n=t.dataset.nav===e;t.classList.toggle(`bg-primary`,n),t.classList.toggle(`text-white`,n),t.classList.toggle(`shadow-lg`,n),t.classList.toggle(`text-slate-400`,!n)}),document.querySelectorAll(`#mobile-nav [data-nav]`).forEach(t=>{let n=t.dataset.nav===e;t.classList.toggle(`text-primary`,n),t.classList.toggle(`text-on-surface-variant`,!n)});let t=`AdminPro`;Pa.forEach(n=>{let r=n.items.find(t=>t.id===e);r&&(t=r.label)});let n=document.getElementById(`header-title`);n&&(n.textContent=t)}function Ra(e,t){document.getElementById(`login-screen`).classList.add(`hidden`),document.getElementById(`app-shell`).classList.remove(`hidden`);let n=document.getElementById(`user-name`);n&&(n.textContent=e||`Usuario`),Ia(`desktop-nav`,t,!1),Ia(`mobile-nav`,t,!0);let s=t||`Vendedor`;a(e=>{let t=!1,n=!1;if(Pa.forEach(r=>{let i=r.items.find(t=>t.id===e);i&&(n=!0,(!i.roles||i.roles.includes(s))&&(t=!0))}),n&&!t)return h(`No tienes permiso para acceder a este módulo`,`warning`),`dashboard`}),document.querySelectorAll(`[data-goto]`).forEach(e=>{let t=e.dataset.goto,n=!1;Pa.forEach(e=>{let r=e.items.find(e=>e.id===t);r&&(!r.roles||r.roles.includes(s))&&(n=!0)}),n?e.classList.remove(`hidden`):e.classList.add(`hidden`)}),i(La),r(`inventory`,Mt()),r(`dashboard`,Ft()),r(`pos`,Sn()),r(`imei`,Jn()),r(`clients`,vr()),r(`credits`,Or()),r(`sales-history`,Rr()),r(`tasks`,Ur()),r(`calendar`,ei()),r(`users`,oi()),r(`reventas`,Oi()),r(`technical`,zi()),r(`expenses`,la()),r(`nominas`,ya()),r(`settings`,Ea()),o(`dashboard`)}function za(e){let t=document.getElementById(`step-credentials`),n=document.getElementById(`step-pin`);t&&t.classList.toggle(`hidden`,e!==`credentials`),n&&n.classList.toggle(`hidden`,e!==`pin`)}function Ba(){document.getElementById(`app-shell`).classList.add(`hidden`),document.getElementById(`login-screen`).classList.remove(`hidden`),za(`credentials`)}async function Va(){await ee(),Na(),Ba()}window.addEventListener(`session-expired`,()=>{Na(),Ba()}),document.getElementById(`login-form`)?.addEventListener(`submit`,Ha),document.getElementById(`pin-form`)?.addEventListener(`submit`,Ua),document.getElementById(`back-to-login`)?.addEventListener(`click`,()=>za(`credentials`)),document.getElementById(`logout-btn`)?.addEventListener(`click`,Va);async function Ha(e){e.preventDefault();let t=document.getElementById(`login-btn`),n=document.getElementById(`login-email`).value.trim(),r=document.getElementById(`login-pwd`).value.trim();t.disabled=!0,t.textContent=`Verificando...`;try{let e=await ae(n,r);e.success&&e.step===`pin`?(ka=n,document.getElementById(`pin-hint`).textContent=`Enviamos un PIN a ${n}`,za(`pin`),document.getElementById(`login-pin`).focus()):h(e.mensaje||`Credenciales incorrectas`,`error`)}catch{h(`Error de conexión`,`error`)}finally{t.disabled=!1,t.textContent=`Ingresar`}}async function Ua(e){e.preventDefault();let t=document.getElementById(`pin-btn`),n=document.getElementById(`login-pin`).value.trim();t.disabled=!0,t.textContent=`Verificando...`;try{let e=await oe(ka,n);e.success&&e.token?(Ma({email:e.email,nombre:e.nombre,rol:e.rol},e.token),Ra(e.nombre,e.rol)):h(e.mensaje||`PIN incorrecto`,`error`)}catch{h(`Error`,`error`)}finally{t.disabled=!1,t.textContent=`Verificar`}}var Wa=ja();Wa&&Wa.token&&f()?(d(Wa.token),Ra(Wa.nombre,Wa.rol)):(Na(),Ba());