(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e={},t=null,n=null;function r(t,n){e[t]=n}function i(e){t=e}function a(e){n=e}async function o(r){if(n){let e=n(r);if(e===!1)return;typeof e==`string`&&(r=e)}document.querySelectorAll(`[data-view]`).forEach(e=>{e.classList.add(`hidden`)});let i=document.querySelector(`[data-view="${r}"]`);if(!i)return console.warn(`View not found:`,r);i.classList.remove(`hidden`),t&&t(r),e[r]&&await e[r]()}var s=`https://adminpro-adminpro.aws-us-west-2.turso.io/v2/pipeline`,c=`eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3NzcyOTIwOTMsImlkIjoiMDE5ZGNlZGItOTIwMS03NGVkLWIwZGYtZjg4MTQ3NjlhODcxIiwicmlkIjoiNWIwMWViNTctYTgxYS00OTI0LWIzMDUtZjk1Y2EwMjUzNmRkIn0.oruUZmv_ZLWlKA2ctQnghAD5PIiJSIeR4nzbZia-q-f1r12IHhLv1hDw9CsReABIceaVRHPS52JMZ4j3lcZ1Bw`,l=`https://script.google.com/macros/s/AKfycbxxs-PUyNqqALN2-azEHaViv5PM1r5oteanvr2Sfydic-bBQJrGWj00R0FO7UAlP4Ug/exec`,u=localStorage.getItem(`adminpro_gas_token`)||``,d=e=>{u=e,localStorage.setItem(`adminpro_gas_token`,e)},f=()=>u,ee=()=>{localStorage.clear(),location.reload()};async function p(e){let t=Array.isArray(e)?e.map(e=>typeof e==`string`?{type:`execute`,stmt:{sql:e}}:e.type?e:{type:`execute`,stmt:e}):[{type:`execute`,stmt:typeof e==`string`?{sql:e}:e}],n=await(await fetch(s,{method:`POST`,headers:{Authorization:`Bearer ${c}`,"Content-Type":`application/json`},body:JSON.stringify({requests:t})})).json();if(n.error)throw Error(n.error.message);return(n.results||[]).map(e=>{if(!e.response||!e.response.result)return[];let{cols:t,rows:n}=e.response.result;return n.map(e=>{let n={};return t.forEach((t,r)=>{n[t.name]=e[r].value}),n})})}var m=e=>e.map(e=>({type:typeof e==`number`?`float`:`text`,value:String(e)}));async function te(e){return await(await fetch(l,{method:`POST`,mode:`cors`,body:JSON.stringify(e)})).json()}var ne=async(e,t,n)=>(await te({action:`uploadFoto`,token:u,base64Data:e,fileName:t,mimeType:n})).url||``,re=async(e,t)=>(await te({action:`uploadSignature`,token:u,base64Data:e,fileName:t})).url||``,ie=async(e,t,n)=>(await te({action:`uploadEvidencia`,token:u,base64Data:e,fileName:t,mimeType:n})).url||``,ae=async(e,t)=>{let n=await p({sql:`SELECT * FROM usuarios WHERE email = ? AND password = ? AND estado = 'Activo'`,args:[{type:`text`,value:e.toLowerCase()},{type:`text`,value:t}]});return!n[0]||n[0].length===0?{success:!1,mensaje:`Credenciales incorrectas`}:await(await fetch(`${l}?action=login&email=${encodeURIComponent(e)}&password=${t}`)).json()},oe=async(e,t)=>{let n=await(await fetch(`${l}?action=verifyPin&email=${encodeURIComponent(e)}&pin=${t}`)).json();if(n.success){d(n.token);let t={...(await p({sql:`SELECT nombre, rol, email FROM usuarios WHERE email = ?`,args:[{type:`text`,value:e.toLowerCase()}]}))[0]?.[0]||{nombre:`Usuario`},token:n.token};return localStorage.setItem(`adminpro_user`,JSON.stringify(t)),{success:!0,...t}}return n},se=async()=>(await p(`SELECT * FROM usuarios ORDER BY nombre ASC`))[0]||[],ce=e=>p({sql:`INSERT INTO usuarios (email, password, nombre, rol, estado) VALUES (?,?,?,?,?)`,args:m(e)}),le=(e,t,n)=>p({sql:`UPDATE usuarios SET email=?, password=?, nombre=?, rol=?, estado=? WHERE email=?`,args:[{type:`text`,value:t},...m(n),{type:`text`,value:e}]}),ue=e=>p({sql:`DELETE FROM usuarios WHERE email = ?`,args:[{type:`text`,value:e}]}),de=async()=>((await p(`SELECT * FROM clientes ORDER BY nombre ASC`))[0]||[]).map(e=>({...e,cedula:e.id})),fe=e=>p({sql:`INSERT INTO clientes VALUES (?,?,?,?,?,?,?)`,args:m([e.cedula,e.nombre,e.telefono,e.direccion,e.email,e.tipo,new Date().toISOString()])}),pe=(e,t)=>p({sql:`UPDATE clientes SET id=?, nombre=?, telefono=?, direccion=?, email=?, tipo=? WHERE id=?`,args:[...m([t.cedula,t.nombre,t.telefono,t.direccion,t.email,t.tipo]),{type:`text`,value:e}]}),me=e=>p({sql:`DELETE FROM clientes WHERE id = ?`,args:[{type:`text`,value:e}]}),he=async()=>(await p(`SELECT * FROM inventario ORDER BY id DESC`))[0].map(e=>({...e,stockActual:e.stock_actual,stockMinimo:e.stock_minimo,precioVenta:e.precio_venta,costo:e.costo})),ge=e=>p({sql:`INSERT INTO inventario VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),_e=(e,t)=>p({sql:`UPDATE inventario SET id=?, nombre=?, marca=?, categoria=?, tipo=?, costo=?, precio_venta=?, stock_minimo=?, stock_actual=?, ubicacion=?, sku=?, imagen=? WHERE id=?`,args:[...m(t),{type:`text`,value:e}]}),ve=e=>p({sql:`DELETE FROM inventario WHERE id = ?`,args:[{type:`text`,value:e}]}),ye=async()=>(await p(`SELECT * FROM equipos`))[0]||[],be=e=>p({sql:`INSERT INTO equipos VALUES (?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),xe=(e,t)=>p({sql:`UPDATE equipos SET imei1=?, imei2=?, id_producto=?, marca=?, nombre=?, proveedor=?, costo=?, venta=?, estado=?, fecha_ingreso=? WHERE imei1=?`,args:[...m(t),{type:`text`,value:e}]}),Se=e=>p({sql:`DELETE FROM equipos WHERE imei1 = ?`,args:[{type:`text`,value:e}]}),Ce=async()=>(await p(`SELECT * FROM ventas ORDER BY fecha DESC`))[0]||[],we=async e=>{let t=`FAC-${Date.now()}`,n=new Date().toISOString(),r=[{sql:`INSERT INTO ventas VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:[{type:`text`,value:t},{type:`text`,value:n},{type:`text`,value:e.cedula||``},{type:`text`,value:e.cliente||``},{type:`text`,value:e.direccion||``},{type:`text`,value:e.productoNombre||``},{type:`text`,value:String(e.total||0)},{type:`text`,value:String(e.items?.length||1)},{type:`text`,value:e.imeis||`N/A`},{type:`float`,value:e.subtotal||0},{type:`float`,value:e.descuento||0},{type:`float`,value:e.total||0},{type:`text`,value:e.metodo||``},{type:`text`,value:e.vendedor||``},{type:`text`,value:e.firmaVendedor||``},{type:`text`,value:e.firmaComprador||``},{type:`text`,value:e.evidencia||``},{type:`text`,value:e.ciudad||``},{type:`text`,value:e.telefono||``},{type:`text`,value:e.tipoFactura||`fisica`}]}];return e.items&&Array.isArray(e.items)?e.items.forEach(e=>{r.push({sql:`UPDATE inventario SET stock_actual = stock_actual - ? WHERE id = ?`,args:[{type:`float`,value:parseFloat(e.qty||1)},{type:`text`,value:e.id}]})}):r.push({sql:`UPDATE inventario SET stock_actual = stock_actual - 1 WHERE id = ?`,args:[{type:`text`,value:e.productoId||``}]}),await p(r),{success:!0,idFactura:t}},Te=async()=>(await p(`SELECT * FROM creditos`))[0].map(e=>({...e,id:e.id_credito,idFactura:e.id_factura_ref,abonado:e.total_abonado,saldo:e.saldo_pendiente,total:e.valor_total})),Ee=e=>p({sql:`INSERT INTO creditos (id_credito, cliente, telefono, id_factura_ref, fecha_deuda, tipo, valor_total, total_abonado, saldo_pendiente, estado, detalle) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,args:m([Date.now().toString(),e.cliente,e.telefono,e.idFactura,new Date().toISOString(),e.tipo||`Crédito`,e.total,0,e.total,`Activo`,e.detalle])}),De=(e,t)=>p({sql:`UPDATE creditos SET id_credito=?, cliente=?, telefono=?, id_factura_ref=?, fecha_deuda=?, tipo=?, valor_total=?, total_abonado=?, saldo_pendiente=?, estado=?, fecha_cancelacion=?, detalle=?, historial_abonos=? WHERE id_credito=?`,args:[...m(t),{type:`text`,value:e}]}),Oe=async()=>(await p(`SELECT * FROM reventas ORDER BY fecha DESC`))[0].map(e=>({...e,id:e.id_reventa,producto:e.producto,costo:e.costo_proveedor,precio:e.precio_venta,utilidad:e.utilidad})),ke=e=>p({sql:`INSERT INTO reventas VALUES (?,?,?,?,?,?,?,?)`,args:m([`REV-${Date.now()}`,new Date().toISOString(),e.producto,e.categoria,e.costo,e.precio,e.proveedor,e.precio-e.costo])}),Ae=e=>p({sql:`DELETE FROM reventas WHERE id_reventa = ?`,args:[{type:`text`,value:e}]}),je=async()=>(await p(`SELECT * FROM servicio_tecnico ORDER BY id_orden DESC`))[0]||[],Me=e=>p({sql:`INSERT INTO servicio_tecnico VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,args:m(e)}),Ne=(e,t)=>p({sql:`UPDATE servicio_tecnico SET id_orden=?, cliente=?, telefono=?, equipo=?, imei_serie=?, falla=?, clave_patron=?, repuestos=?, costo_taller=?, abono=?, precio_final=?, estado=?, evidencias=? WHERE id_orden=?`,args:[...m(t),{type:`text`,value:e}]}),Pe=e=>p({sql:`DELETE FROM servicio_tecnico WHERE id_orden = ?`,args:[{type:`text`,value:e}]}),Fe=async()=>{let e=await p([`SELECT COUNT(*) as c FROM inventario`,`SELECT COUNT(*) as c FROM clientes`,`SELECT SUM(total) as s FROM ventas WHERE date(fecha) = date('now')`,`SELECT SUM(monto) as s FROM egresos WHERE date(fecha) = date('now')`,`SELECT SUM(stock_actual) as s FROM inventario`,`SELECT COUNT(*) as c FROM inventario WHERE stock_actual <= 1`,`SELECT * FROM ventas ORDER BY fecha DESC LIMIT 8`,`SELECT date(fecha) as d, SUM(total) as m FROM ventas WHERE date(fecha) >= date('now','-7 days') GROUP BY d`,`SELECT COUNT(*) as c FROM equipos`,`SELECT productos, COUNT(*) as qty FROM ventas GROUP BY productos ORDER BY qty DESC LIMIT 5`,`SELECT id, nombre, stock_actual, stock_minimo FROM inventario WHERE stock_actual <= 1 LIMIT 5`,`SELECT id_orden, cliente, equipo, estado FROM servicio_tecnico ORDER BY id_orden DESC LIMIT 5`,`SELECT COUNT(*) as c FROM creditos WHERE estado != 'Pagado' AND estado != 'Cancelado'`,`SELECT COUNT(*) as c FROM reventas`]),t=[],n=[];for(let r=6;r>=0;r--){let i=new Date;i.setDate(i.getDate()-r);let a=i.toISOString().slice(0,10);t.push(i.toLocaleDateString(`es-CO`,{weekday:`short`,day:`numeric`}));let o=(e[7]||[]).find(e=>e.d===a);n.push(o?o.m:0)}return{ingresosHoy:e[2]?.[0]?.s||0,egresosHoy:e[3]?.[0]?.s||0,utilidad:(e[2]?.[0]?.s||0)-(e[3]?.[0]?.s||0),totalProductos:e[0]?.[0]?.c||0,totalClientes:e[1]?.[0]?.c||0,totalStock:e[4]?.[0]?.s||0,stockCritico:e[5]?.[0]?.c||0,totalEquipos:e[8]?.[0]?.c||0,ventasRecientes:e[6]||[],topProductos:(e[9]||[]).map(e=>({nombre:e.productos,cantidad:e.qty})),productosBajoStock:(e[10]||[]).map(e=>({...e,stockActual:e.stock_actual})),tecRecientes:e[11]||[],creditosActivos:e[12]?.[0]?.c||0,totalReventas:e[13]?.[0]?.c||0,labels7d:t,ventas7d:n}},Ie=async()=>((await p(`SELECT * FROM egresos ORDER BY fecha DESC`))[0]||[]).map(e=>({...e,id:e.id_gasto})),Le=e=>p({sql:`INSERT INTO egresos VALUES (?,?,?,?,?,?,?)`,args:m([`EGR-${Date.now()}`,new Date().toISOString(),e.categoria,e.concepto,e.responsable,e.monto,``])}),Re=async()=>((await p(`SELECT * FROM nominas ORDER BY fecha DESC`))[0]||[]).map(e=>({...e,id:e.id_nomina})),ze=e=>p({sql:`INSERT INTO nominas VALUES (?,?,?,?,?,?,?,?,?,?)`,args:m([`NOM-${Date.now()}`,e.fecha||new Date().toISOString(),e.empleado,e.periodo,e.salario_base,e.deducciones,e.bonificaciones,e.total_pagar,e.estado||`Pendiente`,e.notas||``])}),Be=e=>p({sql:`DELETE FROM nominas WHERE id_nomina = ?`,args:[{type:`text`,value:e}]}),Ve=async()=>(await p(`SELECT * FROM tareas ORDER BY date(fecha_vencimiento) ASC`))[0]||[],He=e=>p({sql:`INSERT INTO tareas VALUES (?,?,?,?,?,?,?,?,?)`,args:m([`T-${Date.now()}`,e.tarea,e.fecha_inicio,e.fecha_vencimiento,e.prioridad,e.estado||`Pendiente`,e.responsable,e.notas||``,e.color||`#4f46e5`])}),Ue=(e,t)=>p({sql:`UPDATE tareas SET estado = ? WHERE id = ?`,args:[{type:`text`,value:t},{type:`text`,value:e}]}),We=e=>p({sql:`DELETE FROM tareas WHERE id = ?`,args:[{type:`text`,value:e}]});function h(e,t=`success`){let n={success:`bg-green-600`,error:`bg-error`,info:`bg-tertiary`,warning:`bg-amber-500`},r={success:`check_circle`,error:`error`,info:`info`,warning:`warning`},i=document.createElement(`div`);i.className=`fixed bottom-4 right-4 z-[9999] toast-animate`,i.innerHTML=`
  ...
  `}).join(``),document.querySelectorAll(`.cs-item-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.doc,n=O.find(e=>(e.cedula||e.documento||``)===t);n&&(n.documento=t,an(n))})})}function an(e){Kt&&Kt(e),sn()}async function on(e){await en(),Kt=e,Xt.value=``,O.length>0?rn([...O].reverse().slice(0,20)):Qt.innerHTML=`<li class="p-4 text-center text-sm text-on-surface-variant">No hay clientes. Crea uno nuevo.</li>`,$t.classList.add(`hidden`),qt.classList.remove(`hidden`),qt.classList.add(`flex`),setTimeout(()=>Xt.focus(),100)}function sn(){qt.classList.add(`hidden`),qt.classList.remove(`flex`),Kt=null}var k=[],A=[],cn=!1,ln=!1,un=null,j,dn,fn,pn,mn,hn,gn,_n,vn,yn,bn,M,xn,N,P,Sn,Cn,wn,Tn,En,Dn,On,kn,An,jn,Mn,F,I,L;function Nn(){return async()=>{Pn(),cn||=(await Fn(),Ln(),Hn(N,e=>e),Hn(P,e=>e),Hn(F,e=>I=e),!0),In(k),Rn()}}function Pn(){j=document.getElementById(`pos-search`),dn=document.getElementById(`pos-products-grid`),fn=document.getElementById(`pos-cart-items`),pn=document.getElementById(`pos-subtotal`),mn=document.getElementById(`pos-descuento`),hn=document.getElementById(`pos-total`),document.getElementById(`pos-pay-btn`),gn=document.getElementById(`pos-cliente-doc`),_n=document.getElementById(`pos-cliente-nombre`),vn=document.getElementById(`pos-checkout-modal`),yn=document.getElementById(`pos-checkout-close`),bn=document.getElementById(`pos-checkout-cancel`),M=document.getElementById(`pos-checkout-confirm`),xn=document.getElementById(`pos-evidencia-file`),N=document.getElementById(`pos-canvas-cliente`),P=document.getElementById(`pos-canvas-vendedor`),Sn=document.getElementById(`pos-cliente-direccion`),Cn=document.getElementById(`pos-cliente-ciudad`),wn=document.getElementById(`pos-cliente-tel`),Tn=document.getElementById(`pos-digital-fields`),En=document.getElementById(`pos-signatures-container`),Dn=document.getElementById(`pos-imei-container`),On=document.getElementById(`pos-imei-list`),kn=document.getElementById(`pos-signature-modal`),An=document.getElementById(`pos-sig-modal-close`),jn=document.getElementById(`pos-sig-modal-clear`),Mn=document.getElementById(`pos-sig-modal-save`),F=document.getElementById(`pos-canvas-fullscreen`)}async function Fn(){try{k=(await he()).filter(e=>e.stockActual>0)}catch{k=[]}}function In(e){if(e.length===0){dn.innerHTML=`<p class="p-4 col-span-full text-center opacity-50 italic text-sm">Sin stock disponible</p>`;return}dn.innerHTML=e.map(e=>`
  ...
  `).join(``)}function Ln(){j.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();In(t?k.filter(e=>e.nombre.toLowerCase().includes(t)||e.sku.toLowerCase().includes(t)):k)}),document.addEventListener(`barcodeScanned`,e=>{let t=document.querySelector(`[data-view="pos"]`);if(!t||t.classList.contains(`hidden`))return;let n=e.detail,r=k.find(e=>e.sku===n||e.id===n);r?(window.posAddToCart(r.id),h(`✅ ${r.nombre} agregado`,`success`),document.activeElement===j&&(j.value=``,In(k),j.blur())):h(`Código ${n} no encontrado`,`warning`)}),document.getElementById(`pos-scan-btn`)?.addEventListener(`click`,()=>{vt({title:`Escanear`,onScan:e=>{j.value=e,j.dispatchEvent(new Event(`input`));let t=k.find(t=>t.sku===e||t.id===e);t?(window.posAddToCart(t.id),h(`✅ ${t.nombre} agregado`,`success`),setTimeout(()=>{j.value===e&&(j.value=``,j.dispatchEvent(new Event(`input`)))},1500)):h(`Código ${e} no encontrado en inventario`,`warning`)}})}),document.getElementById(`pos-reventa-btn`)?.addEventListener(`click`,()=>{window.__posReventaMode=!0,o(`inventory`),setTimeout(()=>{window.inventoryView&&window.inventoryView.openNuevo&&window.inventoryView.openNuevo(!0)},150)}),document.getElementById(`pos-select-client-btn`)?.addEventListener(`click`,()=>{on(e=>{_n.value=e.nombre,gn.value=e.documento})}),document.querySelectorAll(`input[name="pos-billing-type"]`).forEach(e=>{e.addEventListener(`change`,e=>{let t=e.target.value===`digital`,n=document.getElementById(`pos-evidencia-container`);t?(n.classList.add(`hidden`),Tn.classList.remove(`hidden`),En.classList.remove(`hidden`),Vn()):(n.classList.remove(`hidden`),Tn.classList.add(`hidden`),En.classList.add(`hidden`),Dn.classList.add(`hidden`))})}),An.addEventListener(`click`,()=>kn.classList.add(`hidden`)),jn.addEventListener(`click`,()=>I.clearRect(0,0,F.width,F.height)),Mn.addEventListener(`click`,()=>{if(L){let e=L.getContext(`2d`);e.clearRect(0,0,L.width,L.height),e.drawImage(F,0,0,L.width,L.height)}kn.classList.add(`hidden`)});let e=(e,t)=>{L=e,document.getElementById(`pos-sig-modal-title`).textContent=t,kn.classList.remove(`hidden`),F.width=F.offsetWidth,F.height=F.offsetHeight,I.lineWidth=3,I.lineCap=`round`,I.strokeStyle=`#000`,I.clearRect(0,0,F.width,F.height)};N.parentElement.addEventListener(`click`,()=>e(N,`Firma del Comprador`)),P.parentElement.addEventListener(`click`,()=>e(P,`Firma del Vendedor`)),window.posAddToCart=e=>{let t=k.find(t=>t.id===e);if(!t)return;let n=A.find(t=>t.id===e);if(n){if(n.qty>=t.stockActual)return h(`Sin stock`,`warning`);n.qty++}else A.push({...t,qty:1,precioManual:0});Rn(),setTimeout(()=>{let e=fn.querySelectorAll(`input[oninput*="posUpdatePrice"]`);e.length>0&&e[e.length-1].focus()},100)},window.posRemoveItem=e=>{A=A.filter(t=>t.id!==e),Rn()},window.posUpdateQty=(e,t)=>{let n=A.find(t=>t.id===e),r=k.find(t=>t.id===e);n&&(n.qty+=t,n.qty<=0?window.posRemoveItem(e):n.qty>r.stockActual&&(n.qty=r.stockActual),Rn())},window.posUpdatePrice=(e,t)=>{let n=A.find(t=>t.id===e),r=Number(t.value.replace(/\D/g,``));n&&(n.precioManual=r,t.value=new Intl.NumberFormat(`es-CO`).format(r),zn())},mn.addEventListener(`input`,Rn);let t=e=>{if(A.length!==0){if(!_n.value)return h(`Nombre cliente ok?`,`warning`);un=e,Un()}};document.getElementById(`pos-pay-btn-venta`)?.addEventListener(`click`,()=>t(`venta`)),document.getElementById(`pos-pay-btn-credito`)?.addEventListener(`click`,()=>t(`credito`)),document.getElementById(`pos-pay-btn-separe`)?.addEventListener(`click`,()=>t(`separe`)),yn.addEventListener(`click`,Wn),bn.addEventListener(`click`,Wn),M.addEventListener(`click`,Gn)}function Rn(){if(A.length===0){fn.innerHTML=`
      ...
      </div>`,Bn(0);return}let e=0;fn.innerHTML=A.map(t=>{let n=t.precioManual||0;return e+=n*t.qty,`
    ...
    </div>`}).join(``),Bn(e)}function zn(){let e=0;A.forEach(t=>e+=(t.precioManual||t.precioVenta||0)*t.qty),Bn(e)}function Bn(e){let t=parseFloat(mn.value)||0;pn.textContent=`$${new Intl.NumberFormat(`es-CO`).format(e)}`,hn.textContent=`$${new Intl.NumberFormat(`es-CO`).format(Math.max(0,e-t))}`}function Vn(){let e=A.filter(e=>{let t=k.find(t=>t.id===e.id);return t&&(t.categoria===`Celulares`||t.nombre.toLowerCase().includes(`celular`)||t.nombre.toLowerCase().includes(`teléfono`))});e.length>0?(Dn.classList.remove(`hidden`),On.innerHTML=e.map(e=>`
    ...
    `).join(``)):(Dn.classList.add(`hidden`),On.innerHTML=``)}function Hn(e,t){let n=e.getContext(`2d`);t(n);let r=t=>{let n=e.getBoundingClientRect(),r=(t.touches?t.touches[0].clientX:t.clientX)-n.left,i=(t.touches?t.touches[0].clientY:t.clientY)-n.top;return{x:r*(e.width/n.width),y:i*(e.height/n.height)}},i=!1,a=e=>{i=!0,n.beginPath();let{x:t,y:a}=r(e);n.moveTo(t,a),e.preventDefault()},o=e=>{if(!i)return;let{x:t,y:a}=r(e);n.lineTo(t,a),n.stroke(),e.preventDefault()};e.addEventListener(`mousedown`,a),e.addEventListener(`mousemove`,o),e.addEventListener(`mouseup`,()=>i=!1),e.addEventListener(`touchstart`,a,{passive:!1}),e.addEventListener(`touchmove`,o,{passive:!1}),e.addEventListener(`touchend`,()=>i=!1),e.nextElementSibling.addEventListener(`click`,t=>{t.stopPropagation(),n.clearRect(0,0,e.width,e.height)})}function Un(){vn.classList.remove(`hidden`),vn.classList.add(`flex`),[N,P].forEach(e=>{e.width=e.offsetWidth,e.height=e.offsetHeight;let t=e.getContext(`2d`);t.lineWidth=2,t.lineCap=`round`,t.clearRect(0,0,e.width,e.height)}),Sn.value=``,Cn.value=``,wn.value=``,document.querySelector(`input[name="pos-billing-type"][value="fisica"]`).checked=!0,document.getElementById(`pos-evidencia-container`).classList.remove(`hidden`),Tn.classList.add(`hidden`),En.classList.add(`hidden`),Dn.classList.add(`hidden`)}function Wn(){vn.classList.add(`hidden`),vn.classList.remove(`flex`)}async function Gn(){if(ln)return;ln=!0;let e=document.querySelector(`input[name="pos-billing-type"]:checked`).value;M.textContent=`Procesando...`,M.disabled=!0;try{let t=``,n=``,r=``;if(e===`digital`){let e=document.createElement(`canvas`);if(e.width=N.width,e.height=N.height,N.toDataURL()!==e.toDataURL()){let e=await re(N.toDataURL(`image/png`),`FirmaCliente_${Date.now()}.png`);t=typeof e==`string`?e:e?.url||``}if(P.toDataURL()!==e.toDataURL()){let e=await re(P.toDataURL(`image/png`),`FirmaVendedor_${Date.now()}.png`);n=typeof e==`string`?e:e?.url||``}}if(e===`fisica`)if(xn.files[0]){M.textContent=`Subiendo evidencia...`;let e=xn.files[0],t=await ie(await new Promise(t=>{let n=new FileReader;n.onload=e=>t(e.target.result),n.readAsDataURL(e)}),e.name,e.type);r=typeof t==`string`?t:t?.url||``}else{h(`Por favor sube la foto de la factura física`,`warning`),ln=!1,M.textContent=`Confirmar y Facturar`,M.disabled=!1;return}let i=document.querySelectorAll(`.pos-imei-input`),a={};i.forEach(e=>{let t=e.dataset.id;e.value.trim()&&(a[t]||(a[t]=[]),a[t].push(e.value.trim()))}),M.textContent=`Registrando venta...`;let o=Number(pn.textContent.replace(/\D/g,``)),s=Number(hn.textContent.replace(/\D/g,``)),c=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`),l={cedula:gn.value.trim(),cliente:_n.value.trim(),direccion:Sn.value.trim(),ciudad:Cn.value.trim(),telefono:wn.value.trim(),productoNombre:A.map(e=>`${e.nombre} (x${e.qty})`).join(`, `),productoId:A[0]?.id,items:A.map(e=>({id:e.id,qty:e.qty})),subtotal:o,descuento:Number(mn.value)||0,total:s,metodo:document.getElementById(`pos-metodo-pago`).value,vendedor:c.nombre||`Vendedor`,firmaComprador:t,firmaVendedor:n,evidencia:r,tipoFactura:e,imeis:JSON.stringify(a),emisor:{nombre:`CLAROCELL.COM`,propietario:`Yeison Rangel Rangel`,nit:`1193400777-2`,direccion:`Calle 12 No. 10 - 108, Maicao - La Guajira`,contacto:`3016807310`,correo:`yeison0021@hotmail.com`}},u=await we(l);u.success?(un!==`venta`&&await Ee({cliente:l.cliente,telefono:l.cedula,idFactura:u.idFactura,total:s,detalle:l.productoNombre,tipo:un===`separe`?`Plan Separe`:`Crédito`}),h(`Venta Exitosa`,`success`),A=[],Rn(),gn.value=``,_n.value=``,Wn(),Fn().then(()=>In(k))):h(`Error al guardar`,`error`)}catch(e){h(e.message,`error`)}finally{ln=!1,M.textContent=`Confirmar y Facturar`,M.disabled=!1}}var Kn=[],qn=!1,Jn=!1,Yn,Xn,Zn,Qn,$n,er,tr,nr,R,z,rr,B,ir,ar,V,H,or,sr;function cr(){return async()=>{lr(),qn||=(await ur(),fr(),!0),dr(Kn)}}function lr(){Yn=document.getElementById(`imei-table-body`),Xn=document.getElementById(`imei-search`),Zn=document.getElementById(`imei-filter-status`),Qn=document.getElementById(`imei-new-btn`),$n=document.getElementById(`imei-modal`),er=document.getElementById(`imei-modal-close`),tr=document.getElementById(`imei-modal-backdrop`),nr=document.getElementById(`imei-form`),R=document.getElementById(`imei-save-btn`),z=document.getElementById(`imei-1`),rr=document.getElementById(`imei-2`),B=document.getElementById(`imei-nombre`),ir=document.getElementById(`imei-marca`),ar=document.getElementById(`imei-proveedor`),V=document.getElementById(`imei-costo`),H=document.getElementById(`imei-venta`),or=document.getElementById(`imei-estado`),sr=document.getElementById(`imei-original`)}async function ur(){try{Yn.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando equipos...</td></tr>`,Kn=await ye();let e=await he();B.innerHTML=`<option value="">Seleccione un equipo...</option>`+e.map(e=>`<option value="${e.nombre}" data-marca="${e.marca}" data-costo="${e.costo}" data-precio="${e.precioVenta}" data-prov="${e.proveedor||``}" data-id="${e.id}">${e.nombre}${e.marca?` (`+e.marca+`)`:``}</option>`).join(``)}catch(e){h(`Error cargando equipos: `+e.message,`error`),Kn=[]}}function dr(e){if(e.length===0){Yn.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron equipos</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;Yn.innerHTML=e.map(e=>{e.estado;let n=e.estado===`Disponible`?`bg-green-100 text-green-800`:e.estado===`Vendido`?`bg-red-100 text-red-800`:`bg-yellow-100 text-yellow-800`;return`
    ...
    `}).join(``)}function fr(){let e=()=>{let e=Xn.value.toLowerCase().trim(),t=Zn.value;dr(Kn.filter(n=>{let r=(n.imei1||``).toLowerCase().includes(e)||(n.imei2||``).toLowerCase().includes(e)||(n.nombre||``).toLowerCase().includes(e),i=t?n.estado===t:!0;return r&&i}))},t=e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))};V.addEventListener(`input`,t),H.addEventListener(`input`,t),B.addEventListener(`change`,e=>{let t=e.target.selectedOptions[0];if(t&&t.value){ir.value=t.dataset.marca||``,ar.value=t.dataset.prov||``;let e=parseInt(String(t.dataset.costo||`0`).replace(/\D/g,``))||0,n=parseInt(String(t.dataset.precio||`0`).replace(/\D/g,``))||0;V.value=new Intl.NumberFormat(`es-CO`).format(e),H.value=new Intl.NumberFormat(`es-CO`).format(n)}else ir.value=``,ar.value=``,V.value=``,H.value=``}),Xn.addEventListener(`input`,e),Zn.addEventListener(`change`,e),document.getElementById(`imei-scan-btn`)?.addEventListener(`click`,()=>{vt({title:`Escanear IMEI`,onScan:t=>{Xn.value=t,e(),h(`IMEI: ${t}`,`info`)}})}),document.getElementById(`imei-scan-1`)?.addEventListener(`click`,()=>{vt({title:`Escanear IMEI 1`,onScan:e=>{z.value=e,h(`IMEI 1: ${e}`,`success`)}})}),document.getElementById(`imei-scan-2`)?.addEventListener(`click`,()=>{vt({title:`Escanear IMEI 2`,onScan:e=>{rr.value=e,h(`IMEI 2: ${e}`,`success`)}})}),Qn.addEventListener(`click`,()=>pr(null)),er.addEventListener(`click`,mr),tr.addEventListener(`click`,mr),R.addEventListener(`click`,hr),window.imeiEdit=e=>{let t=Kn.find(t=>t.imei1==e);t&&pr(t)},window.imeiDelete=async t=>{if(confirm(`¿Eliminar el equipo con IMEI ${t}?`))try{h(`Eliminando...`,`info`);let n=await Se(t);n&&n.success?(h(`Equipo eliminado`,`success`),await ur(),e()):h(n.mensaje||`Error al eliminar`,`error`)}catch(e){h(`Error: `+e.message,`error`)}}}function pr(e){nr.reset(),e?(sr.value=e?e.imei1:``,z.value=e?e.imei1:``,rr.value=e?e.imei2:``,B.value=e?e.nombre:``,ir.value=e?e.marca:``,ar.value=e?e.proveedor:``,V.value=e&&e.costo?new Intl.NumberFormat(`es-CO`).format(parseInt(String(e.costo).replace(/\D/g,``))||0):``,H.value=e&&e.venta?new Intl.NumberFormat(`es-CO`).format(parseInt(String(e.venta).replace(/\D/g,``))||0):``,or.value=e?e.estado:`Disponible`,document.getElementById(`imei-modal-title`).textContent=`Editar Equipo`):(sr.value=``,or.value=`Disponible`,document.getElementById(`imei-modal-title`).textContent=`Registrar Equipo`),$n.classList.remove(`hidden`),$n.classList.add(`flex`)}function mr(){$n.classList.add(`hidden`),$n.classList.remove(`flex`)}async function hr(){if(!z.value.trim()){h(`El IMEI Principal es obligatorio`,`warning`),z.focus();return}if(!B.value){h(`Debe seleccionar un equipo del inventario`,`warning`),B.focus();return}if(!Jn){Jn=!0,R.textContent=`Guardando...`,R.disabled=!0;try{let e=sr.value,t={imei1:z.value.trim(),imei2:rr.value.trim(),nombre:B.value.trim(),marca:ir.value.trim(),proveedor:ar.value.trim(),costo:parseInt(V.value.replace(/\D/g,``))||0,venta:parseInt(H.value.replace(/\D/g,``))||0,estado:or.value},n;n=e?await xe(e,t):await be(t),n&&n.success?(h(`Equipo guardado`,`success`),mr(),await ur(),dr(Kn)):h(n?.mensaje||`Error al guardar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{Jn=!1,R.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,R.disabled=!1}}}var U=[],gr=!1,_r=!1,vr,yr,br,xr,Sr,Cr,wr,W,Tr,Er,Dr,Or,kr,Ar,jr;function Mr(){return async()=>{Nr(),gr||=(await Pr(),Ir(),!0),Fr(U)}}function Nr(){vr=document.getElementById(`cli-table-body`),yr=document.getElementById(`cli-search`),br=document.getElementById(`cli-new-btn`),xr=document.getElementById(`cli-modal`),Sr=document.getElementById(`cli-modal-close`),Cr=document.getElementById(`cli-modal-backdrop`),wr=document.getElementById(`cli-form`),W=document.getElementById(`cli-save-btn`),Tr=document.getElementById(`cli-doc`),Er=document.getElementById(`cli-nombre`),Dr=document.getElementById(`cli-tel`),Or=document.getElementById(`cli-email`),kr=document.getElementById(`cli-dir`),Ar=document.getElementById(`cli-tipo`),jr=document.getElementById(`cli-original`)}async function Pr(){try{vr.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando clientes...</td></tr>`,U=await de()}catch(e){h(`Error cargando clientes: `+e.message,`error`),U=[]}}function Fr(e){if(e.length===0){vr.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron clientes</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;vr.innerHTML=e.map(e=>{let n=e.tipo===`VIP`?`bg-amber-100 text-amber-800`:e.tipo===`Empresa`?`bg-blue-100 text-blue-800`:e.tipo===`Mayorista`?`bg-purple-100 text-purple-800`:`bg-surface-container text-on-surface-variant`;return`
      <tr class="hover:bg-surface-container-low transition-colors">
    ...
    `}).join(``)}function Ir(){let e=()=>{let e=yr.value.toLowerCase().trim();Fr(U.filter(t=>(t.cedula||``).toLowerCase().includes(e)||(t.nombre||``).toLowerCase().includes(e)||(t.telefono||``).toLowerCase().includes(e)))};yr.addEventListener(`input`,e),br.addEventListener(`click`,()=>Lr(null)),Sr.addEventListener(`click`,Rr),Cr.addEventListener(`click`,Rr),W.addEventListener(`click`,zr),window.cliEdit=e=>{let t=U.find(t=>t.cedula==e);t&&Lr(t)},window.cliDelete=async t=>{if(confirm(`¿Eliminar al cliente con documento ${t}?`))try{h(`Eliminando...`,`info`);let n=await me(t);n&&n.success?(h(`Cliente eliminado`,`success`),await Pr(),e()):h(n.mensaje||`Error al eliminar`,`error`)}catch(e){h(`Error: `+e.message,`error`)}}}function Lr(e){wr.reset(),e?(jr.value=e.cedula,Tr.value=e.cedula,Er.value=e.nombre||``,Dr.value=e.telefono||``,Or.value=e.email||``,kr.value=e.direccion||``,Ar.value=e.tipo||`Normal`,document.getElementById(`cli-modal-title`).textContent=`Editar Cliente`):(jr.value=``,Ar.value=`Normal`,document.getElementById(`cli-modal-title`).textContent=`Nuevo Cliente`),xr.classList.remove(`hidden`),xr.classList.add(`flex`)}function Rr(){xr.classList.add(`hidden`),xr.classList.remove(`flex`)}async function zr(){if(!wr.checkValidity()){wr.reportValidity();return}if(!_r){_r=!0,W.textContent=`Guardando...`,W.disabled=!0;try{let e=jr.value,t={cedula:Tr.value.trim(),nombre:Er.value.trim(),telefono:Dr.value.trim(),email:Or.value.trim(),direccion:kr.value.trim(),tipo:Ar.value},n;n=e?await pe(e,t):await fe(t),n&&n.success?(h(`Cliente guardado`,`success`),Rr(),await Pr(),Fr(U)):h(n?.mensaje||`Error al guardar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{_r=!1,W.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,W.disabled=!1}}}var G=[],Br=!1,Vr=!1;function Hr(){return async()=>{Br||=(await Ur(),Yr(),!0),Jr(G)}}async function Ur(){let e=document.getElementById(`cred-table-body`);e&&(e.innerHTML=`<tr><td colspan="9" class="p-6 text-center text-on-surface-variant">Cargando créditos...</td></tr>`);try{G=await Te()}catch(e){h(`Error cargando créditos: `+e.message,`error`),G=[]}}var K=e=>`$`+new Intl.NumberFormat(`es-CO`).format(Math.round(e||0));function Wr(e,t){if(!e)return 0;let n=e=>{if(!e)return null;let t=String(e).split(`/`);return t.length===3?new Date(t[2],t[1]-1,t[0]):new Date(e)},r=n(e),i=t?n(t):new Date;return!r||isNaN(r)?0:Math.max(0,Math.floor((i-r)/864e5))}function Gr(e){return e?e.split(`;`).filter(Boolean).map(e=>{let t=e.split(`|`);return{fecha:t[0]||``,monto:parseFloat(t[1])||0,nota:t[2]||``}}):[]}function Kr(e){return e.map(e=>`${e.fecha}|${e.monto}|${e.nota}`).join(`;`)}function qr(e){let t=0,n=0;e.forEach(e=>{e.estado!==`Cancelado`&&(t+=e.saldo||0),n+=e.abonado||0});let r=document.getElementById(`cred-stat-total`),i=document.getElementById(`cred-stat-recaudo`);r&&(r.textContent=K(t)),i&&(i.textContent=K(n))}function Jr(e){qr(G);let t=document.getElementById(`cred-table-body`);if(t){if(e.length===0){t.innerHTML=`<tr><td colspan="9" class="p-6 text-center text-on-surface-variant">No se encontraron créditos</td></tr>`;return}t.innerHTML=e.map(e=>{let t=e.estado===`Cancelado`||e.estado===`Entregado`,n=Wr(e.fecha,t?e.fechaCancelacion:null),r=t?`bg-green-100 text-green-800`:e.estado===`En Mora`?`bg-red-100 text-red-800`:`bg-orange-100 text-orange-800`,i=e.tipo===`Plan Separe`?`bg-emerald-100 text-emerald-800 border-emerald-200`:`bg-blue-100 text-blue-800 border-blue-200`,a=t?`<span class="text-[10px] text-on-surface-variant">Pagó en ${n}d</span>`:`<span class="text-[10px] font-bold ${n>30?`text-red-500`:`text-orange-500`}">${n} días</span>`,o=String(e.telefono||``).replace(/\D/g,``),s=`https://wa.me/57${o}?text=${encodeURIComponent(`Hola ${e.cliente}, le recordamos que tiene un saldo pendiente de ${K(e.saldo)} con nosotros. Gracias.`)}`;return`
    ...
    `}).join(``)}}function Yr(){let e=document.getElementById(`cred-search`),t=document.getElementById(`cred-filter-status`),n=document.getElementById(`cred-filter-tipo`),r=()=>{let r=(e?.value||``).toLowerCase().trim(),i=t?.value||``,a=n?.value||``;Jr(G.filter(e=>{let t=(e.cliente||``).toLowerCase().includes(r)||(e.idFactura||``).toLowerCase().includes(r),n=i?e.estado===i:!0,o=a?(e.tipo||`Crédito`)===a:!0;return t&&n&&o}))};e?.addEventListener(`input`,r),t?.addEventListener(`change`,r),n?.addEventListener(`change`,r);let i=e=>{let t=e.target.value.replace(/\D/g,``);e.target.value=t?new Intl.NumberFormat(`es-CO`).format(parseInt(t)):``};document.getElementById(`cred-new-total`)?.addEventListener(`input`,i),document.getElementById(`cred-new-abono`)?.addEventListener(`input`,i),document.getElementById(`cred-monto-abono`)?.addEventListener(`input`,i);let a=document.getElementById(`cred-modal`),o=document.getElementById(`cred-modal-close`),s=document.getElementById(`cred-modal-backdrop`),c=document.getElementById(`cred-save-btn`),l=()=>{a?.classList.add(`hidden`),a?.classList.remove(`flex`)};o?.addEventListener(`click`,l),s?.addEventListener(`click`,l),window.credAddAbono=e=>{let t=G.find(t=>t.id==e);if(!t)return;document.getElementById(`cred-id`).value=t.id,document.getElementById(`cred-cliente-name`).textContent=t.cliente,document.getElementById(`cred-saldo-actual`).textContent=K(t.saldo);let n=Gr(t.historialAbonos),r=document.getElementById(`cred-historial`);r&&(r.innerHTML=n.length===0?`<p class="text-xs text-on-surface-variant">Sin abonos anteriores</p>`:n.map(e=>`
            ...
            </div>`).join(``)),document.getElementById(`cred-monto-abono`).value=``,document.getElementById(`cred-nota-abono`).value=``,a?.classList.remove(`hidden`),a?.classList.add(`flex`),document.getElementById(`cred-monto-abono`)?.focus()},c?.addEventListener(`click`,async()=>{let e=document.getElementById(`cred-id`).value,t=parseInt((document.getElementById(`cred-monto-abono`).value||``).replace(/\D/g,``))||0,n=(document.getElementById(`cred-nota-abono`)?.value||``).trim();if(!t||t<=0){h(`Ingresa un monto válido`,`warning`);return}if(!Vr){Vr=!0,c.textContent=`Aplicando...`,c.disabled=!0;try{let r=G.find(t=>t.id==e),i=(r.abonado||0)+t,a=Math.max(0,(r.total||0)-i),o=a<=0,s=Gr(r.historialAbonos);s.push({fecha:new Date().toLocaleDateString(`es-CO`),monto:t,nota:n});let c=r.tipo===`Plan Separe`,u=c?`Separado`:`Activo`,d=c?`Entregado`:`Cancelado`,f=await De(e,{...r,abonado:i,saldo:a,estado:o||r.estado===`Cancelado`||r.estado===`Entregado`?d:u,fechaCancelacion:o?new Date().toLocaleDateString(`es-CO`):r.fechaCancelacion||``,historialAbonos:Kr(s)});f?.success?(h(o?`✅ ¡Crédito cancelado!`:`Abono registrado`,`success`),l(),Br=!1,await Ur(),Jr(G)):h(f?.mensaje||`Error al guardar`,`error`)}catch(e){h(`Error: `+e.message,`error`)}finally{Vr=!1,c.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Aplicar Abono`,c.disabled=!1}}});let u=document.getElementById(`cred-new-modal`),d=()=>{u?.classList.add(`hidden`),u?.classList.remove(`flex`)};document.getElementById(`cred-new-btn`)?.addEventListener(`click`,()=>{document.getElementById(`cred-new-form`)?.reset(),document.getElementById(`cred-new-cliente`).value=``,document.getElementById(`cred-new-cliente-doc`).value=``,u?.classList.remove(`hidden`),u?.classList.add(`flex`)}),document.getElementById(`cred-new-close`)?.addEventListener(`click`,d),document.getElementById(`cred-new-backdrop`)?.addEventListener(`click`,d),document.getElementById(`cred-select-client-btn`)?.addEventListener(`click`,()=>{on(e=>{document.getElementById(`cred-new-cliente`).value=e.nombre,document.getElementById(`cred-new-cliente-doc`).value=e.cedula||e.documento||e.telefono||``})}),document.getElementById(`cred-save-new-btn`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`cred-new-cliente`).value.trim(),t=document.getElementById(`cred-new-cliente-doc`).value.trim(),n=parseInt((document.getElementById(`cred-new-total`).value||``).replace(/\D/g,``))||0,r=parseInt((document.getElementById(`cred-new-abono`)?.value||``).replace(/\D/g,``))||0,i=document.getElementById(`cred-new-detalle`).value.trim();if(!e||!n){h(`Cliente y monto son requeridos`,`warning`);return}let a=document.getElementById(`cred-save-new-btn`);a.disabled=!0,a.textContent=`Guardando...`;try{let a=await Ee({cliente:e,telefono:t,total:n,detalle:i,historialAbonos:r>0?Kr([{fecha:new Date().toLocaleDateString(`es-CO`),monto:r,nota:`Abono inicial`}]):``});a?.success?(h(`Crédito creado`,`success`),d(),Br=!1,await Ur(),Jr(G)):h(a?.mensaje||`Error al crear crédito`,`error`)}catch(e){h(`Error: `+e.message,`error`)}finally{a.disabled=!1,a.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}})}var Xr=[],Zr=[];function Qr(){return async()=>{document.getElementById(`sales-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();Zr=Xr.filter(e=>(e.id_factura||``).toLowerCase().includes(t)||(e.cliente||``).toLowerCase().includes(t)||(e.cedula||``).toLowerCase().includes(t)),ei()});let e=()=>{let e=document.getElementById(`sale-detail-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)};document.getElementById(`sale-detail-close`)?.addEventListener(`click`,e),document.getElementById(`sale-detail-backdrop`)?.addEventListener(`click`,e),await $r()}}async function $r(){let e=document.getElementById(`sales-history-list`);if(e)try{e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-on-surface-variant italic text-sm">Cargando todas las ventas...</td></tr>`,Xr=await Ce(),Zr=[...Xr],ei()}catch(t){e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-error italic text-sm">Error: ${t.message}</td></tr>`}}function ei(){let e=document.getElementById(`sales-history-list`);if(e){if(Zr.length===0){e.innerHTML=`<tr><td colspan="7" class="p-8 text-center text-on-surface-variant italic text-sm">No se encontraron ventas</td></tr>`;return}e.innerHTML=Zr.map((e,t)=>{let n=new Date(e.fecha).toLocaleDateString(`es-CO`,{day:`2-digit`,month:`short`}),r=new Intl.NumberFormat(`es-CO`).format(e.total||0);return`
    ...
    `}).join(``)}}window.viewSaleDetail=e=>{let t=Xr.find(t=>t.id_factura===e);if(!t)return;let n=document.getElementById(`sale-detail-modal`),r=document.getElementById(`sale-detail-content`),i=e=>new Intl.NumberFormat(`es-CO`).format(e||0);r.innerHTML=`
    <div class="space-y-6">
  ...
  `,n.classList.remove(`hidden`),n.classList.add(`flex`)};var ti=[],ni=!1;function ri(){return async()=>{ii(),ni||=(await ai(),!0)}}function ii(){let e=document.getElementById(`task-new-btn`),t=document.getElementById(`task-modal-close`),n=document.getElementById(`task-modal-backdrop`),r=document.getElementById(`task-form`);e?.replaceWith(e.cloneNode(!0)),t?.replaceWith(t.cloneNode(!0)),n?.replaceWith(n.cloneNode(!0)),r?.replaceWith(r.cloneNode(!0)),document.getElementById(`task-new-btn`)?.addEventListener(`click`,li),document.getElementById(`task-modal-close`)?.addEventListener(`click`,ui),document.getElementById(`task-modal-backdrop`)?.addEventListener(`click`,ui),document.getElementById(`task-form`)?.addEventListener(`submit`,di)}async function ai(){let e=document.getElementById(`task-list`);try{let e=await Ve(),t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`);ti=t.rol===`Técnico de reparación`||t.rol===`Vendedor`?e.filter(e=>e.responsable===t.nombre):e,oi()}catch(t){e.innerHTML=`<li class="p-8 text-center text-error">Error: ${t.message}</li>`}}function oi(){let e=document.getElementById(`task-list`);if(!ti||ti.length===0){e.innerHTML=`<li class="p-12 text-center text-on-surface-variant italic text-sm">No hay tareas. ¡Buen trabajo!</li>`;return}e.innerHTML=ti.map(e=>{let t=e.estado===`Completada`;return`
      <li class="flex items-center gap-4 p-4 hover:bg-surface-container-low transition-colors group">
        ...
        <div class="text-right">
          <span class="text-[9px] font-black px-2 py-0.5 rounded-full ${si(e.prioridad)}">${e.prioridad}</span>
          <p class="text-[10px] text-on-surface-variant mt-1">${ci(e.fecha_vencimiento)}</p>
        </div>
    ...
    `}),e.innerHTML=r}var J=[],_i=!1,vi=!1,yi=null;function bi(){return async()=>{_i||=(await xi(),Ci(),!0),Si(J)}}async function xi(){let e=document.getElementById(`user-table-body`);try{e&&(e.innerHTML=`<tr><td colspan="5" class="p-8 text-center opacity-50">Cargando equipo...</td></tr>`),J=await se()}catch{h(`Error al cargar usuarios`,`error`),J=[]}}function Si(e){let t=document.getElementById(`user-table-body`);if(t){if(e.length===0){t.innerHTML=`<tr><td colspan="5" class="p-10 text-center opacity-40 italic">No hay usuarios registrados</td></tr>`;return}t.innerHTML=e.map(e=>`
  ...
  `).join(``)}}function Ci(){document.getElementById(`user-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();Si(J.filter(e=>e.nombre.toLowerCase().includes(t)||e.email.toLowerCase().includes(t)))}),document.getElementById(`user-new-btn`)?.addEventListener(`click`,()=>wi()),document.getElementById(`user-modal-close`)?.addEventListener(`click`,Ti),document.getElementById(`user-modal-backdrop`)?.addEventListener(`click`,Ti),document.getElementById(`user-form`)?.addEventListener(`submit`,Ei),window.userEdit=e=>{let t=J.find(t=>t.email===e);t&&wi(t)},window.userDelete=async e=>{if(e===JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).email)return h(`No puedes eliminarte a ti mismo`,`warning`);if(confirm(`¿Eliminar al usuario ${e}?`))try{await ue(e),h(`Usuario eliminado`,`success`),await xi(),Si(J)}catch(e){h(e.message,`error`)}}}function wi(e=null){yi=e?e.email:null,document.getElementById(`user-form`).reset(),document.getElementById(`user-modal-title`).textContent=e?`Editar Usuario`:`Nuevo Usuario`,e&&(document.getElementById(`user-input-name`).value=e.nombre,document.getElementById(`user-input-email`).value=e.email,document.getElementById(`user-input-password`).value=e.password,document.getElementById(`user-input-rol`).value=e.rol,document.getElementById(`user-input-estado`).value=e.estado);let t=document.getElementById(`user-modal`);t.classList.remove(`hidden`),t.classList.add(`flex`)}function Ti(){let e=document.getElementById(`user-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)}async function Ei(e){if(e.preventDefault(),vi)return;vi=!0;let t=document.getElementById(`user-save-btn`);t.disabled=!0,t.innerHTML=`Guardando...`;let n=e=>document.getElementById(e).value.trim(),r=n(`user-input-email`).toLowerCase(),i=[r,n(`user-input-password`),n(`user-input-name`),n(`user-input-rol`),n(`user-input-estado`)];try{yi?await le(yi,r,[i[1],i[2],i[3],i[4]]):await ce(i),h(yi?`Actualizado`:`Creado`,`success`),Ti(),await xi(),Si(J)}catch(e){h(e.message,`error`)}finally{vi=!1,t.disabled=!1,t.innerHTML=`Guardar Usuario`}}var Y=[],Di=!1,Oi=!1,X,ki,Ai,ji,Z,Mi,Ni,Pi,Fi,Ii,Li,Ri,zi,Bi,Vi;function Hi(){return async()=>{Ui(),Di||=(await Wi(),Ki(),!0),Gi(Y)}}function Ui(){X=document.getElementById(`ped-table-body`),ki=document.getElementById(`ped-search`),Ai=document.getElementById(`ped-filter-status`),ji=document.getElementById(`ped-new-btn`),Z=document.getElementById(`ped-modal`),Mi=document.getElementById(`ped-modal-close`),Ni=document.getElementById(`ped-modal-backdrop`),Pi=document.getElementById(`ped-form`),Fi=document.getElementById(`ped-save-btn`),Ii=document.getElementById(`ped-id`),Li=document.getElementById(`ped-producto`),Ri=document.getElementById(`ped-categoria`),zi=document.getElementById(`ped-proveedor`),Bi=document.getElementById(`ped-costo`),Vi=document.getElementById(`ped-precio`)}async function Wi(){try{X&&(X.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando reventas...</td></tr>`),Y=await Oe()}catch(e){h(`Error cargando reventas: `+e.message,`error`),Y=[]}}function Gi(e){if(!X)return;if(e.length===0){X.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron reventas</td></tr>`;return}let t=JSON.parse(localStorage.getItem(`adminpro_user`)||`{}`).rol===`Administrador`;X.innerHTML=e.map(e=>{let n=Number(e.costo||0),r=Number(e.precio||0),i=Number(e.utilidad||r-n),a=e=>new Intl.NumberFormat(`es-CO`).format(e);return`
    ...
    `}).join(``)}function Ki(){let e=()=>{if(!ki)return;let e=ki.value.toLowerCase().trim(),t=Ai?Ai.value:``;Gi(Y.filter(n=>{let r=(n.producto||``).toLowerCase().includes(e)||(n.id||``).toLowerCase().includes(e),i=t?n.categoria===t:!0;return r&&i}))};ki?.addEventListener(`input`,e),Ai?.addEventListener(`change`,e);let t=e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))};Bi?.addEventListener(`input`,t),Vi?.addEventListener(`input`,t),ji?.addEventListener(`click`,()=>qi(null)),Mi?.addEventListener(`click`,Ji),Ni?.addEventListener(`click`,Ji),Fi?.addEventListener(`click`,Yi),window.pedDelete=async e=>{if(confirm(`¿Eliminar la reventa ${e}?`))try{let t=await Ae(e);t&&t.success&&(h(`Eliminada`,`success`),await Wi(),Gi(Y))}catch(e){h(e.message,`error`)}}}function qi(e){Z&&(Pi?.reset(),e&&(Ii&&(Ii.value=e.id),Li&&(Li.value=e.producto||``),Bi&&(Bi.value=e.costo||0),Vi&&(Vi.value=e.precio||0)),Z.classList.remove(`hidden`),Z.classList.add(`flex`))}function Ji(){Z?.classList.add(`hidden`),Z?.classList.remove(`flex`)}async function Yi(){if(!Oi){Oi=!0;try{(await ke({producto:Li?.value.trim(),categoria:Ri?.value,proveedor:zi?.value.trim(),costo:parseInt(Bi?.value.replace(/\D/g,``))||0,precio:parseInt(Vi?.value.replace(/\D/g,``))||0})).success&&(h(`Guardado`,`success`),Ji(),await Wi(),Gi(Y))}catch(e){h(e.message,`error`)}finally{Oi=!1}}}var Q=[],Xi=!1,Zi=!1,Qi=null,$={};function $i(){return async()=>{Xi||=(await ea(),ra(),!0),ta(Q)}}async function ea(){let e=document.getElementById(`tech-grid`);try{e&&(e.innerHTML=`<p class="col-span-full text-center p-10 opacity-50 italic">Cargando servicios...</p>`),Q=await je()}catch{h(`Error al cargar datos`,`error`),Q=[]}}function ta(e){let t=document.getElementById(`tech-grid`);if(!t)return;if(e.length===0){t.innerHTML=`<p class="col-span-full text-center p-20 opacity-30 italic text-sm">No hay órdenes de servicio activas</p>`;return}let n=e=>new Intl.NumberFormat(`es-CO`).format(e||0);t.innerHTML=e.map(e=>{let t=(e.precio_final||0)-(e.abono||0);return`
      <div class="bg-surface-container-lowest border border-surface-variant rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group relative">
        <div class="flex justify-between items-start mb-3">
          ...
          <span class="px-2 py-0.5 rounded-full text-[10px] font-bold ${na(e.estado)}">${e.estado}</span>
        </div>
    ...
    `}).join(``)}function na(e){return{Ingresado:`bg-slate-100 text-slate-700`,"En Revisión":`bg-blue-100 text-blue-700`,"En Taller":`bg-blue-100 text-blue-700`,Reparado:`bg-green-100 text-green-700`,Entregado:`bg-emerald-600 text-white`,"Sin Arreglo":`bg-red-100 text-red-700`}[e]||`bg-slate-100 text-slate-600`}function ra(){document.getElementById(`tech-search`)?.addEventListener(`input`,e=>{let t=e.target.value.toLowerCase().trim();ta(Q.filter(e=>e.cliente.toLowerCase().includes(t)||e.id_orden.toLowerCase().includes(t)||e.equipo.toLowerCase().includes(t)))}),document.getElementById(`tech-new-btn`)?.addEventListener(`click`,()=>ia()),document.getElementById(`tech-modal-close`)?.addEventListener(`click`,aa),document.getElementById(`tech-modal-backdrop`)?.addEventListener(`click`,aa),document.getElementById(`tech-form`)?.addEventListener(`submit`,oa),[`recepcion`,`resultado`].forEach(e=>{let t=document.getElementById(`tech-preview-${e}-btn`),n=document.getElementById(`tech-img-${e}`),r=document.getElementById(`tech-preview-${e}-wrap`),i=document.getElementById(`tech-preview-${e}`),a=document.getElementById(`tech-remove-${e}`);t?.addEventListener(`click`,()=>n?.click()),n?.addEventListener(`change`,n=>{let a=n.target.files[0];if(!a)return;let o=new FileReader;o.onload=n=>{$[e]={file:a,base64:n.target.result.split(`,`)[1],mime:a.type},i.src=n.target.result,t.classList.add(`hidden`),r.classList.remove(`hidden`)},o.readAsDataURL(a)}),a?.addEventListener(`click`,()=>{n.value=``,delete $[e],i.src=``,r.classList.add(`hidden`),t.classList.remove(`hidden`)})}),window.techEdit=e=>{let t=Q.find(t=>t.id_orden===e);t&&ia(t)},window.techPrint=e=>{let t=Q.find(t=>t.id_orden===e);if(!t)return;let n=e=>new Intl.NumberFormat(`es-CO`).format(e||0),r=(t.precio_final||0)-(t.abono||0),i=new Date().toLocaleDateString(`es-CO`,{year:`numeric`,month:`2-digit`,day:`2-digit`,hour:`2-digit`,minute:`2-digit`}),a=`
      <html>
      <head>
    ...
    `,o=window.open(``,``,`width=400,height=600`);o?(o.document.open(),o.document.write(a),o.document.close(),o.focus(),setTimeout(()=>{o.print(),o.close()},250)):h(`Por favor permite las ventanas emergentes para imprimir`,`warning`)},window.techDelete=async e=>{if(confirm(`¿Eliminar orden ${e}?`))try{(await Pe(e)).success&&(h(`Orden eliminada`,`success`),await ea(),ta(Q))}catch(e){h(e.message,`error`)}}}function ia(e=null){if(Qi=e?e.id_orden:null,document.getElementById(`tech-form`).reset(),document.getElementById(`tech-modal-title`).textContent=e?`Editar Orden`:`Ingreso a Servicio Técnico`,$={},[`recepcion`,`resultado`].forEach(e=>{document.getElementById(`tech-preview-${e}-btn`)?.classList.remove(`hidden`),document.getElementById(`tech-preview-${e}-wrap`)?.classList.add(`hidden`),document.getElementById(`tech-preview-${e}`).src=``}),e&&(document.getElementById(`tech-cliente`).value=e.cliente,document.getElementById(`tech-equipo`).value=e.equipo,document.getElementById(`tech-falla`).value=e.falla,document.getElementById(`tech-costo`).value=new Intl.NumberFormat(`es-CO`).format(e.precio_final||0),document.getElementById(`tech-estado`).value=e.estado,e.evidencias))try{let t=JSON.parse(e.evidencias);[`recepcion`,`resultado`].forEach(e=>{t[e]&&(document.getElementById(`tech-preview-${e}`).src=t[e],document.getElementById(`tech-preview-${e}-btn`).classList.add(`hidden`),document.getElementById(`tech-preview-${e}-wrap`).classList.remove(`hidden`),$[e]={url:t[e]})})}catch{}let t=document.getElementById(`tech-modal`);t.classList.remove(`hidden`),t.classList.add(`flex`)}function aa(){let e=document.getElementById(`tech-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)}async function oa(e){if(e.preventDefault(),Zi)return;Zi=!0;let t=document.getElementById(`tech-save-btn`);t.disabled=!0;let n=Qi||`ST-${Date.now().toString().slice(-6)}`;t.innerHTML=`<span class="material-symbols-outlined animate-spin">progress_activity</span> Subiendo...`;let r={};for(let e of[`recepcion`,`resultado`])if($[e])if($[e].base64)try{let t=await ie($[e].base64,`${n}_${e}`,$[e].mime);t&&(r[e]=t)}catch(e){console.error(`Error upload evidencia`,e)}else $[e].url&&(r[e]=$[e].url);let i=[n,document.getElementById(`tech-cliente`).value.trim(),`310`,document.getElementById(`tech-equipo`).value.trim(),`S/N`,document.getElementById(`tech-falla`).value.trim(),`0000`,``,0,0,parseInt(document.getElementById(`tech-costo`).value.replace(/\D/g,``))||0,document.getElementById(`tech-estado`).value,JSON.stringify(r)];try{(Qi?await Ne(Qi,i):await Me(i)).success&&(h(Qi?`Actualizado`:`Ingresado`,`success`),aa(),await ea(),ta(Q))}catch(e){h(e.message,`error`)}finally{Zi=!1,t.disabled=!1,t.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}}var sa=[],ca=!1,la=!1,ua,da,fa,pa,ma,ha,ga,_a,va,ya,ba,xa,Sa;function Ca(){return async()=>{wa(),ca||=(await Ta(),Da(),!0),Ea(sa)}}function wa(){ua=document.getElementById(`exp-table-body`),da=document.getElementById(`exp-search`),fa=document.getElementById(`exp-filter-cat`),pa=document.getElementById(`exp-new-btn`),ma=document.getElementById(`exp-modal`),ha=document.getElementById(`exp-modal-close`),ga=document.getElementById(`exp-modal-backdrop`),_a=document.getElementById(`exp-form`),va=document.getElementById(`exp-save-btn`),ya=document.getElementById(`exp-monto`),ba=document.getElementById(`exp-categoria`),xa=document.getElementById(`exp-concepto`),Sa=document.getElementById(`exp-responsable`)}async function Ta(){try{ua.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">Cargando egresos...</td></tr>`,sa=await Ie()}catch(e){h(`Error cargando egresos: `+e.message,`error`),sa=[]}}function Ea(e){if(e.length===0){ua.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-on-surface-variant">No se encontraron egresos</td></tr>`;return}ua.innerHTML=e.map(e=>`
      <tr class="hover:bg-surface-container-low transition-colors">
        <td class="px-4 py-3">
    ...
    `).join(``)}function Da(){let e=()=>{let e=da.value.toLowerCase().trim(),t=fa.value;Ea(sa.filter(n=>{let r=(n.concepto||``).toLowerCase().includes(e)||(n.responsable||``).toLowerCase().includes(e)||(n.id||``).toLowerCase().includes(e),i=t?n.categoria===t:!0;return r&&i}))};da.addEventListener(`input`,e),fa.addEventListener(`change`,e),ya.addEventListener(`input`,e=>{let t=e.target.value.replace(/\D/g,``);if(!t){e.target.value=``;return}e.target.value=new Intl.NumberFormat(`es-CO`).format(parseInt(t,10))}),pa.addEventListener(`click`,()=>Oa()),ha.addEventListener(`click`,ka),ga.addEventListener(`click`,ka),va.addEventListener(`click`,Aa)}function Oa(){_a.reset();try{let e=localStorage.getItem(`adminpro_user`);if(e){let t=JSON.parse(e);Sa.value=t.nombre||t.email}}catch{Sa.value=`Sistema`}ma.classList.remove(`hidden`),ma.classList.add(`flex`),ya.focus()}function ka(){ma.classList.add(`hidden`),ma.classList.remove(`flex`)}async function Aa(){if(!_a.checkValidity()){_a.reportValidity();return}if(!la){la=!0,va.textContent=`Registrando...`,va.disabled=!0;try{let e=await Le({monto:parseInt(ya.value.replace(/\D/g,``))||0,categoria:ba.value,concepto:xa.value.trim(),responsable:Sa.value});e&&e.success?(h(`Egreso registrado`,`success`),ka(),await Ta(),Ea(sa)):h(e?.mensaje||`Error al registrar`,`error`)}catch(e){h(`Error de conexión: `+e.message,`error`)}finally{la=!1,va.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`,va.disabled=!1}}}var ja=[],Ma=[];function Na(){return async()=>{await Pa(),za()}}async function Pa(){try{let e=document.getElementById(`nom-list`);e&&(e.innerHTML=`<tr><td colspan="6" class="p-4 text-center text-sm text-on-surface-variant">Cargando nóminas...</td></tr>`),[ja,Ma]=await Promise.all([Re(),se()]),Fa(ja),Ia(ja)}catch{h(`Error al cargar nóminas`,`error`)}}function Fa(e){let t=document.getElementById(`nom-list`);if(t){if(!e||e.length===0){t.innerHTML=`
      <tr>
    ...
    `}).join(``),document.querySelectorAll(`.nom-del-btn`).forEach(e=>{e.addEventListener(`click`,async e=>{let t=e.currentTarget.dataset.id;if(confirm(`¿Estás seguro de eliminar este registro de nómina?`))try{await Be(t),h(`Nómina eliminada`,`success`),Pa()}catch{h(`Error al eliminar`,`error`)}})})}}function Ia(e){let t=new Date,n=t.getMonth(),r=t.getFullYear(),i=0,a=0;e.forEach(e=>{let t=new Date(e.fecha);t.getMonth()===n&&t.getFullYear()===r&&e.estado!==`Anulado`&&(i+=parseFloat(e.total_pagar)||0),e.estado===`Pendiente`&&(a+=parseFloat(e.total_pagar)||0)});let o=document.getElementById(`nom-stat-mes`),s=document.getElementById(`nom-stat-pendiente`);o&&(o.textContent=`$`+i.toLocaleString(`es-CO`)),s&&(s.textContent=`$`+a.toLocaleString(`es-CO`))}function La(){let e=parseFloat(document.getElementById(`nom-base`).value)||0,t=parseFloat(document.getElementById(`nom-bonos`).value)||0,n=parseFloat(document.getElementById(`nom-deduc`).value)||0,r=e+t-n;document.getElementById(`nom-total-calc`).textContent=`$`+r.toLocaleString(`es-CO`)}var Ra=!1;function za(){if(Ra)return;Ra=!0;let e=document.getElementById(`nom-modal`),t=document.getElementById(`nom-form`);document.getElementById(`nom-new-btn`)?.addEventListener(`click`,()=>{t.reset();let n=document.getElementById(`nom-empleado`);n&&(n.innerHTML=`<option value="">Seleccione empleado...</option>`+Ma.map(e=>`<option value="${e.nombre}">${e.nombre} (${e.rol})</option>`).join(``)),La(),e.classList.remove(`hidden`),e.classList.add(`flex`)}),document.getElementById(`nom-modal-close`)?.addEventListener(`click`,()=>{e.classList.add(`hidden`),e.classList.remove(`flex`)}),document.getElementById(`nom-modal-cancel`)?.addEventListener(`click`,()=>{e.classList.add(`hidden`),e.classList.remove(`flex`)}),document.getElementById(`nom-base`)?.addEventListener(`input`,La),document.getElementById(`nom-bonos`)?.addEventListener(`input`,La),document.getElementById(`nom-deduc`)?.addEventListener(`input`,La),document.getElementById(`nom-save-btn`)?.addEventListener(`click`,async()=>{if(!t.checkValidity()){t.reportValidity();return}let n=parseFloat(document.getElementById(`nom-base`).value)||0,r=parseFloat(document.getElementById(`nom-bonos`).value)||0,i=parseFloat(document.getElementById(`nom-deduc`).value)||0,a=n+r-i,o={empleado:document.getElementById(`nom-empleado`).value,periodo:document.getElementById(`nom-periodo`).value,salario_base:n,bonificaciones:r,deducciones:i,total_pagar:a,estado:document.getElementById(`nom-estado`).value,notas:document.getElementById(`nom-notas`).value},s=document.getElementById(`nom-save-btn`);s.disabled=!0,s.innerHTML=`<span class="material-symbols-outlined text-[18px] animate-spin">refresh</span> Guardando...`;try{await ze(o),h(`Nómina registrada exitosamente`,`success`),e.classList.add(`hidden`),e.classList.remove(`flex`),Pa()}catch(e){h(`Error al guardar: `+e.message,`error`)}finally{s.disabled=!1,s.innerHTML=`<span class="material-symbols-outlined text-[18px]">save</span> Guardar`}})}function Ba(){return()=>{Va(),Ha()}}function Va(){let e=document.getElementById(`set-avatar`),t=document.getElementById(`set-name`),n=document.getElementById(`set-role`),r=document.getElementById(`set-email`);try{let i=localStorage.getItem(`adminproSession`),a=localStorage.getItem(`adminpro_user`),o=i?JSON.parse(i):a?JSON.parse(a):null;o?(t&&(t.textContent=o.nombre||`Usuario`),n&&(n.textContent=o.rol||`Administrador`),r&&(r.textContent=o.email||`No disponible`),e&&(e.textContent=(o.nombre?o.nombre.charAt(0):`U`).toUpperCase())):console.warn(`No se encontró sesión activa para cargar el perfil.`)}catch(e){console.error(`Error loading profile`,e)}let i=document.getElementById(`set-theme-toggle`);i&&(i.checked=document.documentElement.classList.contains(`dark`))}function Ha(){let e=document.getElementById(`set-logout-btn`);e?.replaceWith(e.cloneNode(!0)),document.getElementById(`set-logout-btn`)?.addEventListener(`click`,()=>{confirm(`¿Estás seguro de que deseas cerrar sesión?`)&&ee()});let t=document.getElementById(`set-theme-toggle`);t?.replaceWith(t.cloneNode(!0)),document.getElementById(`set-theme-toggle`)?.addEventListener(`change`,e=>{e.target.checked?(document.documentElement.classList.add(`dark`),localStorage.setItem(`adminpro_theme`,`dark`)):(document.documentElement.classList.remove(`dark`),localStorage.setItem(`adminpro_theme`,`light`))})}`serviceWorker`in navigator&&window.addEventListener(`load`,()=>{navigator.serviceWorker.register(`/adminpro/sw.js`,{scope:`/adminpro/`}).catch(e=>{console.log(`ServiceWorker registration failed: `,e)})});var Ua=``,Wa=``,Ga=null;document.addEventListener(`keydown`,e=>{if(!(e.ctrlKey||e.altKey||e.metaKey))if(e.key===`Enter`){if(Wa.length>=3){let t=Wa;Wa=``,clearTimeout(Ga),document.dispatchEvent(new CustomEvent(`barcodeScanned`,{detail:t})),(document.activeElement?document.activeElement.tagName:``)!==`TEXTAREA`&&e.preventDefault();return}Wa=``}else e.key.length===1&&(Wa+=e.key,clearTimeout(Ga),Ga=setTimeout(()=>{Wa=``},50))});function Ka(){try{let e=JSON.parse(localStorage.getItem(`adminproSession`)||`null`);return e&&Date.now()<e.expiresAt?e:null}catch{return null}}function qa(e,t){d(t),localStorage.setItem(`adminproSession`,JSON.stringify({...e,token:t,expiresAt:Date.now()+480*60*1e3}))}function Ja(){d(null),localStorage.removeItem(`adminproSession`),localStorage.removeItem(`adminpro_user`)}var Ya=[{label:`Inicio`,items:[{id:`dashboard`,label:`Dashboard`,icon:`dashboard`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]}]},{label:`Operaciones`,items:[{id:`pos`,label:`Ventas (POS)`,icon:`point_of_sale`,roles:[`Administrador`,`Vendedor`]},{id:`sales-history`,label:`Historial de Ventas`,icon:`history`,roles:[`Administrador`,`Vendedor`]},{id:`credits`,label:`Créditos`,icon:`credit_score`,roles:[`Administrador`,`Vendedor`]},{id:`expenses`,label:`Egresos`,icon:`payments`,roles:[`Administrador`]},{id:`nominas`,label:`Nóminas`,icon:`request_quote`,roles:[`Administrador`]}]},{label:`Inventario`,items:[{id:`inventory`,label:`Catálogo General`,icon:`inventory_2`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]},{id:`imei`,label:`Equipos IMEI`,icon:`phone_android`,roles:[`Administrador`,`Vendedor`]},{id:`reventas`,label:`Reventas`,icon:`storefront`,roles:[`Administrador`,`Vendedor`]}]},{label:`Servicios`,items:[{id:`technical`,label:`Servicio Técnico`,icon:`build`,roles:[`Administrador`,`Técnico de reparación`]}]},{label:`Organización`,items:[{id:`tasks`,label:`Lista de Tareas`,icon:`check_circle`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]},{id:`calendar`,label:`Actividad`,icon:`history_toggle_off`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]}]},{label:`Personas`,items:[{id:`clients`,label:`Clientes`,icon:`people`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]},{id:`users`,label:`Equipo / Usuarios`,icon:`manage_accounts`,roles:[`Administrador`]}]},{label:`Otros`,items:[{id:`settings`,label:`Ajustes`,icon:`settings`,roles:[`Administrador`,`Vendedor`,`Técnico de reparación`]}]}];function Xa(e){let t=document.getElementById(`mobile-drawer`),n=document.getElementById(`mobile-drawer-backdrop`),r=document.getElementById(`mobile-drawer-content`);e?(t.classList.remove(`hidden`),setTimeout(()=>{n.classList.replace(`opacity-0`,`opacity-100`),r.classList.replace(`translate-y-full`,`translate-y-0`)},10)):(n.classList.replace(`opacity-100`,`opacity-0`),r.classList.replace(`translate-y-0`,`translate-y-full`),setTimeout(()=>t.classList.add(`hidden`),300))}function Za(e,t,n=!1){let r=document.getElementById(e);if(!r)return;let i=t||`Vendedor`,a=``;if(n){let e=[];e=i===`Técnico de reparación`?[{id:`dashboard`,label:`Home`,icon:`dashboard`},{id:`technical`,label:`Reparar`,icon:`build`},{id:`inventory`,label:`Stock`,icon:`inventory_2`},{id:`tasks`,label:`Tareas`,icon:`check_circle`}]:[{id:`dashboard`,label:`Home`,icon:`dashboard`},{id:`pos`,label:`Venta`,icon:`point_of_sale`},{id:`inventory`,label:`Stock`,icon:`inventory_2`},{id:`tasks`,label:`Tareas`,icon:`check_circle`}],a=e.map(e=>`
    ...
    `;let t=document.getElementById(`mobile-drawer-grid`);t&&(t.innerHTML=Ya.flatMap(e=>e.items).filter(e=>!e.roles||e.roles.includes(i)).map(e=>`
      ...
      `).join(``))}else Ya.forEach(e=>{let t=e.items.filter(e=>!e.roles||e.roles.includes(i));t.length!==0&&(a+=`<p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500 mt-6 mb-2 px-4 italic">${e.label}</p>`,t.forEach(e=>{a+=`
        ...
        `}))});r.innerHTML=a,n&&(document.getElementById(`mobile-more-btn`)?.addEventListener(`click`,()=>Xa(!0)),document.getElementById(`mobile-drawer-close`)?.addEventListener(`click`,()=>Xa(!1)),document.getElementById(`mobile-drawer-backdrop`)?.addEventListener(`click`,()=>Xa(!1)),document.querySelectorAll(`#mobile-drawer-grid [data-nav]`).forEach(e=>{e.addEventListener(`click`,()=>{Xa(!1),o(e.dataset.nav)})})),r.querySelectorAll(`[data-nav]`).forEach(e=>{e.addEventListener(`click`,()=>o(e.dataset.nav))})}function Qa(e){document.querySelectorAll(`#desktop-nav [data-nav]`).forEach(t=>{let n=t.dataset.nav===e;t.classList.toggle(`bg-primary`,n),t.classList.toggle(`text-white`,n),t.classList.toggle(`shadow-lg`,n),t.classList.toggle(`text-slate-400`,!n)}),document.querySelectorAll(`#mobile-nav [data-nav]`).forEach(t=>{let n=t.dataset.nav===e;t.classList.toggle(`text-primary`,n),t.classList.toggle(`text-on-surface-variant`,!n)});let t=`AdminPro`;Ya.forEach(n=>{let r=n.items.find(t=>t.id===e);r&&(t=r.label)});let n=document.getElementById(`header-title`);n&&(n.textContent=t)}function $a(e,t){document.getElementById(`login-screen`).classList.add(`hidden`),document.getElementById(`app-shell`).classList.remove(`hidden`);let n=document.getElementById(`user-name`);n&&(n.textContent=e||`Usuario`),Za(`desktop-nav`,t,!1),Za(`mobile-nav`,t,!0);let s=t||`Vendedor`;a(e=>{let t=!1,n=!1;if(Ya.forEach(r=>{let i=r.items.find(t=>t.id===e);i&&(n=!0,(!i.roles||i.roles.includes(s))&&(t=!0))}),n&&!t)return h(`No tienes permiso para acceder a este módulo`,`warning`),`dashboard`}),document.querySelectorAll(`[data-goto]`).forEach(e=>{let t=e.dataset.goto,n=!1;Ya.forEach(e=>{let r=e.items.find(e=>e.id===t);r&&(!r.roles||r.roles.includes(s))&&(n=!0)}),n?e.classList.remove(`hidden`):e.classList.add(`hidden`)}),i(Qa),r(`inventory`,Mt()),r(`dashboard`,Ft()),r(`pos`,Nn()),r(`imei`,cr()),r(`clients`,Mr()),r(`credits`,Hr()),r(`sales-history`,Qr()),r(`tasks`,ri()),r(`calendar`,mi()),r(`users`,bi()),r(`reventas`,Hi()),r(`technical`,$i()),r(`expenses`,Ca()),r(`nominas`,Na()),r(`settings`,Ba()),o(`dashboard`)}function eo(e){let t=document.getElementById(`step-credentials`),n=document.getElementById(`step-pin`);t&&t.classList.toggle(`hidden`,e!==`credentials`),n&&n.classList.toggle(`hidden`,e!==`pin`)}function to(){document.getElementById(`app-shell`).classList.add(`hidden`),document.getElementById(`login-screen`).classList.remove(`hidden`),eo(`credentials`)}async function no(){await ee(),Ja(),to()}window.addEventListener(`session-expired`,()=>{Ja(),to()}),document.getElementById(`login-form`)?.addEventListener(`submit`,ro),document.getElementById(`pin-form`)?.addEventListener(`submit`,io),document.getElementById(`back-to-login`)?.addEventListener(`click`,()=>eo(`credentials`)),document.getElementById(`logout-btn`)?.addEventListener(`click`,no);async function ro(e){e.preventDefault();let t=document.getElementById(`login-btn`),n=document.getElementById(`login-email`).value.trim(),r=document.getElementById(`login-pwd`).value.trim();t.disabled=!0,t.textContent=`Verificando...`;try{let e=await ae(n,r);e.success&&e.step===`pin`?(Ua=n,document.getElementById(`pin-hint`).textContent=`Enviamos un PIN a ${n}`,eo(`pin`),document.getElementById(`login-pin`).focus()):h(e.mensaje||`Credenciales incorrectas`,`error`)}catch{h(`Error de conexión`,`error`)}finally{t.disabled=!1,t.textContent=`Ingresar`}}async function io(e){e.preventDefault();let t=document.getElementById(`pin-btn`),n=document.getElementById(`login-pin`).value.trim();t.disabled=!0,t.textContent=`Verificando...`;try{let e=await oe(Ua,n);e.success&&e.token?(qa({email:e.email,nombre:e.nombre,rol:e.rol},e.token),$a(e.nombre,e.rol)):h(e.mensaje||`PIN incorrecto`,`error`)}catch{h(`Error`,`error`)}finally{t.disabled=!1,t.textContent=`Verificar`}}var ao=Ka();ao&&ao.token&&f()?(d(ao.token),$a(ao.nombre,ao.rol)):(Ja(),to());